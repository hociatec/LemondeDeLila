...
bool SocialDataStore::IsBlocked(int userId) const
{
    return userId > 0 && std::any_of(
        blockedUsers_.begin(),
        blockedUsers_.end(),
...
std::optional<int> SocialDataStore::UserIdAt(SocialSection section, std::size_t index) const
{
    ...
    case SocialSection::Friends:
        return index < friends_.size() ? std::optional<int>{friends_[index].id} : std::nullopt;
    case SocialSection::IncomingRequests:
        return index < incomingRequests_.size()
            ? std::optional<int>{incomingRequests_[index].requester.id}
            : std::nullopt;
    case SocialSection::OutgoingRequests:
        return index < outgoingRequests_.size()
            ? std::optional<int>{outgoingRequests_[index].addressee.id}
            : std::nullopt;
    case SocialSection::Blocked:
        return index < blockedUsers_.size() ? std::optional<int>{blockedUsers_[index].id} : std::nullopt;
    case SocialSection::Profile:
        return profile_.has_value() ? std::optional<int>{profile_->user.id} : std::nullopt;
    }
    ...
    const std::vector<domain::SocialUser>& blockedUsers)
{
    ...
    for (const auto& blockedUser : blockedUsers)
    {
        ...
        if (blockedUser.id <= 0 || !wasFriend || friendIds.contains(blockedUser.id))
        {
            continue;
        }
