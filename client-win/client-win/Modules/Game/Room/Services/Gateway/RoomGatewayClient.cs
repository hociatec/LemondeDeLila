...
public sealed partial class RoomGatewayClient : IRoomGatewayClient
{
    ...
    public async Task<IRoomSession> CreateAndConnectAsync(string gameType, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("Utilisateur non authentifiÈ.");
        }
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                {
                    Log.Information("WS room.create: socket warm rÈutilisÈ");
                }
                ...
                if (roomId <= 0)
                {
                    ...
                    throw new InvalidOperationException("CrÈation de table ÈchouÈe (roomId invalide).");
                }

                Log.Information("Connexion ‡ la room WS (rÈutilisation socket) roomId={RoomId}", roomId);
            ...
            catch (Exception ex)
            {
                ...
                Log.Warning(ex, "WS room.create: Èchec transitoire (tentative {Attempt}/{MaxAttempts}), retry", attempt, maxAttempts);
                await Task.Delay(GameTiming.Room.GatewayRetryDelay, linked.Token).ConfigureAwait(false);
            }
        }

        throw lastError ?? new InvalidOperationException("CrÈation de table ÈchouÈe.");
    }
        ...
        try
        {
            try
            {
                ...
                else
                {
                    ...
                    Log.Information("WS room.connect: connexion √† {Endpoint}", uri);
                ...
                if (payload?.Room == null)
                {
                    await socket.CloseAsync().ConfigureAwait(false);
                    await socket.DisposeAsync().ConfigureAwait(false);
                    throw new InvalidOperationException("Connexion table √©chou√©e (√©tat manquant).");
                }
