...
Restored status metadata is also shape-checked: categories must be arrays and source must be a non-null, non-array object. Five malformed-save cases cover the boundary; status, numeric invariant and primitive parity suites pass (232 tests). This closes the gap between typed author input and restored JSON for the new metadata.
