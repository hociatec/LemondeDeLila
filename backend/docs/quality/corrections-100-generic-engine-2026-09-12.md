...
| 51–63  | `JsonProgramExtensions`, les schémas et les compilateurs sont dérivés d'un registre statique et figé. Chaque `extension.ts` possède schéma, compilation, actions, handlers et validation. Le noyau central n'a plus de table ni de branche par jeu. Le registre n'utilise ni découverte de fichiers, ni import dynamique, ni I/O. |
...
`engine:extensions:audit`, `game-engine:audit`, `architecture:test`,
