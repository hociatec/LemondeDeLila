...
| `runtime/public-api.ts` | ancien barrel interne, supprimé le 8 septembre 2026 | aucun consommateur de production ; le test restant importe les responsabilités concernées ; la surface auteur reste `sdk/public-api.ts` |
...
| `sounds/infrastructure/storage/sounds.service.ts` | façade de stockage audio, manifeste et délégation des opérations | revue actualisée le 11 septembre : manager de maintenance supprimé ; diagnostic, nettoyage et recherche de source sont séparés ; upload, réencodage et ambiances conservent leurs responsabilités propres |
