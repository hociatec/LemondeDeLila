...
const _multererrorfilter = require("./multer-error.filter");
...
let AdminSoundsController = class AdminSoundsController {
    ...
    async upload(soundId, file) {
        ...
        try {
            ...
            throw new _common.InternalServerErrorException(`Upload son échoué: ${message}`.trim());
        } finally{
            try {
    ...
    static sanitizeFilename(originalName) {
        const base = _path.basename(String(originalName || 'sound'));
        const sanitized = Array.from(base, (char)=>{
            const code = char.charCodeAt(0);
            if (code <= 31 || FORBIDDEN_FILENAME_CHARS.has(char)) {
                return '_';
            }
            return char;
        }).join('');
        return sanitized.replace(/\s+/g, ' ').trim().slice(0, 120);
    }
    constructor(sounds){
        this.sounds = sounds;
        this.logger = new _common.Logger(AdminSoundsController.name);
    }
};
_ts_decorate([
...
_ts_decorate([
    ...
    (0, _common.UseFilters)(_multererrorfilter.MulterErrorFilter),
    (0, _common.UseInterceptors)((0, _platformexpress.FileInterceptor)('file', {
        storage: (0, _multer.diskStorage)({
            destination: (_req, _file, cb)=>cb(null, _os.tmpdir()),
            filename: (_req, file, cb)=>cb(null, `lila-sound-${Date.now()}-${AdminSoundsController.sanitizeFilename(file.originalname)}`)
        }),
