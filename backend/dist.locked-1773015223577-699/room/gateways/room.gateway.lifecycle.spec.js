...
function createSocket() {
    return {
        readyState: 1,
        send: jest.fn((_message, cb)=>{
            if (typeof cb === 'function') cb();
        }),
        close: jest.fn()
    };
}
function baseRoomPayload(overrides) {
    const payload = {
        manifest: {
            id: 'lama',
            name: 'Lama',
...
function createGatewayFixture() {
    const roomsService = {
        ...
        updateRoomPayloadCache: jest.fn().mockResolvedValue(baseRoomPayload({
            room: {
                isPrivate: true
            }
        })),
        togglePrivacy: jest.fn().mockResolvedValue({
            id: 10,
            isPrivate: true
        }),
        ...
        resetRoom: jest.fn().mockResolvedValue({
            id: 10
        })
    };
    const botService = {
    ...
    gateway.withAllowedActionsForClient = jest.fn((payload)=>payload);
    return {
        gateway,
        deps: {
            roomsService,
...
describe('RoomGateway lifecycle scenarios', ()=>{
    ...
    it('creates a room, sends room.created, caches payload, and broadcasts room payload', async ()=>{
        ...
        await gateway.handleRoomCreate(socket, meta, {
            gameType: 'lama',
            name: 'Ma table',
            maxPlayers: 5,
            isPrivate: false
        }, Date.now());
        expect(deps.roomsService.createRoom).toHaveBeenCalledWith(1, 'lama', 'Ma table', 5, false, false);
        ...
        const sent = socket.send.mock.calls.map((call)=>JSON.parse(String(call[0])));
        expect(sent.some((m)=>m.type === 'room.created')).toBe(true);
    });
    it('join: rejects banned users with sendError', async ()=>{
        ...
        await gateway.handleRoomJoin(socket, meta, {
            roomId: 10,
            spectator: false
        }, Date.now());
        expect(gateway.sendError).toHaveBeenCalledWith(socket, 'Banni de cette table.');
    ...
    it('join: rejects silent mode for non-admin', async ()=>{
        ...
        expect(socket.close).toHaveBeenCalledWith(4003, 'Mode caché réservé aux admins');
        expect(deps.roomsService.joinRoom).not.toHaveBeenCalled();
    });
    it('join: rejects explicit spectate when canSpectate=false', async ()=>{
        ...
        expect(socket.close).toHaveBeenCalledWith(4003, 'Spectateur non autorise sur cette table');
        expect(deps.roomsService.joinRoom).not.toHaveBeenCalled();
    });
    it('join: allows silent admin and sends private state to the joining socket', async ()=>{
        ...
        expect(gateway.sendRoomStateToClient).toHaveBeenCalledWith(socket, 10, expect.objectContaining({
    ...
    it('join: falls back to spectator when room already started and spectate is allowed', async ()=>{
        ...
        deps.roomsService.joinRoom.mockRejectedValueOnce(new Error('Table déjà démarrée'));
        deps.roomsService.getRoomPayload.mockResolvedValueOnce(baseRoomPayload({
            room: {
                status: 'started',
    ...
    it('join: throws when room started and spectate is denied', async ()=>{
        ...
        gateway.canSpectate.mockResolvedValueOnce(false);
        await expect(gateway.handleRoomJoin(socket, meta, {
            roomId: 10,
            spectator: false
        }, Date.now())).rejects.toThrow('Table déjà démarrée');
    });
    it('set role: rejects role change when game already started', async ()=>{
        ...
        await expect(gateway.handleSetRole(socket, meta, {})).rejects.toThrow('Partie déjà commencée');
    });
    it('set role: toggles participant -> spectator in public room', async ()=>{
        ...
        expect(deps.roomsService.leaveRoom).toHaveBeenCalledWith(10, 1, expect.objectContaining({
            preserveRoom: true,
            preserveOwner: true
        }));
        expect(meta.role).toBe('spectator');
        expect(gateway.sendRoomState).toHaveBeenCalledWith(10);
        const sent = socket.send.mock.calls.map((call)=>JSON.parse(String(call[0])));
        expect(sent.some((m)=>m.type === 'room.role' && m.payload?.spectator === true)).toBe(true);
    });
    it('set role: private room requires invitation for non-owner/non-participant', async ()=>{
        ...
        await expect(gateway.handleSetRole(socket, meta, {
            spectator: false
    ...
    it('toggle privacy: broadcasts privacy state and announcement', async ()=>{
        const { gateway, deps } = createGatewayFixture();
        deps.roomsService.updateRoomPayloadCache.mockResolvedValueOnce(baseRoomPayload({
            room: {
                isPrivate: true
            }
        }));
        ...
        expect(gateway.broadcast).toHaveBeenCalledWith(10, 'room.privacy', expect.objectContaining({
            isPrivate: true
        }));
        expect(gateway.broadcastRoomIntent).toHaveBeenCalledWith(10, expect.objectContaining({
            type: 'announcement'
        }));
    });
    ...
    it('immediate ack: emits room.ack only for immediate actions', ()=>{
        ...
        gateway.sendImmediateAckIfNeeded(socket, meta, 'room.start', {
            _trace: {
                id: 'trace-1',
                sentAtMs
            }
        }, receivedAtMs);
        gateway.sendImmediateAckIfNeeded(socket, meta, 'room.chat.send', {
            _trace: {
                id: 'trace-2',
                sentAtMs
            }
        ...
        const sent = socket.send.mock.calls.map((call)=>JSON.parse(String(call[0])));
        expect(sent.filter((m)=>m.type === 'room.ack')).toHaveLength(1);
    ...
    it('legacy dispatcher routes room.start and room.create actions', async ()=>{
        ...
        const startSpy = jest.spyOn(gateway, 'handleRoomStart').mockResolvedValue(undefined);
        const createSpy = jest.spyOn(gateway, 'handleRoomCreate').mockResolvedValue(undefined);
        await gateway.executeLegacyRoomCommand(socket, meta, 'room.start', {
            foo: 'bar'
        }, Date.now());
        await gateway.executeLegacyRoomCommand(socket, meta, 'room.create', {
            gameType: 'lama'
        }, Date.now());
    ...
    it('room.intent.execute maps alias room.toggle-role to room.set-role', async ()=>{
        ...
        const execSpy = jest.spyOn(gateway, 'executeLegacyRoomCommand').mockResolvedValue(undefined);
        const ackSpy = jest.spyOn(gateway, 'sendImmediateAckIfNeeded').mockImplementation(()=>undefined);
        await gateway.handleRoomIntentExecute(socket, meta, {
            intentId: 'room.toggle-role',
            payload: {
                spectator: true
            }
        }, Date.now());
        expect(ackSpy).toHaveBeenCalledWith(socket, meta, 'room.set-role', expect.objectContaining({
            spectator: true
        }), expect.any(Number));
        expect(execSpy).toHaveBeenCalledWith(socket, meta, 'room.set-role', expect.objectContaining({
            spectator: true
        }), expect.any(Number));
    });
});
