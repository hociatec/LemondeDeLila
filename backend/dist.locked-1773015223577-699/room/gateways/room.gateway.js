...
const _soundsservice = require("../../sounds/sounds.service");
...
let RoomGateway = class RoomGateway {
    ...
    async broadcastRoomPayload(roomId, payload) {
        ...
        if (focusIntent) {
            ...
            await this.broadcastRoomIntent(roomId, {
                type: 'announcement',
                payload: {
                    message: focusIntent.reason === 'room.started' ? 'la partie démare, bon jeux!' : 'Mise à jour de la table en cours.',
    ...
    async handleSetAmbience(client, meta, payload, receivedAtMs) {
        ...
        await this.perf.measure('ws.room.setAmbience.total', async ()=>{
            ...
            if (soundId != null) {
                const activeAmbiences = await this.sounds.listTableAmbiencesWithFilter();
                const selectable = new Set((activeAmbiences.items ?? []).map((a)=>String(a?.soundId ?? '').trim().toLowerCase()));
    ...
    constructor(roomsService, botService, auth, catalog, perf, invites, clientUpdates, wsTickets, realtimeTracker, sounds){
