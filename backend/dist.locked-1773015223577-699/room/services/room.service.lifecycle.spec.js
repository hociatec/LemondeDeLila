...
function buildUser(id, username, roles = []) {
    return {
        id,
        username,
        roles
...
function createFixture() {
            ...
            transaction: jest.fn(async (callback)=>{
                return callback({
                    getRepository: (entity)=>{
                        if (entity === _roomentity.Room) return txRoomRepo;
                        if (entity === _roomparticipantentity.RoomParticipant) return txParticipantRepo;
    ...
    const service = new _roomservice.RoomService(rooms, participants, vaultSnapshots, users, botService, presence, catalog, stats, tracker, config, redisFactory);
    return {
        service,
        deps: {
            rooms,
...
describe('RoomService lifecycle scenarios', ()=>{
    ...
    it('createRoom: creates room + owner participant and invalidates payload cache', async ()=>{
        ...
        const leaveAllSpy = jest.spyOn(service, 'leaveAllRoomsForUser').mockResolvedValue(undefined);
        const invalidateSpy = jest.spyOn(service, 'invalidateRoomPayloadCache').mockResolvedValue(undefined);
        const room = await service.createRoom(1, 'lama', '  Ma table  ', null, false);
        expect(leaveAllSpy).toHaveBeenCalledWith(1);
        expect(deps.txRoomRepo.create).toHaveBeenCalledWith(expect.objectContaining({
            name: 'Ma table',
            gameType: 'lama',
            maxPlayers: 6,
            status: 'setup'
        }));
        expect(deps.txParticipantRepo.create).toHaveBeenCalledWith(expect.objectContaining({
            role: 'owner'
        }));
    ...
    it('createRoom: blocks construction game for non-admin owner', async ()=>{
        ...
        await expect(service.createRoom(1, 'under-construction')).rejects.toThrow('Jeu en construction: réservé aux admins');
    });
    it('createRoom: allows construction game for admin owner', async ()=>{
        ...
        jest.spyOn(service, 'invalidateRoomPayloadCache').mockResolvedValue(undefined);
    ...
    it('joinRoom: allows rejoin in started room when user is already participant', async ()=>{
        ...
        expect(room.id).toBe(10);
        expect(leaveAllSpy).toHaveBeenCalledWith(2, {
            exceptRoomId: 10
        });
        expect(invalidateSpy).toHaveBeenCalledWith(10);
    ...
    it('joinRoom: rejects started room for non-participant', async ()=>{
        ...
        await expect(service.joinRoom(10, 2)).rejects.toThrow('Table déjà démarrée');
    });
    ...
    it('joinRoom: creates participant for open room', async ()=>{
        ...
        const room = await service.joinRoom(10, 2);
        expect(room.id).toBe(10);
        expect(deps.participants.create).toHaveBeenCalledWith(expect.objectContaining({
            role: 'player'
        }));
    ...
    it('joinRoom: blocks construction game for non-admin player', async ()=>{
        ...
        await expect(service.joinRoom(10, 2)).rejects.toThrow('Jeu en construction: réservé aux admins');
    });
    it('startRoom: requires at least two total participants (humans+bots)', async ()=>{
        ...
        deps.participants.count.mockResolvedValueOnce(1);
        deps.botService.countBotsForRoom.mockResolvedValueOnce(0);
        await expect(service.startRoom(10, 1)).rejects.toThrow('Au moins deux participants sont requis');
    });
    ...
    it('leaveRoom: in started game, marks quit and adds replacement bot when humans remain', async ()=>{
        ...
        deps.participants.count.mockResolvedValueOnce(1).mockResolvedValueOnce(1).mockResolvedValueOnce(1);
        deps.botService.countBotsForRoom.mockResolvedValueOnce(0);
        jest.spyOn(service, 'invalidateRoomPayloadCache').mockResolvedValue(undefined);
    ...
    it('leaveAllRoomsForUser: skips exceptRoomId and leaves other active rooms', async ()=>{
        ...
        await service.leaveAllRoomsForUser(1, {
            exceptRoomId: 10
        });
