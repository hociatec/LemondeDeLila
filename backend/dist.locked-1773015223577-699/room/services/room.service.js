   ...
   */ async adminCleanupRooms(opts) {
    ...
    async invalidateRoomPayloadCache(roomId) {
        ...
        try {
            ...
            this.disableRoomPayloadRedis('invalidation cache room impossible (fallback mémoire)', error);
        }
    }
    async updateRoomPayloadCache(roomId, updater) {
        ...
        try {
            ...
            this.disableRoomPayloadRedis('mise à jour cache room impossible (fallback mémoire)', error);
            return null;
        }
    }
   ...
   */ async findLatestActiveRoomForUser(userId) {
    ...
    ensureRedisInitialized() {
        ...
        try {
            ...
            this.redis.on('error', (error)=>{
                ...
                this.disableRoomPayloadRedis('erreur Redis fatale cache room (fallback mémoire)', error);
            });
        } catch (error) {
            this.disableRoomPayloadRedis('initialisation cache room Redis impossible (fallback mémoire)', error);
        }
    }
    async getCachedRoomPayload(roomId) {
        ...
        try {
            ...
            this.disableRoomPayloadRedis('lecture cache room Redis impossible (fallback mémoire)', error);
            return null;
        }
    }
    async persistRoomPayload(roomId, payload) {
        ...
        try {
            ...
            this.disableRoomPayloadRedis('écriture cache room Redis impossible (fallback mémoire)', error);
        }
    }
    disableRoomPayloadRedis(reason, error) {
    ...
    constructor(rooms, participants, vaultSnapshots, users, botService, presenceService, catalog, stats, realtimeTracker, config, redisFactory){
        ...
        this.redis = null;
        this.roomPayloadRedisDisabled = false;
