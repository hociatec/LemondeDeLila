...
describe('GameEngineService', ()=>{
    it('announces the next pawn chooser even if a previous pawn prompt is still recent', ()=>{
        const engine = new _gameengineservice.GameEngineService({}, new _gamecoreservice.GameCoreService(), {
            getHandler: jest.fn(()=>({}))
        }, {}, {}, {}, {}, {}, {}, {}, {});
        const state = {
            status: 'started',
        ...
        const out = engine.appendFirstTurnAnnouncement(state);
        const messages = (out.log ?? []).map((entry)=>String(entry?.message));
    ...
    it('does not duplicate the same pawn chooser announcement', ()=>{
        ...
        const state = {
            ...
            log: [
                {
                    message: "C'est à Mouche de choisir un pion."
                }
            ],
            metadata: {}
        };
