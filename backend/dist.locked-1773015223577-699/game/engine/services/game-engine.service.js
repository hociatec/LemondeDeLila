...
Object.defineProperty(exports, "GameEngineService", {
    enumerable: true,
    get: function() {
...
const _turnlabelservice = require("../../modules/turn/services/turn-label.service");
const _botrunnerservice = require("../../modules/bot/services/bot-runner.service");
const _botschedulerservice = require("../../modules/bot/services/bot-scheduler.service");
const _botsettingsservice = require("../../modules/bot/services/bot-settings.service");
const _gameenginestatestore = require("./game-engine-state.store");
const _validatedactiondto = require("../dto/validated-action.dto");
const _gameerrors = require("../../../common/errors/game-errors");
const _gameloggerservice = require("../../../common/services/game-logger.service");
const _gamestatsservice = require("../../../stats/services/game-stats.service");
const _gridrenderservice = require("../../modules/grid/services/grid-render.service");
const _shortcututils = require("../shortcuts/shortcut-utils");
...
function extractPawnPromptToken(message) {
    ...
    if (!text) return null;
    const withPlayer = /^c['’]est à (.+?) de choisir (?:son|un) pion(?:[.,!?]|$)/i.exec(text);
...
let GameEngineService = class GameEngineService {
   ...
   */ async getStateForUser(roomId, gameType, userId) {
    ...
    exposeStateForUser(state, gameType, userId) {
        ...
        const exposed = handler?.exposeStateForUser ? handler.exposeStateForUser(state, userId) : handler?.exposeState ? handler.exposeState(state) : state;
        ...
        const finalState = (0, _mojibake.fixMojibakeDeep)(this.stripBoardAndGridIfNotStarted(withLifecycle));
        if (byState) {
            byState.set(cacheKey, finalState);
        } else {
            this.exposedStateByUserCache.set(state, new Map([
                [
                    cacheKey,
                    finalState
                ]
    ...
    async handleKeyPress(roomId, gameType, userId, key) {
        ...
        const declared = handler?.getShortcuts ? handler.getShortcuts({
        ...
        const match = shortcuts.find((s)=>{
            ...
            const k = rawKey.toLowerCase().startsWith(prefix) ? rawKey.substring(prefix.length).trim().toUpperCase() : rawKey.trim().toUpperCase();
            return k === normalized;
        });
        ...
        if (match.type === 'interface') {
            ...
            const panel = panels ? panels[panelId] : undefined;
            ...
            if (!message && panelId === 'turn') {
                ...
                if (status === 'finished') {
                    ...
                    const currentPlayerId = typeof state?.turn?.currentPlayerId === 'number' && Number.isFinite(state.turn.currentPlayerId) ? state.turn.currentPlayerId : null;
                    const players = state.players ?? [];
                    const name = currentPlayerId != null ? String(players.find((p)=>Number(p.id) === currentPlayerId)?.username ?? '').trim() : '';
            ...
            if (!message) {
                const toStringList = (raw)=>Array.isArray(raw) ? raw.map((v)=>this.normalizeMetadataString(v)).filter((v)=>v.length > 0) : [];
            ...
            return message ? {
                kind: 'panel',
                panelId,
                message: (0, _mojibake.fixMojibakeString)(message)
            } : null;
        }
        return null;
    }
    ...
    mergeCommonShortcuts(state, declared) {
        ...
        common.push((0, _shortcututils.interfaceShortcut)('T', 'turn'));
        // Rules overlay (client-side): prefer Ctrl+R (avoid interfering with in-game text inputs).
        common.push((0, _shortcututils.interfaceShortcut)('Ctrl+R', 'rules'));
        // Action shortcuts: emit only when action exists in the exposed state.
        const actions = Array.isArray(state?.actions) ? state.actions : [];
        const types = new Set(actions.map((a)=>typeof a?.type === 'string' ? a.type.trim().toLowerCase() : '').filter((t)=>t));
        const hasRoll = Array.isArray(actions) ? actions.some((a)=>(0, _actionservicehelper.isRollActionType)(a?.type)) : false;
        if (hasRoll) {
            common.push((0, _shortcututils.actionShortcut)('ENTER', 'roll'));
        }
        if (types.has('draw')) {
            common.push((0, _shortcututils.actionShortcut)('SPACE', 'draw'));
        }
        if (types.has('lama_pass')) {
            common.push((0, _shortcututils.actionShortcut)('S', 'lama_pass'));
        }
        const out = [];
        const seen = new Set();
        for (const s of [
    ...
    async getInternalState(roomId, gameType) {
        ...
        if (actualGameType && actualGameType !== gameType) {
            ...
            this.cleanupRoom(roomId, gameType);
            throw new _common.BadRequestException('Type de jeu invalide pour cette table');
        }
        ...
        if (existing) {
            ...
            const maybeFinished = previousStatus === 'finished' ? existing : this.forceFinishedIfWinnerDetected(existing);
            ...
            if (roomStatus === 'started' && maybeFinishedStatus === 'finished') {
                ...
                try {
                    ...
                    this.gameLogger.error('Auto-reset room (stale finished) failed', err instanceof Error ? err : undefined, {
                        roomId,
                        gameType
                    });
                }
                try {
                    ...
                    this.gameLogger.error('Auto-reset game state (stale finished) failed', err instanceof Error ? err : undefined, {
                        roomId,
                        gameType
                    });
                }
            ...
            const hasRunId = storedRunId !== null && roomRunId !== null && roomRunId >= 0 && storedRunId >= 0;
            ...
            if (previousStatus === 'started' && roomStatus && roomStatus !== 'started' && roomStatus !== 'finished') {
            ...
            if (previousStatus === 'started' && nextStatus === 'started' && roomStartedAt && storedStartedAt && (hasRunIdChanged || hasMeaningfulStartedAtChange)) {
   ...
   */ async applyActions(roomId, gameType, actions, actorId, allowBotTurn = false) {
    ...
    async applyActionsInternal(roomId, gameType, actions, actorId, allowBotTurn = false, botActorIdOverride = null) {
        ...
        if (!allowBotTurn && (!actorId || Number.isNaN(actorId))) {
            throw new _common.UnauthorizedException('Authentification requise pour jouer.');
        }
        const currentPlayerId = current.turn?.currentPlayerId ?? null;
        const currentPlayer = current.players?.find((p)=>p.id === currentPlayerId);
        const actingPlayer = actorId != null && Number.isFinite(actorId) ? current.players?.find((p)=>p.id === actorId) ?? null : null;
        if (!allowBotTurn) {
            if (!actingPlayer || actingPlayer.isBot) {
                throw new _common.UnauthorizedException('Mode spectateur : action de jeu interdite');
            }
        }
        const allowOutOfTurnActions = (()=>{
            ...
            const allowedTypes = new Set(available.map((a)=>this.normalizeActionType(a.type)).filter((t)=>t.length > 0));
            if (allowedTypes.size === 0) return false;
            const requestedTypes = (Array.isArray(actions) ? actions : []).map((a)=>this.normalizeActionType(a.type)).filter((t)=>t.length > 0);
        ...
        const actorOverride = allowOutOfTurnActions || handler?.validateActor?.(current, actions, actorId ?? null) === true;
        ...
        const botActorId = allowBotTurn ? botActorIdOverride ?? currentPlayerId : null;
        if (allowBotTurn && botActorId == null) {
            throw new _common.BadRequestException('Action bot invalide : acteur introuvable.');
        }
        if (allowBotTurn && typeof botActorId === 'number') {
            const bot = current.players?.find((p)=>p.id === botActorId) ?? null;
            if (!bot?.isBot) {
                throw new _common.BadRequestException('Action bot invalide.');
            }
        }
        ...
        this.gameLogger.logPlayerAction({
            ...
            roomId,
            gameType,
            playerId: allowBotTurn ? botActorId ?? undefined : actorId ?? undefined,
        ...
        if (drawAction) {
            const actionPlayerId = allowBotTurn ? botActorId ?? null : actorId ?? null;
            marked = {
                ...marked,
                lastDraw: {
        ...
        if ((marked.status || '').toLowerCase() === 'finished') {
            ...
            try {
                if (outcomesByPlayerId && Object.keys(outcomesByPlayerId).length > 0) {
                    const endgameMessagesByPlayerId = await this.buildEndgameMessagesByPlayerId(marked.players ?? []);
                    ...
                    const log = Array.isArray(marked.log) ? [
                        ...marked.log
                    ] : [];
                    const recent = new Set(log.slice(-80).map((e)=>String(e?.message ?? '').trim()));
                    let nextLog = log;
                    for (const [playerIdRaw, outcome] of Object.entries(outcomesByPlayerId)){
                        ...
                        const chosen = normalizedOutcome === 'won' ? byPlayer.victoryMessage : byPlayer.defeatMessage;
                        if (!chosen) {
                            continue;
                        }
                        const pid = Number(playerIdRaw);
                        const name = Number.isFinite(pid) && pid > 0 ? nameById.get(pid) ?? `Joueur ${pid}` : `Joueur ${playerIdRaw}`;
        ...
        if ((marked.status || '').toLowerCase() === 'finished') {
            ...
            try {
                ...
                this.gameLogger.error('Finalize finished game failed', err instanceof Error ? err : undefined, {
                    roomId,
                    gameType
                });
            }
            try {
                const endedPayload = await this.buildEndedPayload(roomId, gameType, marked);
                this.endedBroadcaster?.(gameType, roomId, marked, endedPayload);
            } catch (err) {
                this.gameLogger.error('Broadcast game.ended failed', err instanceof Error ? err : undefined, {
                    roomId,
                    gameType
                });
            }
            try {
                ...
                this.gameLogger.error('Schedule finished game reset failed', err instanceof Error ? err : undefined, {
                    roomId,
                    gameType
                });
            }
    ...
    buildPlayersFromPayload(payload) {
        const result = [];
        const roomPlayers = Array.isArray(payload?.room?.players) ? payload.room.players : [];
        ...
        const roomBots = Array.isArray(payload?.room?.bots) ? payload.room.bots : [];
        for (const bot of roomBots){
            const rawId = typeof bot?.id === 'number' ? bot.id : typeof bot?.id === 'string' ? Number(bot.id) : NaN;
   ...
   */ forceFinishedIfWinnerDetected(state) {
        ...
        if (finishedAt.length > 0) {
            return state.status === 'finished' ? state : {
                ...state,
                status: 'finished'
            };
        }
        ...
        for (const key of [
            ...
            const normalizedMeta = key === 'winnerId' ? meta : {
                ...meta,
                winnerId: value
            };
            return {
    ...
    deriveFinishedOutcomes(state) {
        ...
        const humanIdSet = new Set(humans.map((p)=>typeof p?.id === 'number' && Number.isFinite(p.id) ? p.id : null).filter((id)=>id != null));
        ...
        const existingOutcomes = existingOutcomesRaw && Object.keys(existingOutcomesRaw).length > 0 ? Object.fromEntries(Object.entries(existingOutcomesRaw).filter(([key])=>{
            const id = Number(key);
            return Number.isFinite(id) && humanIdSet.has(id);
        })) : null;
        let winnerId = winnerFromMeta;
        if (winnerId == null && existingOutcomes) {
            const winners = Object.entries(existingOutcomes).filter(([, v])=>v === 'won').map(([k])=>Number(k)).filter((n)=>Number.isFinite(n));
            if (winners.length === 1) {
                winnerId = winners[0];
            }
        }
        ...
        if (existingOutcomes && Object.keys(existingOutcomes).length > 0) {
            ...
            outcomesByPlayerId = Object.fromEntries(humans.map((p)=>[
                    String(p.id),
    ...
    async buildEndedPayload(roomId, gameType, state) {
        const metadata = this.toMetadata(state);
        ...
        if (outcomesByPlayerId) {
            for (const [playerId, raw] of Object.entries(outcomesByPlayerId)){
                ...
                if (normalized === 'won' || normalized === 'lost' || normalized === 'draw' || normalized === 'unknown') {
        ...
        const finishedAt = typeof finishedAtRaw === 'string' && finishedAtRaw.trim().length > 0 ? finishedAtRaw.trim() : new Date().toISOString();
        const turnIndex = typeof state?.turnIndex === 'number' && Number.isFinite(state.turnIndex) ? state.turnIndex : null;
        const endgameMessagesByPlayerId = await this.buildEndgameMessagesByPlayerId(players);
        return {
            roomId,
            gameType,
            status: 'finished',
    ...
    async buildEndgameMessagesByPlayerId(players) {
        if (!this.socialProfiles) {
            return {};
        }
        const playerIds = Array.from(new Set((players ?? []).filter((p)=>p && p.isBot !== true).map((p)=>typeof p?.id === 'number' ? p.id : null).filter((id)=>id != null && Number.isFinite(id))));
        ...
        try {
            const rows = await this.socialProfiles.find({
                ...
                where: {
                    userId: (0, _typeorm1.In)(playerIds)
                }
            });
            const out = {};
            for (const row of rows){
                const userId = typeof row?.userId === 'number' && Number.isFinite(row.userId) ? row.userId : null;
                if (userId == null) {
                    continue;
                }
                const victoryMessage = this.normalizeProfileEndgameMessage(row.victoryMessage);
                const defeatMessage = this.normalizeProfileEndgameMessage(row.defeatMessage);
    ...
    normalizeProfileEndgameMessage(value) {
        if (typeof value !== 'string') {
            return null;
        }
        const text = (0, _mojibake.fixMojibakeString)(value).trim();
        if (!text) {
            return null;
        }
    ...
    async playBotTurnInternal(roomId, gameType) {
        this.gameLogger.debug('Bot turn tick', {
            roomId,
            gameType
        });
        ...
        const botPlayer = botActorId != null ? state.players?.find((p)=>p.id === botActorId) : null;
        ...
        if (!botActions || botActions.length === 0) {
            const fallback = handler?.getAvailableActions ? handler.getAvailableActions(state, botActorId) : [];
            if (Array.isArray(fallback) && fallback.length > 0 && botActorId != null) {
    ...
    syncRosterForStartedRoom(state, payload) {
        try {
            let changed = false;
            if (!state || String(state.status ?? '').toLowerCase().trim() !== 'started') {
                return state;
            }
            ...
            if (desiredPlayers.length > 0) {
                const same = players.length === desiredPlayers.length && players.every((p, i)=>p?.id === desiredPlayers[i]?.id);
                if (!same) {
            ...
            const roomBotNames = roomBots.map((b)=>this.normalizeUsernameForLog(b.name)).filter((n)=>n.length > 0);
            ...
            const allowedBotIds = new Set(roomBots.map((b)=>-Math.abs(b.id)).filter((id)=>Number.isFinite(id) && id < 0));
            // Bots already present in the game state (initial bots / already replaced seats).
            const assignedBotNames = new Set(players.filter((p)=>p.isBot === true).map((p)=>this.normalizeUsernameForLog(p.username)).filter((n)=>n.length > 0));
            ...
            const mappedPlayers = players.map((p)=>{
                ...
                if (roomUsername) {
                    if (isBot || this.normalizeUsernameForLog(p.username) !== roomUsername) {
                        changed = true;
                        return {
                            ...p,
                            isBot: false,
            ...
            if (typeof currentPlayerId === 'number' && currentPlayerId !== 0 && !nextPlayers.some((p)=>p?.id === currentPlayerId)) {
            ...
            if (typeof pendingPlayerId === 'number' && pendingPlayerId !== 0 && !nextPlayers.some((p)=>p?.id === pendingPlayerId)) {
                changed = true;
                state = {
                    ...state,
   ...
   */ async restoreInternalState(roomId, gameType, state) {
    ...
    getBotActorIdForState(state, handler) {
        ...
        const pendingPlayerId = pendingPlayerIdRaw != null && Number.isFinite(Number(pendingPlayerIdRaw)) ? Number(pendingPlayerIdRaw) : null;
    ...
    pendingSignature(pending) {
        ...
        return JSON.stringify({
            ...
            initiatorPlayerId: typeof pending.initiatorPlayerId === 'number' ? pending.initiatorPlayerId : null,
            targetPlayerId: typeof pending.targetPlayerId === 'number' ? pending.targetPlayerId : null
        });
    }
    ...
    async applySystemActions(roomId, gameType, actions) {
        await this.enqueueMutation(this.buildKey(roomId, gameType), async ()=>{
            ...
            const fallbackActorId = typeof meta['ownerPlayerId'] === 'number' ? meta['ownerPlayerId'] : current.turn?.currentPlayerId ?? current.players?.[0]?.id ?? null;
    ...
    async scheduleBotTurn(roomId, gameType, state) {
            ...
            if (step === 'round_pause') {
                ...
                this.botScheduler.schedule({
                    ...
                    run: async ()=>{
                        ...
                        if (typeof untilMs === 'number' && latestUntilMs !== null && latestUntilMs !== untilMs) {
                            return;
                        }
                        await this.applySystemActions(roomId, gameType, [
                            {
                                type: 'lama_resume_round',
                                payload: {}
                            }
                        ]);
                    },
        ...
        if (gameType === 'arche-de-mnemosyne') {
            ...
            const questionId = questionMeta && typeof questionMeta['id'] === 'string' ? questionMeta['id'] : null;
        ...
        const handler = this.registry.getHandler(gameType);
        const botActorId = this.getBotActorIdForState(state, handler);
        const botPlayer = botActorId != null ? state.players?.find((p)=>p.id === botActorId) ?? null : null;
        if (!botPlayer?.isBot) {
            this.botScheduler.clear(key);
            return;
        }
        ...
        const pending = state.pending ?? null;
        const pendingType = typeof pending?.type === 'string' ? pending.type.trim().toLowerCase() : '';
        const isQuizPending = gameType === 'arche-de-mnemosyne' && pending?.type === 'quiz';
        const configMeta = this.getMetadataObject(meta, 'config');
        const quizTimerSeconds = isQuizPending && configMeta && typeof configMeta['timerSeconds'] === 'number' ? Number(configMeta['timerSeconds']) : null;
        const quizTimerMs = quizTimerSeconds != null && Number.isFinite(quizTimerSeconds) ? Math.max(1, quizTimerSeconds) * 1000 : null;
        ...
        const stateForSchedule = immediateStart ? {
            ...
            metadata: {
                ...meta,
                botImmediateStartPending: false
            }
        } : state;
        ...
        this.botScheduler.schedule({
            ...
            run: async ()=>{
                ...
                if (latestTurnIndex !== expectedTurnIndex || latestCurrentPlayerId !== expectedCurrentPlayerId || latestBotActorId !== expectedBotActorId || latestPendingSig !== expectedPendingSig) {
    ...
    async checkAccess(roomId, userId, ownerOnly = false) {
        ...
        if (ownerOnly && !isOwner) {
            throw new _common.UnauthorizedException('Seul le propriétaire peut effectuer cette action');
        }
        if (!ownerOnly && !isParticipant && !isOwner) {
            throw new _common.UnauthorizedException('Accès non autorisé à cette table');
        }
    }
    ...
    ensureRandomStarterAtGameStart(baseState, state) {
        ...
        const starterId = typeof baseStarterId === 'number' && players.some((p)=>p?.id === baseStarterId) ? baseStarterId : players[0]?.id ?? null;
    ...
    appendFirstTurnAnnouncement(state) {
        ...
        const recentMessages = log.slice(-6).map((entry)=>String(entry?.message ?? '').trim());
        if (pendingType === 'choose_pawn') {
            const expectedPromptToken = `prompt:choose-pawn:${normalizePromptToken(name)}`;
            const hasSamePrompt = recentMessages.some((message)=>extractPawnPromptToken(message) === expectedPromptToken);
            if (hasSamePrompt) {
                return state;
            }
            return this.core.appendLog(state, `C'est à ${name} de choisir un pion.`);
        }
        if (recentMessages.some((message)=>message.toLowerCase().startsWith("c'est au tour de "))) {
            return state;
        }
        ...
        this.botScheduler.schedule({
            ...
            run: async ()=>{
                await this.enqueueMutation(this.buildKey(roomId, gameType), async ()=>{
                    ...
                    if (expectedFinishedAt && latestFinishedAt && latestFinishedAt !== expectedFinishedAt) {
                        return;
                    }
                    try {
                        await this.rooms.resetRoomSystem(roomId);
                    } catch (err) {
                        this.gameLogger.error('Auto-reset room after game finished failed', err instanceof Error ? err : undefined, {
                            roomId,
                            gameType
                        });
                    }
                    ...
                    try {
                        ...
                        this.gameLogger.error('Auto-reset game state after finish failed', err instanceof Error ? err : undefined, {
                            roomId,
                            gameType
                        });
                    }
                    ...
                    try {
                        ...
                        this.gameLogger.error('Broadcast fresh state after finish failed', err instanceof Error ? err : undefined, {
                            roomId,
                            gameType
                        });
                    }
    ...
    async normalizeBotThinking(roomId, gameType, state) {
        const since = typeof state.botThinkingSince === 'number' ? state.botThinkingSince : null;
    ...
    async validateActions(state, handler, actions, actorId) {
        ...
        if (list.length > GameEngineService.MAX_ACTIONS_PER_MESSAGE) {
            throw new _common.BadRequestException("Trop d'actions dans un seul message");
        }
        // Step 1: Validate DTOs with class-validator
        let validatedDtos = [];
        try {
            validatedDtos = await (0, _validatedactiondto.validateActions)(list, {
        ...
        const sanitized = validatedDtos.map((dto)=>(0, _validatedactiondto.sanitizeAction)(dto));
        ...
        for (const action of sanitized){
            ...
            if (type.length > GameEngineService.MAX_ACTION_TYPE_LENGTH) {
                throw new _common.BadRequestException('Action invalide : type trop long');
            }
            if (allowedTypes && !allowedTypes.has(type)) {
                if (this.shouldSilentlyIgnoreUnavailableAction(type, action, state, actorId ?? null)) {
                    continue;
                }
                throw new _common.BadRequestException(`Action inconnue ou indisponible: ${type}`);
            }
            let payloadBytes = 0;
            ...
            if (payload != null) {
                try {
                    ...
                    throw new _common.BadRequestException('Action invalide : payload non sérialisable');
                }
            }
            if (payloadBytes > GameEngineService.MAX_ACTION_PAYLOAD_BYTES) {
                throw new _common.BadRequestException('Action invalide : payload trop volumineux');
            }
            totalBytes += payloadBytes;
            if (totalBytes > GameEngineService.MAX_MESSAGE_PAYLOAD_BYTES) {
                throw new _common.BadRequestException('Message invalide : payload total trop volumineux');
            }
            ...
            if (handler?.validateAction) {
                try {
                    ...
                    this.gameLogger.logValidationFailure(`Game-specific validation failed for action: ${type}`, [
                        {
                            actionType: type,
                            error: message
                        }
                    ...
                    throw new _common.BadRequestException(message || `Action invalide: ${type}`);
                }
            }
            out.push(normalized);
        }
    ...
    isOutOfTurnMessage(message) {
        ...
        return normalized.includes('pas votre tour') || normalized.includes("n'est pas votre tour") || normalized.includes('attendez votre tour') || normalized.includes('tour en cours');
    }
    exposeState(state, gameType) {
        ...
        const label = this.turnLabel.compute(state, gameType);
        const handler = this.registry.getHandler(gameType);
        const exposed = handler?.exposeState ? handler.exposeState(state) : state;
        const withLabel = this.attachTurnLabel(exposed, label);
        ...
        return (0, _mojibake.fixMojibakeDeep)(this.stripBoardAndGridIfNotStarted(withLifecycle));
    }
    ...
    appendBoardArrivalAnnouncements(_gameType, handler, previous, next) {
        try {
            ...
            }).filter((p)=>p != null && p.nextPos != null && p.prevPos != null && p.nextPos !== p.prevPos).sort((a, b)=>a.id - b.id);
            if (changed.length === 0) {
                return next;
            }
            let out = next;
            for (const p of changed){
                ...
                const hasRecentArrival = recentMsgs.some((m)=>{
                    ...
                    return lower.includes(needleByNumber) || needleByLabel && lower.includes(needleByLabel) || lower.includes(needleByPlacement);
                });
                if (hasRecentArrival) {
                    continue;
                }
    ...
    appendSkipTurnAnnouncements(state) {
        try {
            ...
            for (const entry of skipped){
                ...
                if (id == null) continue;
                const remaining = typeof data['remainingAfter'] === 'number' ? data['remainingAfter'] : 0;
    ...
    attachViewerContext(state, userId) {
        ...
        const viewerUsername = viewerPlayer && typeof viewerPlayer.username === 'string' ? viewerPlayer.username : viewerPlayer ? `Joueur ${viewerPlayer.id}` : null;
        return {
            ...state,
            extras: {
                ...extras,
    ...
    attachUiDescriptors(state) {
        ...
        const uiNow = uiExistingNow ? {
            ...uiExistingNow
        } : {};
        const panelsExistingNow = GameEngineService.extractPanels(uiExistingNow);
        const panelsNow = panelsExistingNow ? {
            ...panelsExistingNow
        } : {};
        const existingTurn = panelsNow['turn'];
        const existingTurnMessage = existingTurn && typeof existingTurn['message'] === 'string' ? existingTurn['message'] : null;
        const hasTurnMessage = typeof existingTurnMessage === 'string' && existingTurnMessage.trim().length > 0;
        if (!hasTurnMessage) {
            panelsNow['turn'] = {
                title: 'Tour',
        ...
        const panels = panelsExisting ? {
            ...panelsExisting
        } : {};
        const hasGameDefinedPanels = Object.keys(panels).some((id)=>id !== 'turn');
        const currentPlayerView = extrasAfter['currentPlayerView'] ?? null;
        const metadata = stateWithTurnPanel.metadata && typeof stateWithTurnPanel.metadata === 'object' ? stateWithTurnPanel.metadata : {};
        const upsertPanel = (id, title, message)=>{
            if (!id || !title || !message) return;
            const existing = panels[id];
            const existingMessage = existing && typeof existing['message'] === 'string' ? existing['message'] : null;
            const hasMessage = typeof existingMessage === 'string' && existingMessage.trim().length > 0;
        ...
        const buildListMessage = (title, itemsRaw)=>{
            const items = Array.isArray(itemsRaw) ? itemsRaw.map((x)=>this.normalizeMetadataString(x)).filter((x)=>x) : [];
            if (items.length === 0) return `${title}: (vide)`;
            const max = 12;
            const shown = items.length > max ? items.slice(0, max) : items;
            const body = shown.join(', ');
            return items.length > max ? `${title}: ${body}, ... (+${items.length - max})` : `${title}: ${body}`;
        };
        ...
        const buildJoinedLinesMessage = (title, linesRaw)=>{
            const lines = Array.isArray(linesRaw) ? linesRaw.map(normalizeSentence).filter((x)=>x) : [];
            if (lines.length === 0) return `${title}: inconnue.`;
            return lines.join(' ');
        };
        if (!hasGameDefinedPanels && currentPlayerView && typeof currentPlayerView === 'object') {
        ...
        const maxPollution = typeof metadata['maxPollution'] === 'number' ? metadata['maxPollution'] : null;
        if (!hasGameDefinedPanels && (pollution !== null || maxPollution !== null)) {
            let message = 'Pollution: inconnue.';
            if (pollution !== null && maxPollution !== null) message = `Pollution: ${pollution}/${maxPollution}.`;
    ...
    attachStartLifecycle(state, userId) {
        const status = String(state?.status ?? '').toLowerCase().trim();
        const pendingType = String(state?.pending?.type ?? '').toLowerCase().trim();
        const currentPlayerId = typeof state?.turn?.currentPlayerId === 'number' ? state.turn.currentPlayerId : null;
        ...
        const pendingPlayerId = typeof pendingPlayerIdRaw === 'number' ? pendingPlayerIdRaw : Number(pendingPlayerIdRaw);
        const viewerPendingTurn = userId != null && Number.isFinite(pendingPlayerId) && pendingPlayerId === userId;
        const viewerPendingFallback = userId != null && !Number.isFinite(pendingPlayerId) && currentPlayerId != null && currentPlayerId === userId;
        const started = status === 'started';
        const hasConfigPrompt = pendingType === 'config_prompt' || pendingType.endsWith('_set_config');
        const startReady = started && !hasConfigPrompt;
        const viewerMustChoosePawn = userId != null && started && this.isPawnPendingType(pendingType) && (viewerPendingTurn || viewerPendingFallback);
        const viewerTurnActionable = userId != null && started && currentPlayerId != null && currentPlayerId === userId && hasActions && !botThinking && !viewerMustChoosePawn;
        const metadataRaw = state?.metadata && typeof state.metadata === 'object' ? state.metadata : {};
        const lifecycleRaw = metadataRaw['lifecycle'] && typeof metadataRaw['lifecycle'] === 'object' ? metadataRaw['lifecycle'] : {};
        ...
        if (typeof currentStartReady === 'boolean' && currentStartReady === startReady && typeof currentViewerTurnActionable === 'boolean' && currentViewerTurnActionable === viewerTurnActionable && typeof currentViewerMustChoosePawn === 'boolean' && currentViewerMustChoosePawn === viewerMustChoosePawn) {
            return state;
        }
        return {
            ...state,
    ...
    isPawnPendingType(pendingType) {
        const normalized = String(pendingType ?? '').trim().toLowerCase();
    ...
    constructor(rooms, core, registry, turnLabel, botRunner, botScheduler, botSettings, gridRender, store, gameLogger, stats, socialProfiles){
        this.rooms = rooms;
        this.core = core;
        ...
        this.exposedStateByUserCache = new WeakMap();
    }
};
GameEngineService.MAX_ACTIONS_PER_MESSAGE = 12;
GameEngineService.MAX_ACTION_TYPE_LENGTH = 64;
GameEngineService.MAX_ACTION_PAYLOAD_BYTES = 16 * 1024;
GameEngineService.MAX_MESSAGE_PAYLOAD_BYTES = 64 * 1024;
GameEngineService.BOT_THINKING_TTL_MS = 25_000;
GameEngineService.FINISHED_STATE_GRACE_MS = 5_000;
GameEngineService = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_param(11, (0, _common.Optional)()),
    _ts_param(11, (0, _typeorm.InjectRepository)(_socialprofileentity.SocialProfile)),
    ...
    _ts_metadata("design:paramtypes", [
        ...
        typeof _turnlabelservice.TurnLabelService === "undefined" ? Object : _turnlabelservice.TurnLabelService,
        typeof _botrunnerservice.BotRunnerService === "undefined" ? Object : _botrunnerservice.BotRunnerService,
        typeof _botschedulerservice.BotSchedulerService === "undefined" ? Object : _botschedulerservice.BotSchedulerService,
        typeof _botsettingsservice.BotSettingsService === "undefined" ? Object : _botsettingsservice.BotSettingsService,
        typeof _gridrenderservice.GridRenderService === "undefined" ? Object : _gridrenderservice.GridRenderService,
        typeof _gameenginestatestore.GameEngineStateStore === "undefined" ? Object : _gameenginestatestore.GameEngineStateStore,
        typeof _gameloggerservice.GameLoggerService === "undefined" ? Object : _gameloggerservice.GameLoggerService,
