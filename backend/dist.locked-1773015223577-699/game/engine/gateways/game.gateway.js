...
let GameGateway = class GameGateway {
    async handleConnection(client, ...args) {
        ...
        const ticketValidation = this.wsTickets.validateIfTokenPresentDetailed(client, args, 'game', true);
        if (!ticketValidation.ok) {
            this.logger.warn(`Connexion WS game refusée (ticket) reason=${ticketValidation.reason} ticketPresent=${ticketValidation.ticketPresent} userId=${auth.id} clientVersion=${clientVersion ?? 'n/a'}`);
        ...
        const interval = setInterval(()=>{
            ...
            if (Date.now() - last > this.pingIntervalMs * 2) {
                ...
                (0, _playinglogger.playingLog)('ws.game.heartbeat.timeout', {
    ...
    handleDisconnect(client) {
        ...
        if (meta) {
            (0, _playinglogger.playingLog)('ws.game.disconnect', {
                userId: meta.userId,
    ...
    async handleMessage(client, raw) {
        ...
        try {
                ...
                case 'game.ping':
                    this.safeSend(client, {
                        ...
                        payload: {
                            ...
                            clientSentAtMs: typeof payload?.clientSentAtMs === 'number' ? payload.clientSentAtMs : null
                        }
                    });
                    break;
                case 'game.state':
            ...
            (0, _playinglogger.playingLog)('ws.game.error', {
    ...
    async handleJoin(client, meta, payload) {
        const roomId = Number(this.parseNumberFromPayload(payload, 'roomId', 'room')) || 0;
        ...
        await this.perf.measure('ws.game.join.total', async ()=>{
            ...
            (0, _playinglogger.playingLog)('ws.game.join', {
                userId: meta.userId,
                roomId,
                gameType
            });
    ...
    async handleRules(client, meta, payload) {
        const gameType = (this.parseStringFromPayload(payload, 'gameType') ?? meta.gameType ?? '').trim();
        if (!gameType) {
    ...
    async handleState(client, meta, payload) {
        ...
        const { traceId, sentAtMs } = this.extractTrace(payload);
        const clientToServerMs = this.computeClientLatency(receivedAtMs, sentAtMs);
        await this.perf.measure('ws.game.state.total', async ()=>{
            await this.engine.checkReadAccess(roomId, meta.userId);
            const state = await this.engine.getStateForUser(roomId, gameType, meta.userId);
            this.setRoom(meta, roomId, gameType, client);
            (0, _playinglogger.playingLog)('ws.game.state.request', {
                userId: meta.userId,
                roomId,
                gameType
            });
    ...
    async handleTurn(client, meta, payload) {
        const ctx = await this.ensureRoomContext(client, meta, payload);
        if (!ctx) {
        ...
        const { traceId, sentAtMs } = this.extractTrace(payload);
        const clientToServerMs = this.computeClientLatency(receivedAtMs, sentAtMs);
        await this.perf.measure('ws.game.turn.total', async ()=>{
            await this.engine.checkReadAccess(roomId, meta.userId);
            const state = await this.engine.getStateForUser(roomId, gameType, meta.userId);
            this.setRoom(meta, roomId, gameType, client);
            const currentPlayerId = state?.turn?.currentPlayerId ?? null;
            const players = Array.isArray(state?.players) ? state.players : [];
            const current = players.find((p)=>p?.id === currentPlayerId) ?? null;
            const currentUsername = (()=>{
                let name = typeof current?.username === 'string' ? current.username : '';
            ...
            (0, _playinglogger.playingLog)('ws.game.turn.request', {
                userId: meta.userId,
                roomId,
                gameType,
    ...
    async handleActions(client, meta, payload) {
        const ctx = await this.ensureRoomContext(client, meta, payload);
        if (!ctx) return;
        const { roomId, gameType } = ctx;
        const receivedAtMs = Date.now();
        ...
        await this.perf.measure('ws.game.actions.total', async ()=>{
            this.setRoom(meta, roomId, gameType, client);
            const actions = this.extractIncomingActions(payload);
            (0, _playinglogger.playingLog)('ws.game.actions', {
                userId: meta.userId,
                roomId,
                gameType,
    ...
    async handleKey(client, meta, payload) {
        const ctx = await this.ensureRoomContext(client, meta, payload);
        ...
        const { traceId, sentAtMs } = this.extractTrace(payload);
        const clientToServerMs = this.computeClientLatency(receivedAtMs, sentAtMs);
        await this.perf.measure('ws.game.key.total', async ()=>{
            ...
            const result = await this.engine.handleKeyPress(roomId, gameType, meta.userId, normalized);
            if (!result) {
                return;
            }
            if (result.kind === 'action') {
                await this.engine.applyActions(roomId, gameType, result.actions, meta.userId);
                this.safeSend(client, {
                    type: 'game.ack',
                    payload: {
                        action: 'game.key',
    ...
    async handleBot(meta, payload) {
        const roomId = Number(this.parseNumberFromPayload(payload, 'roomId') ?? meta.roomId ?? 0);
        const gameType = this.parseStringFromPayload(payload, 'gameType') ?? meta.gameType ?? '';
        if (!roomId || !gameType) {
            return;
        }
        await this.perf.measure('ws.game.bot.play.total', async ()=>{
            ...
            (0, _playinglogger.playingLog)('ws.game.bot.play', {
                userId: meta.userId,
                roomId,
                gameType
            });
    ...
    broadcastState(gameType, roomId, state) {
        ...
        for (const socket of Array.from(targets)){
            ...
            const encodedPatch = previousPayload ? this.buildEncodedPatch(previousPayload, payload, fullStateEncoded) : null;
        ...
        (0, _playinglogger.playingLog)('ws.game.broadcast', {
            roomId,
            gameType,
    ...
    broadcastEnded(gameType, roomId, state, payload) {
        const room = this.buildRoomKey(gameType, roomId);
        const targets = this.rooms.get(room);
        if (!targets) return;
        for (const socket of Array.from(targets)){
            ...
            try {
                const exposed = this.engine.exposeStateForUser(state, gameType, userId);
                const extras = exposed?.extras && typeof exposed.extras === 'object' ? exposed.extras : {};
                const viewerPayloadId = this.parseNumberFromPayload(extras, 'viewerPlayerId');
            ...
            const outcomesByPlayerId = payload.outcomesByPlayerId && typeof payload.outcomesByPlayerId === 'object' ? payload.outcomesByPlayerId : {};
            const viewerOutcome = (()=>{
            ...
            const endgameMessagesByPlayerId = payload.endgameMessagesByPlayerId && typeof payload.endgameMessagesByPlayerId === 'object' ? payload.endgameMessagesByPlayerId : {};
            const viewerEndgameMessage = (()=>{
                ...
                const raw = viewerOutcome.toLowerCase() === 'won' ? byPlayer.victoryMessage : viewerOutcome.toLowerCase() === 'lost' ? byPlayer.defeatMessage : null;
            ...
            const publicEndgameMessagesByPlayerId = (()=>{
                ...
                for (const [playerId, rawOutcome] of Object.entries(outcomesByPlayerId)){
                    ...
                    const outcome = typeof rawOutcome === 'string' ? rawOutcome : typeof rawOutcome === 'number' || typeof rawOutcome === 'boolean' ? String(rawOutcome) : '';
                    ...
                    const chosen = outcome === 'won' ? byPlayer.victoryMessage : byPlayer.defeatMessage;
            ...
            const { endgameMessagesByPlayerId: _hiddenMessagesByPlayerId, ...publicPayload } = payload;
        ...
        (0, _playinglogger.playingLog)('ws.game.ended', {
            roomId,
            gameType,
            subscribers: targets ? targets.size : 0,
    ...
    setRoom(meta, roomId, gameType, client) {
        if (meta.roomId && meta.gameType) {
    ...
    forceDisconnectRoomClients(roomId) {
        const targets = Array.from(this.clients.entries()).filter(([, meta])=>meta?.roomId === roomId).map(([socket])=>socket);
        if (targets.length === 0) {
            return;
        }
    ...
    sendError(client, message, context) {
        if (client.readyState !== WS_READY_STATE_OPEN) return;
        this.safeSend(client, {
            type: 'error',
            context,
    ...
    unsafeStringify(payload) {
        return JSON.stringify(payload);
    }
    sanitizeOutgoingPayload(payload) {
        return (0, _mojibake.fixMojibakeDeep)(payload);
    }
    safeSend(client, payload) {
        ...
        try {
            ...
            this.logger.warn('Echec envoi WS game', err);
            try {
                client.close();
            } catch  {
            /* ignore */ }
    ...
    sendState(client, payload) {
        ...
        try {
            ...
            this.logger.warn('Echec envoi WS game.state', err);
            this.lastPayloadBySocket.delete(client);
            try {
                client.close();
            } catch  {
    ...
    getEncodedStateMessage(payload) {
    ...
    buildEncodedPatch(previousPayload, nextPayload, fullStateEncoded) {
    ...
    buildTopLevelPatch(previousPayload, nextPayload) {
        if (!previousPayload || typeof previousPayload !== 'object' || !nextPayload || typeof nextPayload !== 'object') {
            return null;
        }
        ...
        const baseTurnIndex = typeof rawBaseTurnIndex === 'number' && Number.isFinite(rawBaseTurnIndex) ? rawBaseTurnIndex : null;
        return {
            baseTurnIndex,
            set,
            unset
    ...
    async tryAutoJoinFromUrl(client, args) {
        const meta = this.clients.get(client);
        if (!meta) return;
        const params = this.extractJoinParams(client, args);
        if (!params) return;
        const { roomId, gameType } = params;
        await this.perf.measure('ws.game.auto_join.total', async ()=>{
            await this.engine.checkReadAccess(roomId, meta.userId);
            const state = await this.engine.getStateForUser(roomId, gameType, meta.userId);
            this.setRoom(meta, roomId, gameType, client);
            (0, _playinglogger.playingLog)('ws.game.auto_join', {
                userId: meta.userId,
                roomId,
                gameType
            });
    ...
    async ensureRoomContext(client, meta, payload) {
        const payloadRoomId = this.parseNumberFromPayload(payload, 'roomId', 'room') ?? 0;
        const payloadGameType = this.parseStringFromPayload(payload, 'gameType', 'game') ?? '';
        ...
        if (metaRoomId > 0 && metaGameType) {
            return {
                roomId: metaRoomId,
                gameType: metaGameType
            };
        }
        // Fallback for "warm" sockets: infer from active participation in DB.
        try {
            const inferred = await this.roomService.findLatestActiveRoomForUser(meta.userId);
        ...
        try {
            ...
            if (urlCandidate) {
                ...
                const roomId = Number(url.searchParams.get('roomId') || url.searchParams.get('room') || 0);
                const gameType = (url.searchParams.get('gameType') || url.searchParams.get('game') || '').trim().toString();
    ...
    extractTrace(payload) {
        ...
        const traceId = typeof trace['id'] === 'string' && trace['id'].trim() ? trace['id'].trim() : null;
        const sentAtMs = typeof trace['sentAtMs'] === 'number' && Number.isFinite(trace['sentAtMs']) ? trace['sentAtMs'] : null;
        return {
            traceId,
            sentAtMs
        };
    }
    computeClientLatency(receivedAtMs, sentAtMs) {
        if (sentAtMs === null) {
            return null;
        }
        return Math.max(0, receivedAtMs - sentAtMs);
    }
    parseNumberFromPayload(payload, ...keys) {
        if (!payload) {
            return null;
        }
    ...
    parseStringFromPayload(payload, ...keys) {
        if (!payload) {
            return null;
        }
        for (const key of keys){
    ...
    extractIncomingActions(payload) {
    ...
    resolveUrlCandidate(client, args) {
        const wsClient = client;
        const request = Array.isArray(args) ? args[0] : undefined;
        const candidate = (request && typeof request === 'object' && 'url' in request && typeof request.url === 'string' && request.url) ?? wsClient.url ?? wsClient.upgradeReq?.url ?? wsClient.req?.url ?? null;
        if (typeof candidate !== 'string') {
            return null;
        }
        return candidate;
    }
    buildRoomKey(gameType, roomId) {
        return `${gameType}:${roomId}`;
    }
    constructor(engine, auth, perf, clientUpdates, wsTickets, roomService, content){
        this.engine = engine;
        this.auth = auth;
        this.perf = perf;
        ...
        this.encodedStateMessageCache = new WeakMap();
        this.topLevelSnapshotCache = new WeakMap();
        this.lastPayloadBySocket = new WeakMap();
        this.pingIntervalMs = 25000;
        this.pongGraceMs = 4000;
        this.minPatchBytesSaved = 96;
        ...
        this.engine.setEndedBroadcaster((gameType, roomId, state, payload)=>this.broadcastEnded(gameType, roomId, state, payload));
        this.roomService.setRoomDeletedNotifier((roomId)=>{
            this.forceDisconnectRoomClients(roomId);
        });
        // Log d'amorçage pour vérifier le chemin de log.
        (0, _playinglogger.playingLog)('ws.game.gateway.init', {
