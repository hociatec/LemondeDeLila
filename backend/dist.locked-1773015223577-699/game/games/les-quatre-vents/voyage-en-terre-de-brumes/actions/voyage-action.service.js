...
Object.defineProperty(exports, "VoyageActionService", {
    enumerable: true,
    get: function() {
        return VoyageActionService;
    }
});
...
let VoyageActionService = class VoyageActionService {
    ...
    handleAnswerQuiz(state, action) {
        ...
        const choiceIndex = typeof payload.choiceIndex === 'number' && Number.isFinite(payload.choiceIndex) ? Math.trunc(payload.choiceIndex) : null;
        const choice = choiceIndex != null && Array.isArray(quiz.choices) && quiz.choices[choiceIndex] ? String(quiz.choices[choiceIndex]).trim() : answerRaw;
        const ok = quiz.answer != null ? normalize(choice) === normalize(quiz.answer) : false;
        let next = {
            ...state,
            pending: null
        };
        ...
        if (ok) {
            ...
            next = this.discardDrawnCard(next, 'legend', quiz.card, {
                keep: false
            });
        }
        meta = this.getMeta(next);
    ...
    handleChooseTarget(state, action) {
        ...
        if (last != null && last === targetId) {
            ...
            return {
                ...next,
                pending
            }; // on laisse le pending ouvert
        }
        if (kind === 'swap') {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} échange sa place avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetId)}.`);
            next = this.swapPositions(next, playerId, targetId);
            next = this.setLastTarget(next, playerId, targetId);
        }
        if (kind === 'skip1') {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetId)} perd son prochain tour.`);
            next = this.addSkip(next, targetId, 1);
            next = this.setLastTarget(next, playerId, targetId);
        }
        if (kind === 'swap_card') {
            const count = Math.max(1, Math.trunc(Number(pending?.data?.count ?? 1)));
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} échange ${count} carte(s) avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetId)}.`);
    ...
    applyLanding(state, playerId, context) {
        ...
        if (!tile) return next;
        const label = tile.label?.trim() ? tile.label.trim() : tile.title?.trim() ? tile.title.trim() : `Case ${pos + 1}`;
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} place ${this.pawnLabel(next, playerId)} en case ${pos + 1} (${label}).`);
    ...
    applyCard(state, playerId, deckType, card) {
        ...
        if (deckType === 'legend') {
            ...
            next = this.incrementCollection(next, playerId, 'legend');
            next = this.discardDrawnCard(next, 'legend', card, {
                keep: true
            });
        ...
        if (deckType === 'treasure') {
            ...
            next = this.discardDrawnCard(next, 'treasure', card, {
                keep: true
            });
            return this.applyGenericEffect(next, playerId, card.effect);
        }
        if (deckType === 'landscape') {
            ...
            next = this.discardDrawnCard(next, 'landscape', card, {
                keep
            });
            return this.applyGenericEffect(next, playerId, card.effect);
        }
        if (deckType === 'farce') {
            ...
            next = this.discardDrawnCard(next, 'farce', card, {
                keep
            });
            return this.applyGenericEffect(next, playerId, card.effect);
        }
    ...
    applyGenericEffect(state, playerId, textRaw) {
        ...
        if (/choisissez\s+un\s+joueur/i.test(text) && /perd\s+son\s+prochain\s+tour/i.test(text)) {
            const otherPlayers = this.otherPlayers(next, playerId);
        ...
        if (/tirez\s+au\s+hasard\s+une\s+carte/i.test(text) && /vous\s+la\s+perdez/i.test(text)) {
        ...
        if (/échange/i.test(text) && /carte/i.test(text)) {
            ...
            if (/second\s+joueur/i.test(text)) {
                const players = Array.isArray(next.players) ? next.players : [];
                const ids = players.map((p)=>p?.id).filter((id)=>Number.isFinite(id));
                const targetId = ids.length >= 2 ? ids[1] === playerId ? ids[0] : ids[1] : null;
                if (targetId != null) {
                    next = this.core.appendLog(next, `Échange automatique avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetId)}.`);
                    next = this.exchangeRandomCards(next, playerId, targetId, count);
        ...
        if (/échange/i.test(text) && /position/i.test(text)) {
            if (/dernier\s+joueur/i.test(text)) {
                ...
                if (other.length) {
                    const meta = this.getMeta(next);
                    const last = other.map((p)=>p.id).sort((a, b)=>(meta.positions?.[a] ?? 0) - (meta.positions?.[b] ?? 0))[0];
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} échange sa place avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, last)}.`);
    ...
    parseQuizCard(playerId, card) {
        ...
        const choiceLines = effect.split(/\s*(?=[*]?[ABC]\))/i).map((s)=>s.trim()).filter((s)=>/^[*]?[ABC]\)/i.test(s));
    ...
    drawCard(meta, deckType) {
        const deckId = deckType === 'legend' ? 'legend' : deckType === 'farce' ? 'farce' : deckType === 'treasure' ? 'treasure' : 'landscape';
        const decks = asRecord(meta.decks);
        const rawDeck = asRecord(decks[deckId]);
        const deck = {
            cards: Array.isArray(rawDeck.cards) ? rawDeck.cards : [],
            discard: Array.isArray(rawDeck.discard) ? rawDeck.discard : []
        };
    ...
    finishByScore(state) {
        ...
        if (winnerId != null) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, winnerId)} remporte la partie !`);
        }
        return next;
    }
...
VoyageActionService = _ts_decorate([
