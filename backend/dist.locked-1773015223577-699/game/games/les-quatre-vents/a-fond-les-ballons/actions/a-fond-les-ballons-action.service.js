...
Object.defineProperty(exports, "AFondLesBallonsActionService", {
    enumerable: true,
    get: function() {
        return AFondLesBallonsActionService;
    }
});
...
let AFondLesBallonsActionService = class AFondLesBallonsActionService {
    ...
    handleRoll(state) {
        ...
        if (afterMeta.winnerId != null) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, afterMeta.winnerId)} remporte la partie !`);
            return {
                ...next,
                status: 'finished'
            };
    ...
    handleSwapChooseTarget(state, action) {
        const status = String(state.status ?? '').toLowerCase();
        if (status !== 'started') return state;
        ...
        const targetPlayerId = typeof payload.targetPlayerId === 'number' ? payload.targetPlayerId : Number(payload.targetPlayerId);
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} échange sa place avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId)}.`);
        next = this.decrementTrapImmunity(next, currentId);
        return this.advanceTurnWithSkipLogs(next);
    }
    handleSwapDecline(state) {
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} refuse l'échange de place.`);
        next = this.decrementTrapImmunity(next, currentId);
        return this.advanceTurnWithSkipLogs(next);
    }
    handleDraw(state) {
        ...
        if (kind === 'boutique') {
            ...
            const drawn = Array.isArray(data.drawn) ? data.drawn.map((entry)=>asLoufoqueCard(entry)).filter((entry)=>entry !== null) : [];
            next = {
                ...state,
                pending: null
            };
            ...
            next = this.core.appendLog(next, card ? `Boutique : carte ${drawIndex} : ${card.text}` : `Boutique : carte ${drawIndex} : aucune carte disponible.`);
    ...
    handleChoosePawn(state, action) {
        ...
        const pawns = Array.isArray(meta.pawns) && meta.pawns.length > 0 ? meta.pawns : options.map((p)=>({
                id: toText(p.id),
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} a choisi le pion : ${String(pawn.label ?? 'pion').trim()}.`);
        ...
        const starterId = typeof nextMeta.setupStarterId === 'number' ? nextMeta.setupStarterId : state.turn?.currentPlayerId ?? players[0]?.id ?? null;
    ...
    advanceTurnWithSkipLogs(state) {
        ...
        while(attempts < players.length * 2){
            ...
            if (remaining > 0) {
                ...
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, pid)} passe son tour.`);
                attempts += 1;
            } else {
                break;
            }
    ...
    appendTurnAnnouncement(state) {
        const currentId = state.turn?.currentPlayerId ?? null;
        if (currentId == null) return state;
        return this.core.appendLog(state, `C'est au tour de ${(0, _playernamehelper.resolvePlayerNameFromState)(state, currentId)}.`);
    }
    availablePawns(meta, pawnByPlayerId) {
    ...
    applyLanding(state, playerId, position, depth) {
        let next = state;
        let meta = this.getMeta(next);
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} place ${this.pawnLabel(next, playerId)} en case ${position + 1}. (${label}).`);
        if (!tile) return next;
        ...
        if (tile.type === 'glissade') {
            ...
            next = this.core.appendLog(next, `Glissade : ${delta > 0 ? 'avancez' : 'reculez'} de ${Math.abs(delta)} case(s).`);
            return this.moveBy(next, playerId, delta, depth);
        }
        if (tile.type === 'tornade') {
            return this.startSwapPending(next, playerId, 'Tornade : choisissez un joueur à échanger dans la liste, puis Entrée.');
        }
        if (tile.type === 'chaton') {
    ...
    drawAndApplyLoufoque(state, playerId, depth) {
        let next = state;
        let meta = this.getMeta(next);
        const draw = this.drawLoufoque(meta);
        meta = draw.meta;
        ...
        switch(card.id){
            ...
            case 4:
                ...
                for (const p of next.players ?? []){
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, p.id)} passera son prochain tour.`);
    ...
    applyBoutiqueWorstCard(state, playerId, depth) {
        if (state.pending) return state;
        return {
            ...state,
            pending: {
    ...
    startSwapPending(state, playerId, label) {
        const players = Array.isArray(state.players) ? state.players : [];
        const targets = players.filter((p)=>p?.id !== playerId).map((p)=>({
                targetPlayerId: p.id,
        ...
        const pending = {
            ...
            blocking: true,
            choices: [
                ...targets.map((t)=>t.targetUsername),
                'Ne pas échanger'
            ],
            data: {
                targets
    ...
    moveToNextType(state, playerId, type, depth) {
        ...
        if (idx < 0) {
            return this.core.appendLog(state, `Aucune case de type ${type} n'a été trouvée devant vous.`);
        }
        return this.applyLanding(state, playerId, idx, depth + 1);
    }
    skipTurns(state, playerId, turns) {
        const meta = this.getMeta(state);
        const statuses = meta.statuses;
    ...
    grantTrapImmunity(state, playerId, turns) {
        const meta = this.getMeta(state);
        const statuses = meta.statuses;
    ...
    computeTarget(current, delta, finalIndex) {
    ...
    pawnLabel(state, id) {
        ...
        const pawn = Array.isArray(meta?.pawns) ? meta.pawns.find((p)=>toText(p?.id) === pawnId) : null;
        const title = toText(pawn?.label);
        if (title) return `"${title}"`;
        return 'un pion';
    }
    compactTileLabel(rawLabel, position) {
    ...
    constructor(core, random, _turns, deckPolicies, setupFlow){
        this.core = core;
        this.random = random;
...
AFondLesBallonsActionService = _ts_decorate([
...
function normalizeAFondMeta(input) {
    ...
    return {
        ...
        pawnByPlayerId: asRecord(raw.pawnByPlayerId) ?? {},
        setupStarterId: typeof raw.setupStarterId === 'number' ? raw.setupStarterId : null,
        charactersByPlayerId: asRecord(raw.charactersByPlayerId) ?? {},
        statuses: {
            skipTurn: asRecord(asRecord(raw.statuses).skipTurn) ?? {},
            trapImmunityTurns: asRecord(asRecord(raw.statuses).trapImmunityTurns) ?? {}
        },
        decks: {
            loufoque: Array.isArray(asRecord(raw.decks).loufoque) ? asRecord(raw.decks).loufoque : [],
            discardLoufoque: Array.isArray(asRecord(raw.decks).discardLoufoque) ? asRecord(raw.decks).discardLoufoque : []
        },
...
function asPendingSwap(value) {
    ...
    const targets = Array.isArray(data.targets) ? data.targets.map((entry)=>{
    ...
    return {
        ...
        playerId,
        blocking: true,
        choices: (Array.isArray(record.choices) ? record.choices : []).map((entry)=>toText(entry)).filter((entry)=>entry.length > 0),
        data: {
            targets
        }
    };
}
function asLoufoqueCard(value) {
    if (!value || typeof value !== 'object') return null;
    const record = asRecord(value);
...
function defaultLoufoqueDeck() {
    return [
        ...
        {
            id: 8,
            text: "Le Grand Chaton éternue violemment. Reculez d'une case."
        },
        {
            id: 9,
        ...
        {
            id: 23,
            text: 'Vous trouvez un trampoline géant. Avancez de 4 cases.'
        },
        {
            id: 24,
