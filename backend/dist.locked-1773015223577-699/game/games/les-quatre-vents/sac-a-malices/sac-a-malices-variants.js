...
_export(exports, {
    ...
    get SAC_VARIANT_BY_ID () {
        return SAC_VARIANT_BY_ID;
    },
    get SAC_VARIANT_COUNT () {
        return SAC_VARIANT_COUNT;
    },
    get buildVariantChoiceLabel () {
        return buildVariantChoiceLabel;
    },
    get getVariantIndex () {
        return getVariantIndex;
    },
    get parseVariantInput () {
        return parseVariantInput;
    }
});
const baseRules = {
    startMoney: 2000,
...
const SAC_VARIANT_BY_ID = SAC_VARIANTS.reduce((acc, variant)=>{
...
const normalizeKey = (value)=>String(value ?? '').trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
const SAC_VARIANT_COUNT = SAC_VARIANTS.length;
