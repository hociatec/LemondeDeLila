...
function getAvailableActions(state, playerId) {
    ...
    if (setupStep === 'setup_config') {
        ...
        if (ownerId != null && ownerId === playerId) {
            return _sacamalicesvariants.SAC_VARIANTS.map((variant)=>({
    ...
    if (pending.type === 'choose_property') {
        ...
        const options = Array.isArray(pendingData.options) ? pendingData.options.map((item)=>{
            const option = asRecord(item);
            return {
                tileIndex: Number(option.tileIndex)
            };
        }) : [];
    ...
    if (inJail) {
        if (allowPayFine) actions.push({
            type: 'pay_fine',
            payload: {}
        });
        if (jailCardCount > 0) actions.push({
            type: 'use_jail_card',
            payload: {}
        });
    }
...
function validateAction(state, action, actorId) {
    ...
    if (normalized === 'sac_set_variant') {
        ...
        const setupStep = typeof meta.setupStep === 'string' ? meta.setupStep.trim() : '';
        if (setupStep !== 'setup_config') {
            throw new _gameerrors.PlayerActionError('Configuration indisponible.', {
                gameType: 'sac-a-malices'
            });
        }
        const ownerId = typeof meta.ownerPlayerId === 'number' ? meta.ownerPlayerId : null;
