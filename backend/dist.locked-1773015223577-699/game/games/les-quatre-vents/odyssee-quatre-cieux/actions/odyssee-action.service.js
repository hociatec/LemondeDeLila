...
Object.defineProperty(exports, "OdysseeActionService", {
    enumerable: true,
    get: function() {
        return OdysseeActionService;
    }
});
...
const ODYSSEE_DEFAULT_PAWN_NAMES = [
    'Aube',
    'Brise',
    'Comete',
    'Dune'
...
let OdysseeActionService = class OdysseeActionService {
    ...
    handleMovePawn(state, action) {
        ...
        if (!pending || pendingRow.type !== 'choose_pawn' || Number(pendingRow.playerId ?? null) !== currentId) return state;
        ...
        const moves = Array.isArray(pendingData.moves) ? pendingData.moves.map((entry)=>{
    ...
    resolvePawnName(state, playerId, pawnIndex) {
        ...
        const byIndex = typeof names[pawnIndex] === 'string' ? String(names[pawnIndex]).trim() : '';
...
OdysseeActionService = _ts_decorate([
