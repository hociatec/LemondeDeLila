...
let PanierExpressService = class PanierExpressService extends _abstractgameservice.AbstractGameService {
    ...
    exposeState(state) {
        ...
        const current = currentId == null ? null : (ensured.players ?? []).find((p)=>toPlayerIdValue(p?.id) === currentId) ?? null;
    ...
    exposeStateForUser(state, userId) {
        ...
        const viewerId = requestedViewerId != null && (ensured.players ?? []).some((p)=>toPlayerIdValue(p?.id) === requestedViewerId) ? requestedViewerId : null;
    ...
    assignBotPawns(state) {
        ...
        assignedBots.forEach((bot)=>{
            next = this.core.appendLog(next, `[Panier Express] ${this.utils.playerName(next, bot.id)} a choisi le pion: ${bot.pawn}.`);
        });
        return next;
    }
    ...
    ensureStarted(state) {
        ...
        if (!meta.pawnAnnouncementsDone) {
            readyPlayers.forEach((p)=>{
                const pawn = this.getPawnText(p);
                if (!pawn) return;
                withLogs = this.core.appendLog(withLogs, `[Panier Express] ${this.utils.playerName(withLogs, p.id)} a choisi le pion: ${pawn}.`);
            });
            metaAfterLogs = {
                ...metaAfterLogs,
    ...
    finalizeStarterAfterPawnSelection(state) {
        ...
        if (!metaNow.shoppingListAnnouncementsDone) {
            ...
            next = {
                ...
                metadata: {
                    ...metaNow,
                    shoppingListAnnouncementsDone: true
                }
            };
        }
        return next;
        ...
        switch(event){
            ...
            case 'journee-bio':
                {
                    ...
                    const targetNames = targets.map((target)=>this.utils.playerName(next, target.playerId)).filter((name)=>name.length > 0);
                    next = this.core.appendLog(next, targetNames.length ? `[Panier Express] Journée bio : bonus pour ${targetNames.join(', ')}.` : `[Panier Express] Journée bio : aucun joueur sur un stand Bio.`);
                    next = this.appendActionLog(next, playerId, 'event', {
                        event,
                        effect: 'multi_draw'
                    });
            ...
            default:
                if (event === 'erreur-de-livraison' || event === 'produit-avarie' || event === 'emballage-oublie') {
                    const discarded = discardRandomCourse(playerId);
                    next = this.core.appendLog(next, discarded ? `[Panier Express] ${this.utils.playerName(state, playerId)} défausse "${this.utils.formatCourseLabel(discarded)}".` : `[Panier Express] Aucune carte à défausser.`);
                    next = this.appendActionLog(next, playerId, 'event', {
                        event,
                        effect: 'discard_random',
                        discarded
    ...
    handleMerchantRequestAccept(state, action) {
        ...
        if (!inventory.includes(ingredient)) {
            return this.core.appendLog(state, `[Panier Express] Case Échange : ${this.utils.playerName(state, actorId)} n'a pas "${this.utils.formatCourseLabel(ingredient)}". Refusez la demande.`);
        }
        let next = {
            ...state,
            pending: null
    ...
    handleChoosePawn(state, action) {
        ...
        next = this.core.appendLog(next, `[Panier Express] ${this.utils.playerName(state, resolved.playerId)} a choisi le pion: ${chosen}.`);
    ...
    handlePickChoice(state, action) {
        ...
        const buildCourseSets = ()=>{
            ...
            const summerFruitStandIds = new Set([
                'fruitier',
                'bio-fruits',
                'fruits-exotiques',
                'fruits-rouges'
            ]);
            const winterVegStandIds = new Set([
                'primeur-hivernal'
            ]);
    ...
    handleAnswerQuiz(state, action) {
        ...
        next = this.core.appendLog(next, `[Panier Express] Réponse ${correct ? 'correcte' : 'incorrecte'} pour ${this.utils.playerName(state, playerId)}.`);
...
function toPlayerIdValue(value) {
