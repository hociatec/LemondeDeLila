...
Object.defineProperty(exports, "PanierExpressExchangeService", {
    enumerable: true,
    get: function() {
        return PanierExpressExchangeService;
    }
});
...
const _interactiveexchangemodel = require("../../../../modules/exchange/model/interactive-exchange.model");
...
let PanierExpressExchangeService = class PanierExpressExchangeService {
    ...
    acceptOffer(state, targetPlayerId) {
        ...
        (0, _playinglogger.playingLog)('panier.exchange.resolve', {
    ...
    requestExchange(state, playerId) {
        ...
        if ([
            ...
            const targets = (state.players ?? []).filter((p)=>p.id !== playerId).map((p)=>({
                    playerId: p.id,
                    username: p.username
                }));
        ...
        if (resolvedCard === 'troc-rapide') {
            ...
            return {
                ...
                pending: {
                    ...
                    choices: inv,
                    data: {
                        kind: 'exchange.troc_rapide.choose_give',
                        targetPlayerId
                    }
                }
            };
        ...
        if (resolvedCard === 'troc-fruit-legume') {
            ...
            return {
                ...
                pending: {
                    ...
                    choices,
                    data: {
                        kind: 'exchange.troc_fruit_legume.choose_target',
                        targets
                    }
                }
            };
        ...
        if (resolvedCard === 'marche-noir') {
            ...
            return {
                ...
                pending: {
                    ...
                    choices: cards,
                    data: {
                        kind: 'exchange.marche_noir.discard'
                    }
                }
            };
        }
        if (resolvedCard === 'echange-devant' || resolvedCard === 'echange-derriere') {
            ...
            const idx = players.findIndex((p)=>p.id === playerId);
            const targetPlayerId = Number(players[resolvedCard === 'echange-devant' ? (idx + 1) % players.length : (idx - 1 + players.length) % players.length]?.id);
            const me = (state.players ?? []).find((p)=>p.id === playerId);
            const target = (state.players ?? []).find((p)=>p.id === targetPlayerId);
            ...
            const positionLabel = resolvedCard === 'echange-devant' ? 'juste devant vous' : 'juste derrière vous';
        ...
        if (resolvedCard === 'defausse-aleatoire') {
            ...
            return this.core.appendLog(addToDiscardState({
                ...state,
                players,
                metadata: updatedMeta
            }, card), `[Panier Express] Défausse aléatoire : ${this.utils.playerName(state, playerId)} défausse "${cardLabel}".`);
        }
        const started = this.exchangeFlow.start({
            ...state,
            metadata
        }, playerId, resolvedCard, this.adapter());
        if (started.kind === 'started') {
            const pending = started.pending;
            const targetsCount = Array.isArray(pending?.targets) ? pending.targets.length : 0;
            (0, _playinglogger.playingLog)('panier.exchange.pending', {
                roomId: state.metadata?.roomId ?? null,
                gameType: state.metadata?.gameType ?? null,
                userId: playerId,
        ...
        const reason = started.kind === 'no_targets' ? `[Panier Express] Aucun joueur disponible pour un échange (${resolvedCard}).` : `[Panier Express] Pas d'échange possible (${resolvedCard}).`;
        return this.core.appendLog({
            ...state,
            metadata
        }, reason);
    ...
    adapter() {
        return {
            listTargets: _interactiveexchangemodel.defaultExchangeTargets,
...
PanierExpressExchangeService = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        ...
        typeof _interactiveexchangeservice.InteractiveExchangeService === "undefined" ? Object : _interactiveexchangeservice.InteractiveExchangeService,
...
function addCardToPlayerState(utils, state, playerId, card) {
    ...
    const meta = state.metadata ?? {};
    const currentDiscards = Array.isArray(meta?.discards?.courses) ? meta.discards.courses.map((v)=>String(v)) : [];
    return {
        ...state,
        players,
        metadata: {
...
function removeFromInventoryState(utils, state, playerId, card) {
    ...
    const players = (state.players ?? []).map((p)=>{
        ...
        return {
            ...p,
            inventory: removeOne(inv, trimmed)
        };
    });
    return {
        ...state,
