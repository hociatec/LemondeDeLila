...
describe('AventureSauvageActionService', ()=>{
    it('demande au joueur de choisir son pion via le pending label', async ()=>{
        const moduleRef = await _testing.Test.createTestingModule({
            providers: [
                ...
                _boardeffectspoliciesservice.BoardEffectsPoliciesService,
        ...
        const setup = moduleRef.get(_aventuresauvagesetupservice.AventureSauvageSetupService);
        const actions = moduleRef.get(_aventuresauvageactionservice.AventureSauvageActionService);
        const state = setup.hydrateInitialState(makeBaseState());
        const next = actions.applyActions(state, []);
        expect(String(next.pending?.label ?? '')).toContain("C'est à Lilas de choisir son pion.");
    });
    ...
    it('utilise le libelle possessif pour le pion et le log de de', async ()=>{
        ...
        expect(messages.some((m)=>/Lilas lance le d(?:e|é|é)\s*:\s*"\d"\./.test(m))).toBe(true);
    });
    it('melange les decks au demarrage', async ()=>{
        ...
        const animalIds = Array.isArray(animalDeck) ? animalDeck.map((card)=>{
