...
Object.defineProperty(exports, "ContesPresenterService", {
    enumerable: true,
    get: function() {
        return ContesPresenterService;
    }
});
...
let ContesPresenterService = class ContesPresenterService {
    exposeStateForUser(state, userId) {
        ...
        const scoreLines = players.map((p)=>{
            ...
            const name = p?.username && String(p.username).trim() ? String(p.username).trim() : `Joueur ${pid}`;
        ...
        return {
            ...state,
            catalog: {
                phases: _gamedefinition.CONTES_CACAHUETES_GAME.phaseOrder.map((p)=>p.id),
                victory: null
            },
...
ContesPresenterService = _ts_decorate([
