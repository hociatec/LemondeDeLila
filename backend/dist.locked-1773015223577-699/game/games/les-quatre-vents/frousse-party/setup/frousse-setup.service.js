...
Object.defineProperty(exports, "FrousseSetupService", {
    enumerable: true,
    get: function() {
        return FrousseSetupService;
    }
});
...
let FrousseSetupService = class FrousseSetupService {
    hydrateInitialState(base) {
        ...
        const meta = {
            ...
            pawns: (0, _pawncataloghelper.loadCanonicalPawns)(Array.isArray(pawns.pawns) ? pawns.pawns : []).map((pawn)=>({
                    id: pawn.id,
                    name: pawn.name,
        ...
        const pendingInfo = this.setupFlow.createSequentialPawnPending({
            ...
            pawns: (Array.isArray(meta.pawns) ? meta.pawns : []).map((p)=>({
                    id: toText(p?.id),
...
FrousseSetupService = _ts_decorate([
