...
Object.defineProperty(exports, "FrousseActionService", {
    enumerable: true,
    get: function() {
        return FrousseActionService;
    }
});
...
let FrousseActionService = class FrousseActionService {
    ...
    handleChoosePawn(state, action) {
        ...
        const withLog = this.core.appendLog(next, `[Frousse Party] ${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} a choisi le pion: ${label}.`);
    ...
    ensurePawnSelection(state) {
        ...
        const usedForPending = new Set(players.map((p)=>(0, _pawnsutils.resolvePawnId)(p?.pawn)).filter((id)=>Boolean(id)));
        const choicesForPending = (Array.isArray(metaForPending.pawns) ? metaForPending.pawns : []).map((p)=>({
                id: toText(p?.id),
        ...
        const pendingInfo = this.setupFlow.createSequentialPawnPending({
            ...
            isAssigned: (candidateId)=>{
                ...
                return Boolean((0, _pawnsutils.resolvePawnId)(player?.pawn));
            },
    ...
    handleRoll(state) {
        ...
        if (skipNow > 0) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} passe son tour${suffix}.`);
        ...
        if (blocked) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} tente de se libérer : dé = "${rollLabel}".`);
            const ok = blocked.kind === 'need_roll_one_of' ? blocked.allowed.includes(roll.value) : blocked.kind === 'need_roll_min' ? roll.value >= blocked.min : blocked.kind === 'need_roll_even' ? roll.value % 2 === 0 : false;
            if (!ok) {
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} reste bloqué.`);
                return this.advanceTurnWithAnnouncement(next);
            }
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} se libère !`);
            return this.advanceTurnWithAnnouncement(next);
        }
        const roll = this.roll(meta, currentId);
        meta = roll.meta;
        ...
        if (roll.rolls && roll.rolls.length >= 2) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} lance deux dés : "${roll.rolls[0]}" et "${roll.rolls[1]}" (garde "${roll.value}").`);
        } else {
            const rollLabel = this.formatRollLabel(roll);
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} lance le dé : "${rollLabel}".`);
        }
    ...
    handleChooseTarget(state, action) {
        ...
        if (!ctx || ctx.kind !== 'swap' || ctx.actorId !== currentId) return {
            ...state,
            pending: null
        };
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} échange sa position avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId)}.`);
        return this.advanceTurnWithAnnouncement(next);
    }
    handleSwapDecline(state) {
        if (String(state.status ?? '').toLowerCase() !== 'started') return state;
        const pending = asPendingRecord(state.pending);
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} refuse l'échange de position.`);
        return this.advanceTurnWithAnnouncement(next);
    }
    handleDraw(state) {
        if (String(state.status ?? '').toLowerCase() !== 'started') return state;
    ...
    applyLanding(state, playerId) {
        ...
        if (tile) {
            ...
            const placement = this.boardEffects.createPlacementLog({
                playerLabel: (0, _playernamehelper.resolvePlayerNameFromState)(next, playerId),
        ...
        if (pos >= 49) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} s'échappe du manoir !`);
            return {
                ...next,
                metadata: {
                    ...next.metadata ?? {},
    ...
    applyDrawCard(state, playerId) {
        ...
        if (/Fantôme/i.test(draw.card.category) && meta.statuses.ignoreNextGhost?.[playerId]) {
            meta = {
                ...meta,
                statuses: {
                    ...meta.statuses,
        ...
        if (/Farce/i.test(draw.card.category) && meta.statuses.ignoreNextPrank?.[playerId]) {
            meta = {
                ...meta,
                statuses: {
                    ...meta.statuses,
        ...
        if (/Piège/i.test(draw.card.category) && meta.statuses.ignoreNextTrap?.[playerId]) {
            meta = {
                ...meta,
                statuses: {
                    ...meta.statuses,
        ...
        const withEffect = formatCardDrawLog((0, _playernamehelper.resolvePlayerNameFromState)(next, playerId), cardText, effectLabel);
    ...
    applyCard(state, playerId, card) {
        ...
        if (/Bonus/i.test(card.category) && card.localNumber === 13) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} saute 6 cases.`);
            next = this.move(next, playerId, 6);
            return this.applyLanding(next, playerId);
        }
        // Ghost random swap (admin: random target).
        if (/Fantôme/i.test(card.category) && /fantôme farceur/i.test(text) && /échang|échange/i.test(text)) {
            ...
            }, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} échange sa position avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, target.id)}.`);
            return next;
        }
        ...
        if (/Ignorez le prochain piège/i.test(text) || /Ignorez les pièges/i.test(text)) {
            meta = {
                ...meta,
                statuses: {
                    ...meta.statuses,
        ...
        if (/gardez le plus petit/i.test(text) || /gardez le chiffre le plus bas/i.test(text)) {
            meta = {
                ...meta,
                statuses: {
                    ...meta.statuses,
        ...
        // Teleport to case 40.
        if (/jusqu'a la case 40/i.test(text) || /jusqu['’]a la case 40/i.test(text)) {
            next = this.setPos(next, playerId, 39);
            return this.applyLanding(next, playerId);
        }
        // Relance immédiate.
        if (/Relancez le dé/i.test(text) || /Relancez/i.test(text) && /dé/i.test(text)) {
            meta.keepTurnNow = true;
            return {
                ...next,
                metadata: {
        ...
        // Go to case 1.
        if (/case départ/i.test(text) || /Retour a la case une/i.test(text) || /Retournez/i.test(text) && /case une/i.test(text)) {
    ...
    otherPlayers(state, me) {
        ...
        return players.filter((p)=>p?.id != null && p.id !== me).map((p)=>({
                id: p.id,
                username: (0, _playernamehelper.resolvePlayerNameFromState)(state, p.id)
            }));
    }
    otherPlayerIds(meta, me) {
        return Object.keys(meta.positions ?? {}).map(Number).filter((id)=>Number.isFinite(id) && id !== me);
    }
    getMeta(state) {
        return normalizeFrousseMeta(state.metadata);
    }
    clearKeepTurnNow(state) {
    ...
    pawnLabel(state, id) {
        ...
        const meta = this.getMeta(state);
        const fromMeta = Array.isArray(meta?.pawns) ? meta.pawns.find((p)=>toText(p?.id) === pawnId) : null;
    ...
    pawnPossessiveLabel(state, id) {
        ...
        const stripped = inner.replace(/^(un|une|le|la|les)\s+/i, '').replace(/^l['’]\s*/i, '').trim();
    ...
    finalizeStarterAfterPawnSelection(state) {
        ...
        const everyoneHasPawn = players.every((p)=>(0, _pawnsutils.resolvePawnId)(p?.pawn));
        ...
        if (typeof starter?.id === 'number') {
            next = this.core.appendLog(next, `[Frousse Party] Début de partie : ${(0, _playernamehelper.resolvePlayerNameFromState)(next, starter.id)} commence.`);
            next = this.core.appendLog(next, `C'est au tour de ${(0, _playernamehelper.resolvePlayerNameFromState)(next, starter.id)}.`);
        }
        return next;
    }
...
FrousseActionService = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        ...
        typeof _boardeffectspoliciesservice.BoardEffectsPoliciesService === "undefined" ? Object : _boardeffectspoliciesservice.BoardEffectsPoliciesService,
...
function normalizeFrousseMeta(input) {
    ...
    return {
        ...
        decks: {
            ...
            discard: Array.isArray(decks.discard) ? decks.discard : []
        },
        pawns: Array.isArray(raw.pawns) ? raw.pawns : [],
        pendingContext: asRecord(raw.pendingContext) ?? null,
...
function extractMoveDelta(text) {
    ...
    let total = 0;
    const forwardOrBackPattern = /(avancez|reculez)\s+(?:de|d['’])\s*(\d+|un|une|deux|trois|quatre|cinq|six)(?:\s+cases?)?/gi;
    let fbMatch;
    ...
    const narrativeForward = text.match(/avancez[\s\S]*?d['’]\s*(\d+|un|une|deux|trois|quatre|cinq|six)\s+case/i);
    if (narrativeForward) return parseNumberish(narrativeForward[1]);
    const narrativeBack = text.match(/recul(?:ez|ant|e|es)?[\s\S]*?d['’]\s*(\d+|un|une|deux|trois|quatre|cinq|six)\s+case/i);
    ...
    const forwardAposWords = text.match(/Avancez\s+d['’]\s*(un|une|deux|trois|quatre|cinq|six)\s+case/i);
    ...
    const backAposWords = text.match(/Reculez\s+d['’]\s*(un|une|deux|trois|quatre|cinq|six)\s+case/i);
...
function describeCardEffect(card) {
    ...
    if (/Ignorez le prochain piège/i.test(text)) return 'Ignorez le prochain piège.';
    if (/Ignorez les pièges jusqu['’]au prochain symbole/i.test(text)) return 'Ignorez les pièges jusqu’au prochain symbole.';
    if (/Ignorez la prochaine carte Fantôme/i.test(text)) return 'Ignorez la prochaine carte Fantôme.';
    if (/annule une farce/i.test(text) || /rien ne vous arrive/i.test(text)) return 'Ignorez la prochaine farce.';
    if (/Sautez\s+6\s+cases/i.test(text) || /Bonus/i.test(card.category) && card.localNumber === 13) return 'Sautez 6 cases.';
    if (/Doublez votre prochain lancer/i.test(text)) return 'Doublez le prochain lancer de dé.';
    ...
    if (/jusqu['’]a la case 40/i.test(text)) return 'Allez directement a la case 40.';
    ...
    if (/case depart/i.test(text) || /Retour a la case une/i.test(text)) return 'Retournez a la case depart.';
