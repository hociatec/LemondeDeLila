...
Object.defineProperty(exports, "PiratesEnVadrouillePresenterService", {
    enumerable: true,
    get: function() {
        return PiratesEnVadrouillePresenterService;
    }
});
...
let PiratesEnVadrouillePresenterService = class PiratesEnVadrouillePresenterService {
    exposeStateForUser(state, userId) {
        ...
        const scoreLines = players.map((p)=>{
            ...
            const treasures = Array.isArray(collection?.treasures) ? collection.treasures.length : 0;
            return `${name} : ${treasures} trésor${treasures > 1 ? 's' : ''}`;
        });
        return {
            ...state,
            catalog: {
                phases: _piratesenvadrouilledefinition.PIRATES_GAME.phaseOrder.map((p)=>p.id),
                victory: null
            },
...
PiratesEnVadrouillePresenterService = _ts_decorate([
