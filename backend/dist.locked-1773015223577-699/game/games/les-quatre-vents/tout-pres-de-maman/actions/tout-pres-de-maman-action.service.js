...
Object.defineProperty(exports, "ToutPresDeMamanActionService", {
    enumerable: true,
    get: function() {
        return ToutPresDeMamanActionService;
    }
});
...
let ToutPresDeMamanActionService = class ToutPresDeMamanActionService {
    ...
    handleRoll(state) {
        ...
        if (skip > 0) {
            ...
            return this.turns.advanceTurn(this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} reste sur place (tour sauté).`));
        }
        let nextMeta = {
            ...meta
        };
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} avance de ${total} case(s) et place ${this.pawnLabel(next, playerId)} en case ${target + 1} (${tile?.title ?? `case ${target + 1}`}).`);
    ...
    handleMeeting(state, playerId, depth) {
        ...
        const others = (state.players ?? []).filter((p)=>p?.id != null && p.id !== playerId).filter((p)=>(meta.positions?.[p.id ?? 0] ?? -1) === pos);
        let next = state;
        if (others.length) {
            next = this.core.appendLog(next, `Rencontre : ${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} avance avec ses amis.`);
    ...
    handleFinish(state, playerId, index, depth) {
        ...
        if (tokens >= ToutPresDeMamanActionService.TOKENS_TO_WIN) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} retrouve maman avec ${tokens} jetons eucalyptus !`);
            return next;
        }
        ...
        const next = this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerNameFromState)(state, playerId)} manque de jetons et recule de ${rewind} case(s) pour en retrouver.`);
    ...
    gainTokens(state, playerId, amount) {
        ...
        const next = this.replaceMeta(state, updatedMeta);
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} gagne ${amount} jeton(s) eucalyptus.`);
    }
    loseToken(state, playerId) {
        const meta = this.getMeta(state);
        const current = meta.tokens?.[playerId] ?? 0;
        if (current <= 0) {
            return this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerNameFromState)(state, playerId)} n’a pas de jeton à perdre.`);
        }
        const updatedMeta = {
            ...meta,
            tokens: {
        ...
        const next = this.replaceMeta(state, updatedMeta);
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} perd un jeton eucalyptus.`);
    }
    addSkip(state, playerId, amount, tileOrCard) {
        ...
        const next = this.replaceMeta(state, updatedMeta);
        const label = 'text' in tileOrCard ? `carte ${tileOrCard.id}` : tileOrCard?.title ?? 'effet spécial';
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} perd ${amount} tour(s) (${label}).`);
    }
    setBonusReroll(state, playerId) {
        ...
        const next = this.replaceMeta(state, updatedMeta);
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} pourra relancer le dé au prochain tour.`);
    }
    ...
    transferToken(state, playerId) {
        ...
        const next = this.replaceMeta(state, updatedMeta);
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} partage un jeton avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetId)}.`);
    }
    ...
    rollAndAdvance(state, playerId, depth) {
        ...
        let next = this.replaceMeta(state, nextMeta);
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} relance le dé et avance de ${roll.roll}.`);
        return this.moveAndApply(next, playerId, roll.roll, depth);
    }
    rollAndMaybeAdvance(state, playerId, depth) {
        const meta = this.getMeta(state);
        const roll = this.random.rollDice(meta, 6);
        let next = this.replaceMeta(state, {
            ...meta,
            ...roll.meta
        });
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} fait ${roll.roll} au dé.`);
        if (roll.roll >= 4) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} avance d’une case grâce à la réussite.`);
    ...
    pickOtherPlayer(state, playerId) {
        const candidates = Array.isArray(state.players) ? state.players.filter((p)=>p?.id && p.id !== playerId) : [];
        return candidates.length ? candidates[0].id ?? null : null;
    }
    pawnLabel(state, playerId) {
        const players = Array.isArray(state.players) ? state.players : [];
        const player = players.find((p)=>p?.id === playerId);
        const playerRecord = player && typeof player === 'object' ? player : {};
        const explicitLabel = typeof playerRecord.pawnLabel === 'string' ? playerRecord.pawnLabel.trim() : '';
        if (explicitLabel) return `"${explicitLabel}"`;
        const pawnId = typeof playerRecord.pawn === 'string' ? playerRecord.pawn.trim() : '';
        if (pawnId) return `"${pawnId}"`;
        const fallback = (0, _playernamehelper.resolvePlayerNameFromState)(state, playerId);
        return `"${fallback}"`;
    }
...
ToutPresDeMamanActionService.TOKENS_TO_WIN = 3;
ToutPresDeMamanActionService.MAX_DEPTH = 12;
ToutPresDeMamanActionService = _ts_decorate([
