...
describe('CaActionService', ()=>{
    function makeService(rolls = []) {
        ...
        return {
            service: new _caactionsservice.CaActionService(random, turns, core, deckPolicies),
            random
        };
    }
    ...
    it('handles neutral vs card landing and draw flow', ()=>{
        ...
        const neutral = service.applyActions(state, [
            {
                type: 'roll',
                payload: {}
            }
        ...
        const cardLanding = service.applyActions(state, [
            {
                type: 'roll',
                payload: {}
            }
        ...
        const afterDraw = service.applyActions(cardLanding, [
            {
                type: 'draw',
                payload: {}
            }
    ...
    it('handles mirror/double/next-delta flags on roll', ()=>{
        ...
        state = {
            ...
            metadata: {
                ...
                statuses: {
                    ...
                    doubleNextRoll: {
                        ...meta(state).statuses.doubleNextRoll,
                        1: true
                    },
                    nextPlayerDelta: 1
                }
        ...
        const out = service.handleRoll(state);
    ...
    it('resolves choose_target for swap and mirror contexts', ()=>{
        ...
        state = {
            ...
            metadata: {
                ...
                positions: {
                    ...meta(state).positions,
                    1: 3,
                    2: 6
                }
            }
        };
        let out = service.handleChooseTarget(state, {
            type: 'choose_target',
            payload: {
                targetPlayerId: 2
            }
        ...
        out = service.handleChooseTarget(state, {
            type: 'choose_target',
            payload: {
                targetPlayerId: 2
            }
    ...
    it('resolves choose_next_player and choose_next_delta', ()=>{
        ...
        const picked = service.handleChooseNextPlayer(state, {
            type: 'choose_next_player',
            payload: {
                playerId: 3
            }
        ...
        const delta = service.handleChooseNextDelta(state, {
            type: 'choose_next_delta',
            payload: {
                delta: -1
            }
    ...
    it('covers conditional card variants 51..60', ()=>{
        ...
        for(let id = 51; id <= 60; id += 1){
            const out = service.applyConditional({
                ...base,
                metadata: {
                    ...meta(base),
                    positions: {
    ...
    it('covers special after-move cards 33..37', ()=>{
        ...
        for(let id = 33; id <= 37; id += 1){
            const out = service.applySpecialAfterMove({
                ...base,
                metadata: {
                    ...meta(base),
                    positions: {
    ...
    it('covers rule card variants 61..70 including pending branches', ()=>{
        ...
        for(let id = 61; id <= 70; id += 1){
            const out = service.applyCardEffects({
                ...base,
                metadata: {
                    ...meta(base),
                    positions: {
            ...
            if (out.pending?.type === 'choose_next_player') {
                const resolved = service.handleChooseNextPlayer(out, {
                    type: 'choose_next_player',
                    payload: {
                        playerId: 2
                    }
            ...
            if (out.pending?.type === 'choose_next_delta') {
                const resolved = service.handleChooseNextDelta(out, {
                    type: 'choose_next_delta',
                    payload: {
                        delta: 1
                    }
            ...
            if (out.pending?.type === 'choose_target') {
                const resolved = service.handleChooseTarget(out, {
                    type: 'choose_target',
                    payload: {
                        targetPlayerId: 2
                    }
    ...
    it('covers move/skip/swap branches and drawCard fallback paths', ()=>{
        const { service } = makeService();
        const base = makeStartedState();
        const moved = service.applyCardEffects(base, 1, card(10, 'move', {
            moveDelta: 2
        }));
        expect(moved).toBeDefined();
        const skipped = service.applyCardEffects(base, 1, card(11, 'skip'));
        expect(skipped).toBeDefined();
        const swapped = service.applyCardEffects(base, 1, card(12, 'swap', {
            moveDelta: 1
        }));
        expect(swapped).toBeDefined();
        const emptyDraw = service.drawCard({
            ...meta(base),
            decks: {
                cards: [],
                discard: []
