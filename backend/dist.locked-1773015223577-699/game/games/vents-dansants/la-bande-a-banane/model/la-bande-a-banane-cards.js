...
_export(exports, {
    get BANDE_A_BANANE_CARD_BY_ID () {
        return BANDE_A_BANANE_CARD_BY_ID;
    },
    get BANDE_A_BANANE_DECK () {
        return BANDE_A_BANANE_DECK;
    }
});
const createCopies = (prefix, count, details)=>Array.from({
        length: count
    }, (_, index)=>({
            id: `${prefix}-${index + 1}`,
            ...details
...
const BANDE_A_BANANE_DECK = deck;
const BANDE_A_BANANE_CARD_BY_ID = Object.fromEntries(deck.map((card)=>[
        card.id,
        card
    ]));
