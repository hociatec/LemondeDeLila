...
Object.defineProperty(exports, "ZigEtZagActionService", {
    enumerable: true,
    get: function() {
        return ZigEtZagActionService;
    }
});
...
let ZigEtZagActionService = class ZigEtZagActionService {
    ...
    handleSelectCard(state, action) {
        ...
        if (!(0, _roundstatehelper.playerHasCard)(meta, actorId, cardId)) {
            return state;
        }
        if (!(0, _roundstatehelper.isCardAllowed)(round, actorId, cardId)) {
            return state;
        }
    ...
    handleDrawCard(state, action) {
        ...
        const hand = (0, _roundstatehelper.getPlayerHand)(meta, actorId);
        if (!hand.length) {
            return state;
        }
        ...
        const withDrawLog = this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerName)(players, actorId)} pioche.`);
    ...
    promoteToBattleFaceUp(state, players, round) {
        ...
        round.tiedPlayers.forEach((playerId)=>{
            if ((0, _roundstatehelper.getPlayerHandSize)(meta, playerId) <= 0) {
                const entry = plays.find((play)=>play.playerId === playerId);
                if (entry) {
                    entry.lostByNoCard = true;
                }
                const message = `${(0, _playernamehelper.resolvePlayerName)(players, playerId)} n'a plus de cartes.`;
        ...
        if (!waitingPlayers.length) {
            return this.finishRound(nextState, players, {
                ...round,
                plays
            }, null);
        }
    ...
    resolveBattle(state, players, round) {
        ...
        const meta = this.getMeta(state);
        const faceUpPlays = round.plays.filter((play)=>round.tiedPlayers.includes(play.playerId) && !play.lostByNoCard && play.faceUpCard && !play.invalidJoker);
        const results = faceUpPlays.map((play)=>{
            const def = _zigetzagcards.ZIG_ET_ZAG_CARD_BY_ID[play.faceUpCard];
            return def ? {
        ...
        const winners = results.filter((entry)=>entry.value === maxValue).map((entry)=>entry.playerId);
        if (winners.length === 1) {
        ...
        const waitingPlayers = winners.filter((playerId)=>(0, _roundstatehelper.getPlayerHandSize)(meta, playerId) > 0);
        if (!waitingPlayers.length) {
    ...
    finishRound(state, players, round, winnerId) {
        ...
        if (finalWinner != null) {
            nextState = this.core.appendLog({
                ...nextState,
                status: 'finished'
            }, `${(0, _playernamehelper.resolvePlayerName)(players, finalWinner)} remporte Zig et Zag !`);
            nextState = {
                ...nextState,
                metadata: {
                    ...nextState.metadata ?? {},
    ...
    logRound(state, summary, players) {
        ...
        if (summary.winnerId != null && summary.cardsWon > 0) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(players, summary.winnerId)} remporte ${summary.cardsWon} cartes.`);
        }
        return next;
    }
    appendRevealLogs(state, players, round, onlyPlayers) {
        let next = state;
        const filter = Array.isArray(onlyPlayers) && onlyPlayers.length ? new Set(onlyPlayers) : null;
        for (const play of round.plays ?? []){
            if (filter && !filter.has(play.playerId)) continue;
            if (round.stage === 'selection' && play.lostByNoCard) {
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(players, play.playerId)} n'a plus de cartes.`);
                continue;
            }
            if (play.faceUpCard) {
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(players, play.playerId)} dévoile ${this.formatCardLabel(play.faceUpCard)}.`);
            }
        }
        return next;
    }
    appendCollectiveRevealLog(state, players, round, onlyPlayers) {
        const filter = Array.isArray(onlyPlayers) && onlyPlayers.length ? new Set(onlyPlayers) : null;
        const revealPlayers = (round.plays ?? []).filter((play)=>{
            if (filter && !filter.has(play.playerId)) return false;
            return Boolean(play.faceUpCard);
        }).map((play)=>(0, _playernamehelper.resolvePlayerName)(players, play.playerId));
        if (revealPlayers.length <= 1) {
            return state;
        }
        const summary = revealPlayers.length === 2 ? `${revealPlayers[0]} et ${revealPlayers[1]} dévoilent leurs cartes.` : `${revealPlayers.slice(0, -1).join(', ')} et ${revealPlayers[revealPlayers.length - 1]} dévoilent leurs cartes.`;
    ...
    setCurrentPlayerWithAnnouncement(state, players, playerId, force = false) {
        ...
        if (!force && typeof previousPlayerId === 'number' && previousPlayerId === playerId) {
            return next;
        }
        return this.core.appendLog(next, `C'est au tour de ${(0, _playernamehelper.resolvePlayerName)(players, playerId)}.`);
    }
    ...
    ensureRoundState(state, players) {
        ...
        const round = (0, _roundstatehelper.buildInitialRoundState)(meta, safePlayers);
    ...
    normalizeRoundState(round) {
        ...
        const normalizeIdList = (list)=>{
            ...
            return arr.map((v)=>asNumberOrNull(v)).filter((v)=>typeof v === 'number');
        };
    ...
    recordPlayedCard(round, playerId, cardId) {
        const plays = round.plays.map((play)=>play.playerId === playerId ? {
                ...play
            } : play);
        const entry = plays.find((play)=>play.playerId === playerId);
        if (!entry) {
        ...
        if (round.stage === 'battle_face_down') {
            ...
            entry.invalidJoker = round.stage !== 'selection' && !(0, _roundstatehelper.isCardAllowed)(round, playerId, cardId);
        }
        return {
            ...round,
            plays
    ...
    playCardWithId(state, players, meta, round, playerId, cardId) {
        const { metadata: drainedMeta, removed } = (0, _roundstatehelper.removeCardFromHand)(meta, playerId, cardId);
        if (!removed) {
            return state;
        }
    ...
    normalizeWaitingPlayers(round) {
        return (round.waitingPlayers ?? []).map((value)=>{
            if (typeof value === 'number' && Number.isFinite(value)) return value;
            if (typeof value === 'string') {
                const n = Number(value.trim());
                return Number.isFinite(n) ? n : null;
    ...
    evaluateFaceUpPlays(round) {
        const faceUpResults = round.plays.filter((play)=>play.faceUpCard && !play.lostByNoCard).map((play)=>{
            const def = _zigetzagcards.ZIG_ET_ZAG_CARD_BY_ID[play.faceUpCard];
            return def ? {
                playerId: play.playerId,
                value: def.value
        ...
        const winners = faceUpResults.filter((entry)=>entry.value === maxValue).map((entry)=>entry.playerId);
        return {
    ...
    addCardsToWinner(metadata, winnerId, cards) {
        ...
        const hand = Array.isArray(decks[winnerId]) ? [
            ...decks[winnerId]
        ] : [];
        decks[winnerId] = [
            ...hand,
    ...
    applyWinnerCaptureBonus(metadata, players, round, winnerId) {
        ...
        const playerIds = (Array.isArray(players) ? players : []).map((player)=>player?.id).filter((id)=>typeof id === 'number');
        ...
        const winnerPlayCount = round.plays.find((play)=>play.playerId === winnerId)?.playedCards?.length ?? 0;
        ...
        const loserDeck = Array.isArray(decks[loserId]) ? [
            ...decks[loserId]
        ] : [];
        if (!loserDeck.length) {
            return metadata;
    ...
    detectWinner(metadata, players) {
        ...
        const ownerOfAll = playerIds.find((playerId)=>(metadata.playerDecks[playerId]?.length ?? 0) === _zigetzagcards.ZIG_ET_ZAG_TOTAL_CARDS);
        return ownerOfAll ?? null;
    }
    formatCardLabel(cardId) {
        return _zigetzagcards.ZIG_ET_ZAG_CARD_BY_ID[cardId]?.name ?? cardId;
    }
...
ZigEtZagActionService = _ts_decorate([
