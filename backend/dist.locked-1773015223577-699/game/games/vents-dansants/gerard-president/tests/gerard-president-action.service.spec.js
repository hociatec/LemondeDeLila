...
function withCurrentPlayer(turn, currentPlayerId) {
    return {
        currentPlayerId,
        direction: turn?.direction ?? 1,
        skippedPlayerIds: turn?.skippedPlayerIds,
        label: turn?.label
    };
}
describe('GerardPresidentActionService', ()=>{
    it('handles theme -> names -> winner flow', ()=>{
        ...
        state = {
            ...state,
            turn: withCurrentPlayer(state.turn, current),
            metadata: {
                ...getMeta(state),
        ...
        state = {
            ...state,
            turn: withCurrentPlayer(state.turn, 1),
            metadata: {
                ...getMeta(state),
    ...
    it('handles pass branch during collecting phase', ()=>{
        ...
        const state = {
            ...initial,
            turn: withCurrentPlayer(initial.turn, current),
            metadata: {
                ...getMeta(initial),
                roundPhase: 'collecting_names',
                pendingPlayers: [
