...
let CatPattesActionService = class CatPattesActionService {
    ...
    handleSetConfig(state, action) {
        ...
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} fixe la partie à ${roundsToPlay} manche(s), objectif ${_catpattesstateentity.CAT_PATTES_GOAL} pattes par manche.`);
    }
    playPattes(state, playerId, card) {
        ...
        if (nextPosition === goalPattes) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} atteint ${goalPattes} pattes et remporte la manche.`);
            if (completedRounds >= roundsToPlay) {
                const winnerId = this.resolveWinnerByTotalPattes(next);
                const winnerName = winnerId != null ? (0, _playernamehelper.resolvePlayerNameFromState)(next, winnerId) : null;
                next = this.setMeta(next, {
                    ...this.getMeta(next),
                    winnerId,
                    drawnPlayerId: null
                });
                next = this.core.appendLog(next, winnerName ? `${winnerName} remporte la partie avec le plus de pattes cumulées.` : 'Partie terminée.');
                return {
                    ...next,
                    status: 'finished'
                };
    ...
    playObstacle(state, _playerId, targetId, card) {
    ...
    appendPlayedCardNarration(state, playerId, card) {
        return this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerNameFromState)(state, playerId)} joue ${card.name}.`);
    }
    clearDrawn(state) {
        const meta = this.getMeta(state);
        return this.setMeta(state, {
    ...
    resolveWinnerByTotalPattes(state) {
        const meta = this.getMeta(state);
        const players = Array.isArray(state.players) ? state.players : [];
