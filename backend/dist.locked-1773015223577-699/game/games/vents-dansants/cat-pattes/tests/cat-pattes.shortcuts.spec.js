...
describe('CatPattesShortcuts', ()=>{
    it('declares draw/score/progression shortcuts', ()=>{
        ...
        expect(shortcuts.some((s)=>String(s?.type) === 'interface' && String(s?.id) === 'info')).toBe(true);
        expect(shortcuts.some((s)=>String(s?.type) === 'interface' && String(s?.id) === 'discard')).toBe(true);
    });
});
