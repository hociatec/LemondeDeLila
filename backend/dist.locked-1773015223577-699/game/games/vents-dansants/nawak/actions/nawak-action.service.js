...
Object.defineProperty(exports, "NawakActionService", {
    enumerable: true,
    get: function() {
        return NawakActionService;
    }
});
...
let NawakActionService = class NawakActionService {
    ...
    handleChooseAnswer(state, action) {
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(state.players, currentId)} choisit "${answerLabel}".`);
    ...
    handleVoteAnswer(state, action) {
        ...
        let next = this.setMeta(state, meta);
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(state.players, currentId)} vote pour ${(0, _playernamehelper.resolvePlayerName)(state.players, targetPlayerId)}.`);
        const playerIds = this.getPlayerIds(state.players);
    ...
    finishVoting(state, playerIds, votes) {
        ...
        const scoreboard = playerIds.map((pid)=>`${(0, _playernamehelper.resolvePlayerName)(state.players, pid)} ${scores[pid] ?? 0} pts`).join(' / ');
        next = this.core.appendLog(next, `Scores : ${scoreboard}`);
        if (winnerId != null && !tie) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(state.players, winnerId)} atteint ${scores[winnerId] ?? 0} points !`);
            return {
                ...next,
                status: 'finished'
            };
...
NawakActionService = _ts_decorate([
