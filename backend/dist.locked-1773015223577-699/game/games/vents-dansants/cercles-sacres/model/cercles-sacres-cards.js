...
_export(exports, {
    get CERCLES_SACRES_CARD_BY_ID () {
        return CERCLES_SACRES_CARD_BY_ID;
    },
    get CERCLES_SACRES_DECK () {
        return CERCLES_SACRES_DECK;
    }
});
const TOTEM_NAMES = [
    'L?Aigle visionnaire',
...
const createThemeCards = (theme, names)=>names.map((name, index)=>({
            id: `${theme}-${index + 1}`,
            name,
            theme
        }));
...
const CERCLES_SACRES_DECK = deck;
const CERCLES_SACRES_CARD_BY_ID = Object.fromEntries(deck.map((card)=>[
        card.id,
        card
    ]));
