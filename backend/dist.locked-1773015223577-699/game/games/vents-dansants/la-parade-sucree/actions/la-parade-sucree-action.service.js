...
Object.defineProperty(exports, "LaParadeSucreeActionService", {
    enumerable: true,
    get: function() {
        return LaParadeSucreeActionService;
    }
});
...
let LaParadeSucreeActionService = class LaParadeSucreeActionService {
    ...
    handlePlayCard(state, action) {
        ...
        const sequenceValue = _laparadesucreecards.LA_PARADE_SEQUENCE[meta.sequenceIndex];
        ...
        nextMeta = {
            ...nextMeta,
            sequenceIndex: meta.sequenceIndex + 1
        };
        let next = this.setMeta(state, nextMeta);
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} pose ${definition.name} (${definition.value}).`);
    ...
    applySpecialReward(state, playerId, value) {
        const reward = _laparadesucreecards.LA_PARADE_SPECIAL_REWARDS[value];
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} rafle les friandises de la case ${value} (+${gainValue}).`);
        return next;
    }
    computeCandyValue(reward) {
        ...
        for (const [key, amount] of Object.entries(reward)){
            const candyType = key;
            total += (_laparadesucreecards.CANDY_VALUES[candyType] ?? 0) * (amount ?? 0);
        }
        return total;
    }
    ...
    isGameFinished(meta) {
        const allPlayed = meta.sequenceIndex >= _laparadesucreecards.LA_PARADE_SEQUENCE.length;
    ...
    finishGame(state) {
        ...
        return this.core.appendLog(next, winnerId ? `${(0, _playernamehelper.resolvePlayerNameFromState)(next, winnerId)} rafle la Parade Sucrée !` : 'Match nul gourmand !');
    }
...
LaParadeSucreeActionService = _ts_decorate([
