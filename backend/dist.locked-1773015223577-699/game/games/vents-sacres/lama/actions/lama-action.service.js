...
let LamaActionService = class LamaActionService {
    ...
    applyOne(state, action) {
        ...
        if (type === 'lama_pass') {
            const allowPlayAfterDraw = this.shared.asBoolean(metaForTurn?.allowPlayAfterDraw);
            const tracker = metaForTurn.turnTracker ?? null;
    ...
    constructor(shared, drawService, passService, playService, quitService, returnService, infoService, setupService, logger){
        this.shared = shared;
