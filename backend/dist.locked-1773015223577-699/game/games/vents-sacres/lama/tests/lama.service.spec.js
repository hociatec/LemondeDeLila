...
describe('LamaService', ()=>{
    ...
    it('rejects conflicting setup when deck is too small for players and starting hand', async ()=>{
        ...
        expect(messages.some((m)=>m.includes('configuration invalide'))).toBe(true);
    });
    ...
    it('logs every player action (for NVDA announcements)', async ()=>{
        ...
        expect(passMessages.some((m)=>m.includes('passe'))).toBe(true);
        expect(Boolean(afterPass.metadata?.droppedOutByPlayerId?.['1'])).toBe(false);
