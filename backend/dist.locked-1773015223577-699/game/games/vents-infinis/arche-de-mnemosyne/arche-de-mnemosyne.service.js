...
let ArcheDeMnemosyneService = class ArcheDeMnemosyneService extends _abstractgameservice.AbstractGameService {
    ...
    exposeStateForUser(state, userId) {
        ...
        const safeTargetPoints = Number.isFinite(targetPoints) && targetPoints > 0 ? Math.trunc(targetPoints) : 20;
        const scoreMessage = scoreLines.length ? `Score: ${scoreLines.map((s)=>`${s.name}: ${s.score}`).join(', ')}. Objectif: ${safeTargetPoints} point(s).` : `Score: indisponible. Objectif: ${safeTargetPoints} point(s).`;
        const safeMeta = {
            ...metaRest,
