...
const _deckpoliciesservice = require("./deck-policies.service");
describe('DeckPoliciesService', ()=>{
    let service;
    beforeEach(()=>{
        service = new _deckpoliciesservice.DeckPoliciesService(new _randomservice.RandomService());
    });
    it('draws from deck without reshuffle when deck has cards', ()=>{
        const out = service.drawOne({
            meta: {
                deck: [
                    'a',
                    'b'
    ...
    it('supports whole-meta rng reshuffle and puts drawn card in discard when requested', ()=>{
        const out = service.drawFromPile({
            meta: {
                seed: 42,
                counter: 0
            },
