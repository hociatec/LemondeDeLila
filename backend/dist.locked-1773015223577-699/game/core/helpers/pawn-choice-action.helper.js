...
Object.defineProperty(exports, "resolvePendingPawnChoiceAction", {
    enumerable: true,
    get: function() {
        return resolvePendingPawnChoiceAction;
    }
});
function resolvePendingPawnChoiceAction(params) {
    ...
    const playerIdRaw = typeof pending.playerId === 'number' ? pending.playerId : params.state?.turn?.currentPlayerId ?? null;
    if (typeof playerIdRaw !== 'number' || !Number.isFinite(playerIdRaw)) {
        return null;
    }
    const payload = params.action?.payload && typeof params.action.payload === 'object' ? params.action.payload : {};
    const rawChoice = payload.pawnId ?? payload.pawn ?? payload.value ?? null;
    const optionsRaw = Array.isArray(pending?.data?.pawns) ? pending.data.pawns : [];
    const options = optionsRaw.map((entry)=>normalizePawnChoiceOption(entry)).filter((entry)=>entry !== null);
