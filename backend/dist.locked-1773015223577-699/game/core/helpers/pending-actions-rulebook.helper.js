...
_export(exports, {
    get getPendingCardChoiceActionsForPlayer () {
        return getPendingCardChoiceActionsForPlayer;
    },
    get getPendingChooseTargetActionsForPlayer () {
        return getPendingChooseTargetActionsForPlayer;
    },
    get getPendingDrawActionsForPlayer () {
        return getPendingDrawActionsForPlayer;
    },
    get getPendingIndexedChoiceActionsForPlayer () {
        return getPendingIndexedChoiceActionsForPlayer;
    },
    get getPendingNumberChoiceActionsForPlayer () {
        return getPendingNumberChoiceActionsForPlayer;
    },
    get getPendingNumberSetChoiceActionsForPlayer () {
        return getPendingNumberSetChoiceActionsForPlayer;
    },
    get getPendingStringChoiceActionsForPlayer () {
        return getPendingStringChoiceActionsForPlayer;
    },
    get validatePendingCardChoiceActionForActor () {
        return validatePendingCardChoiceActionForActor;
    },
    get validatePendingChooseTargetActionForActor () {
        return validatePendingChooseTargetActionForActor;
    },
    get validatePendingDrawActionForActor () {
        return validatePendingDrawActionForActor;
    },
    get validatePendingIndexedChoiceActionForActor () {
        return validatePendingIndexedChoiceActionForActor;
    },
    get validatePendingNumberChoiceActionForActor () {
        return validatePendingNumberChoiceActionForActor;
    },
    get validatePendingNumberSetChoiceActionForActor () {
        return validatePendingNumberSetChoiceActionForActor;
    },
    get validatePendingStringChoiceActionForActor () {
        return validatePendingStringChoiceActionForActor;
    }
});
...
function validatePendingDrawActionForActor(params) {
    ...
    if (!params.pending || toText(asRecord(params.pending).type) !== pendingType) {
        return {
            ok: false,
...
function getPendingChooseTargetActionsForPlayer(pending, playerId, options) {
    ...
    return targets.map((target)=>Number(target?.[targetKey])).filter((value)=>Number.isFinite(value)).map((targetPlayerId)=>({
            type: pendingType,
            payload: {
                [targetKey]: targetPlayerId
            }
        }));
}
function validatePendingChooseTargetActionForActor(params) {
    ...
    if (!Number.isFinite(targetPlayerId) || !targets.some((t)=>Number(t?.[targetKey]) === targetPlayerId)) {
        return {
            ok: false,
...
function getPendingStringChoiceActionsForPlayer(pending, playerId, options) {
    ...
    const choices = getChoicesFromPending({
        ...
        choicesContainer,
        choicesKey
    }).map((value)=>toText(value)).filter((value)=>value.length > 0);
...
function validatePendingNumberChoiceActionForActor(params) {
    ...
    if (!Number.isFinite(min) || !Number.isFinite(max) || max < min || !Number.isFinite(value) || value < min || value > max) {
        return {
            ok: false,
...
function getPendingNumberSetChoiceActionsForPlayer(pending, playerId, options) {
    ...
    if (!pending || toText(asRecord(pending).type) !== pendingType) return [];
    if (!samePlayer(asRecord(pending).playerId, playerId)) return [];
    const values = asArray(getDataRecord(pending)[valuesKey]).map((entry)=>Number(entry)).filter((entry)=>Number.isFinite(entry));
    return values.map((value)=>({
            type: actionType,
            payload: {
                [payloadValueKey]: value
...
function validatePendingNumberSetChoiceActionForActor(params) {
    ...
    const values = asArray(getDataRecord(params.pending)[valuesKey]).map((entry)=>Number(entry)).filter((entry)=>Number.isFinite(entry));
    const value = Number(asRecord(params.payload)[payloadValueKey]);
...
function getPendingCardChoiceActionsForPlayer(pending, playerId, options) {
    ...
    }).filter((card)=>card.cardType.length > 0 && Number.isFinite(card.cardId)).map((card)=>({
            type: actionType,
            payload: {
...
function validatePendingCardChoiceActionForActor(params) {
    ...
    const found = cards.some((entry)=>{
        const card = asRecord(entry);
        return toText(card[payloadCardTypeKey]) === cardType && Number(card[payloadCardIdKey]) === cardId;
    });
