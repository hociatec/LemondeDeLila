...
let GameCoreService = class GameCoreService {
    cloneLogWithStableTimestamps(logRaw) {
        ...
        for (const entryRaw of log){
            ...
            const asEntry = typeof entryRaw === 'string' ? {
                message: entryRaw
            } : entryRaw;
            const message = String(asEntry?.message ?? '');
            const tsRaw = typeof asEntry?.timestamp === 'string' ? String(asEntry.timestamp).trim() : '';
            if (tsRaw) {
            ...
            const nextMs = lastMs != null ? lastMs + 1 : Date.now() - Math.max(0, log.length) * 2;
            lastMs = nextMs;
            out.push({
                message,
                timestamp: new Date(nextMs).toISOString()
            });
        }
        return out;
    }
