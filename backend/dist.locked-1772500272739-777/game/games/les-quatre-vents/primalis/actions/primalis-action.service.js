...
Object.defineProperty(exports, "PrimalisActionService", {
    enumerable: true,
    get: function() {
        return PrimalisActionService;
    }
});
...
let PrimalisActionService = class PrimalisActionService {
    ...
    handleRoll(state) {
        ...
        if (face === 'relance') {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} utilise la relance et obtient "${reroll.roll}".`);
        }
        ...
        switch(tile.n){
            case 1:
                if (face === 'egg' || face === 'leaf') {
                    const addition = face === 'egg' ? {
                        eggs: 1
                    } : {
                        leaves: 1
                    };
                    next = this.addResources(next, playerId, addition);
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} double sa récolte sur la case 1.`);
                }
                break;
            case 2:
                if (resources.carnivores > resources.herbivores) {
                    next = this.addResources(next, playerId, {
                        herbivores: -1
                    });
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} perd un herbivore (case 2).`);
                }
                break;
            case 3:
                if (face === 'leaf') {
                    ...
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} gagne une feuille supplémentaire (case 3).`);
                }
                break;
            case 4:
                if (face === 'carnivore') {
                    ...
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} récupère un œuf bonus (case 4).`);
                }
                break;
            case 6:
            ...
            case 7:
                ...
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} collecte une feuille magique (case 7).`);
                break;
            case 8:
                if (face === 'herbivore' || face === 'carnivore') {
                    ...
                    next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} transforme sa relance en feuilles (case 8).`);
                }
                break;
            case 9:
                break;
    ...
    applyDanger(state, playerId, tile) {
        let next = state;
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} déclenche un Danger : la comète avance.`);
    ...
    finishGame(state) {
        ...
        for (const entry of entries){
            ...
            if (score > bestScore) {
                ...
                if ((entry.resources.leaves ?? 0) > (best.resources.leaves ?? 0)) {
                    best = entry;
                } else if (entry.resources.leaves === best.resources.leaves && (entry.resources.eggs ?? 0) > (best.resources.eggs ?? 0)) {
                    best = entry;
                }
            }
        }
        ...
        return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, best.id)} survit à la comète avec ${best.resources.herbivores + best.resources.carnivores} dinosaures et ${best.resources.leaves} feuilles.`);
    }
    mapFace(value) {
        switch(value){
            case 1:
...
PrimalisActionService = _ts_decorate([
