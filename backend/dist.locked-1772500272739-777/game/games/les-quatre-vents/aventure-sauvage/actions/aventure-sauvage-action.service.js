...
Object.defineProperty(exports, "AventureSauvageActionService", {
    enumerable: true,
    get: function() {
        return AventureSauvageActionService;
    }
});
...
let AventureSauvageActionService = class AventureSauvageActionService {
    ...
    handleChoosePawn(state, action) {
        ...
        const nextMeta = {
            ...meta,
            pawns: Array.isArray(meta.pawns) && meta.pawns.length > 0 ? meta.pawns : options.map((p)=>({
                    id: toText(p.id),
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} a choisi le pion : ${String(chosen.label ?? 'pion').trim()}.`);
        ...
        const starterName = (0, _playernamehelper.resolvePlayerNameFromState)(started, resolvedStarterId ?? players[0]?.id ?? 0);
    ...
    applyLanding(state, playerId, position) {
        let next = state;
        ...
        next = this.core.appendLog(next, this.boardEffects.createPlacementLog({
    ...
    drawAndApplyCard(state, playerId, deck) {
        let next = state;
        let meta = this.getMeta(next);
        ...
        const prefix = deck === 'animal' ? 'Carte Animal rigolo' : 'Carte Coup de patte';
    ...
    addSkipTurns(state, playerId, turns) {
        const meta = this.getMeta(state);
    ...
    advanceTurn(state) {
        ...
        const meta = this.getMeta(state);
        const statuses = meta.statuses ?? {};
        const baseSkipTurn = statuses.skipTurn ?? {};
        const skipTurn = {
            ...baseSkipTurn
        };
        const currentId = state.turn?.currentPlayerId ?? null;
        const currentIndex = currentId != null ? players.findIndex((p)=>p?.id === currentId) : state.turnIndex;
        const startIndex = currentIndex >= 0 ? currentIndex : typeof state.turnIndex === 'number' ? state.turnIndex : 0;
    ...
    constructor(core, random, setupFlow, boardEffects, deckPolicies, turnPolicies){
        this.core = core;
        this.random = random;
...
AventureSauvageActionService = _ts_decorate([
...
function normalizeAventureMeta(input) {
    ...
    return {
        ...
        decks: {
            animal: Array.isArray(rawDecks.animal) ? rawDecks.animal : [],
            patte: Array.isArray(rawDecks.patte) ? rawDecks.patte : [],
            discardAnimal: Array.isArray(rawDecks.discardAnimal) ? rawDecks.discardAnimal : [],
            discardPatte: Array.isArray(rawDecks.discardPatte) ? rawDecks.discardPatte : []
        },
