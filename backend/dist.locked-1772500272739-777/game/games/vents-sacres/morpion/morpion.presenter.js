...
let MorpionPresenter = class MorpionPresenter extends _basepresenterservice.BasePresenterService {
    exposeStateForUser(state, userId) {
        ...
        for(let y = 0; y < size; y++){
            for(let x = 0; x < size; x++){
                ...
                if (!ownerId) continue;
                const mapped = String(glyphByPlayerId?.[String(ownerId)] ?? '').trim().toUpperCase();
                const glyph = mapped === 'X' || mapped === 'O' ? mapped : ownerId === player0 ? 'X' : ownerId === player1 ? 'O' : '@';
                entities.push({
                    id: `mark:${idx}`,
                    type: 'mark',
                    ownerId,
    ...
    getAvailableActionsForUser(state, userId) {
        ...
        if (pendingType === 'choose_pawn') {
            ...
            return pawns.map((pawn)=>String(pawn?.id ?? '').trim().toUpperCase()).filter((id)=>id === 'X' || id === 'O').map((pawnId)=>({
                    type: 'choose_pawn',
                    payload: {
                        pawnId
                    }
