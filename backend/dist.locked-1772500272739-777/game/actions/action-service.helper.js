...
_export(exports, {
    get applyActionPipeline () {
        return applyActionPipeline;
    },
    get applyActionsSequentially () {
        return applyActionsSequentially;
    },
    get dispatchByActionType () {
        return dispatchByActionType;
    },
    get harmonizeActionStateReturn () {
        return harmonizeActionStateReturn;
    },
    get isRollActionType () {
        return isRollActionType;
    },
    get isRollAlias () {
        return isRollAlias;
    },
    get normalizeActionType () {
        return normalizeActionType;
    },
    get normalizeLegacyRollAliasToUpper () {
        return normalizeLegacyRollAliasToUpper;
    },
    get normalizeLowerActionType () {
        return normalizeLowerActionType;
    },
    get normalizeRollActionType () {
        return normalizeRollActionType;
    }
});
...
function applyActionPipeline(state, action, handlers) {
    ...
    const payload = handlers.validate ? handlers.validate(state, action) : undefined;
    const transitionResult = handlers.transition(state, action, payload);
    const effectedState = handlers.effects ? handlers.effects(state, action, payload, transitionResult) : transitionResult;
    return handlers.logs ? handlers.logs(state, action, payload, transitionResult, effectedState) : effectedState;
}
...
function dispatchByActionType(type, handlers, fallback) {
    ...
    const resolvedType = handlers[type] ? type : alias && handlers[alias] ? alias : type;
