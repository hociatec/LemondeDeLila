...
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `JWT_ALGORITHM`                                                              | `RS256`, unique algorithme accepté.                                                                                                                       |
...
| `CLIENT_WX_UPDATES_UPLOAD_TOKEN`                                             | Secret d'authentification des publications WX.                                                                                                           |
| `CLIENT_WX_MIN_VERSION`                                                      | Version WX minimale d'urgence.                                                                                                                           |
...
Le code WX du backend est isolé dans `src/modules/update` (`UpdateModule`). En présence
...
Le déploiement normal crée une release isolée et adressée par le SHA-256 exact
du snapshot source. Depuis la racine du projet :

```bash
sudo updatecmd backend
```

La commande crée un instantané cohérent, installe les
...
précédente. Le workflow de secours fournit explicitement son checkout par
`--source`, ce qui empêche de déployer un état mutable laissé sur le runner.
Voir `tools/updatecmd/README.md` pour l'installation et les autres commandes.
...
- Les jeux prototypes (ex. _Mission Nemesis_) sont désactivés tant que `ENABLE_PROTOTYPE_GAMES=true` n’est pas défini. Ils n’apparaissent plus dans le catalogue tant que leur manifest contient `"enabled": false`.
