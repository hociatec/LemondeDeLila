...
let FrousseActionService = class FrousseActionService {
    ...
    handleRoll(state) {
        ...
        if (blocked) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} tente de se libérer : dé = "${rollLabel}".`);
            const ok = blocked.kind === 'need_roll_one_of'
                ...
                : blocked.kind === 'need_roll_min'
                    ...
                    : blocked.kind === 'need_roll_even'
                        ...
                        : false;
            if (!ok) {
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} reste bloqué.`);
                return this.advanceTurnWithAnnouncement(next);
            }
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} se libère !`);
            return this.advanceTurnWithAnnouncement(next);
        }
        const roll = this.roll(meta, currentId);
        meta = roll.meta;
        ...
        if (roll.rolls && roll.rolls.length >= 2) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} lance deux dés : "${roll.rolls[0]}" et "${roll.rolls[1]}" (garde "${roll.value}").`);
        }
        else {
            const rollLabel = this.formatRollLabel(roll);
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} lance le dé : "${rollLabel}".`);
        }
    ...
    applyLanding(state, playerId) {
        ...
        if (pos >= 49) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} s'échappe du manoir !`);
    ...
    applyDrawCard(state, playerId) {
        ...
        if (/Fantôme/i.test(draw.card.category) &&
    ...
    applyCard(state, playerId, card) {
        ...
        if (/Fantôme/i.test(card.category) &&
            ...
            next = this.core.appendLog({ ...next, metadata: { ...(next.metadata ?? {}), ...meta } }, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} échange sa position avec ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, target.id)}.`);
            return next;
        }
        if (/échang|echange/i.test(text) &&
    ...
    finalizeStarterAfterPawnSelection(state) {
        ...
        if (typeof starter?.id === 'number') {
            next = this.core.appendLog(next, `[Frousse Party] Début de partie : ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, starter.id)} commence.`);
