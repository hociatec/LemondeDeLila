...
exports.MissionGalaxieActionService = void 0;
...
let MissionGalaxieActionService = class MissionGalaxieActionService {
    ...
    applyActions(state, actions) {
        return (0, action_service_helper_1.applyActionsSequentially)((0, action_service_helper_1.harmonizeActionStateReturn)(state), actions, (next, action) => {
            const current = (0, action_service_helper_1.harmonizeActionStateReturn)(next);
    ...
    handleRoll(state) {
        ...
        if (skipTurns > 0) {
            ...
            const skipped = this.core.appendLog({ ...state, metadata: { ...(state.metadata ?? {}), ...meta } }, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)} passe son tour (${skipTurns} restant).`);
    ...
    handleDraw(state) {
        ...
        if (!(0, pending_action_service_1.isPendingType)(state, 'draw'))
            return state;
        ...
        const deckName = typeof pendingData.deck === 'string'
            ? pendingData.deck
            : undefined;
        if (!deckName)
            return state;
        ...
        next = (0, pending_action_service_1.createPendingState)(next, pendingState);
        next = {
            ...next,
            metadata: { ...(next.metadata ?? {}), ...updatedMeta },
        };
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} pioche la carte "${card.title}".`);
    }
    handleChooseOption(state, action) {
        ...
        if (currentId == null)
            return state;
        if (!(0, pending_action_service_1.isPendingType)(state, 'choose_option'))
            return state;
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} répond à "${card.title}" : ${isCorrect ? 'Correct' : 'Erreur'} (${delta >= 0 ? 'avance' : 'recule'} ${Math.abs(delta)}).`);
    ...
    handleChooseEventMove(state, action) {
        ...
        if (currentId == null)
            return state;
        const pending = state.pending;
        if (!(0, pending_action_service_1.isPendingType)(state, 'choose_event_move'))
            return state;
        const payload = asRecord(action.payload);
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} applique ${delta >= 0 ? 'un boost' : 'une perturbation'} à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetPlayerId)} (${delta >= 0 ? '+' : ''}${delta}).`);
    ...
    applyLanding(state, playerId) {
        ...
        switch (tile.type) {
            case 'move':
                if (typeof tile.delta === 'number' && tile.delta !== 0) {
                    next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} suit l'effet du plateau (${tile.delta >= 0 ? 'avance' : 'recule'} ${Math.abs(tile.delta)}).`);
            ...
            case 'skip': {
                ...
                return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} doit sauter ${addition} tour(s).`);
            }
            case 'question':
        ...
        if (tile.keepTurn) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} reçoit un tour bonus.`);
        }
        return next;
    }
    promptDraw(state, playerId, deck) {
        ...
        return (0, pending_action_service_1.createPendingState)(state, pending);
    }
    applySwapNearest(state, playerId) {
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} échangée sa position avec ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, closest.id)}.`);
    }
    applyEventCard(state, playerId, card) {
        let next = this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} déclenche l'événement "${card.title}".`);
        const effect = card.effect;
        switch (effect.kind) {
            case 'move':
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} avance de ${effect.delta} cases.`);
                next = this.move(next, playerId, effect.delta);
                return this.applyLanding(next, playerId);
            case 'skip':
                next = this.addSkip(next, playerId, effect.turns);
                return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} doit sauter ${effect.turns} tour(s).`);
            case 'none':
                return next;
            case 'reroll':
                next = this.setKeepTurn(next);
                return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} relance immédiatement le dé.`);
            case 'keepTurn':
                next = this.setKeepTurn(next);
                return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} rejoue immédiatement.`);
            case 'goto':
                next = this.setPos(next, playerId, Math.max(0, Math.min(effect.target - 1, this.getMeta(next).tiles.length - 1)));
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} avance jusqu'à la case ${effect.target}.`);
                return this.applyLanding(next, playerId);
            case 'skipOthers':
                next = this.skipOthers(next, playerId, effect.turns);
                return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} force les autres à sauter ${effect.turns} tour(s).`);
    ...
    promptPlayerMove(state, playerId, deltas) {
        ...
        for (const delta of deltas) {
            for (const player of targetPlayers) {
                ...
                options.push({
                    ...
                    delta,
                    label: `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)} ${delta >= 0 ? `avance de ${delta}` : `recule de ${Math.abs(delta)}`}`,
                });
            }
        }
        const pending = {
        ...
        next = (0, pending_action_service_1.createPendingState)(next, pending);
        next = {
            ...next,
            metadata: { ...(next.metadata ?? {}), ...nextMeta },
        };
    ...
    skipOthers(state, playerId, turns) {
        ...
        for (const player of players) {
            if (player?.id == null || player.id === playerId)
                continue;
    ...
    finishGame(state, playerId) {
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} atteint la planète légendaire !`);
    }
...
exports.MissionGalaxieActionService = MissionGalaxieActionService;
exports.MissionGalaxieActionService = MissionGalaxieActionService = __decorate([
...
//# sourceMappingURL=mission-galaxie-action.service.js.map