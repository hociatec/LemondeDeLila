...
exports.SetupFlowService = void 0;
...
let SetupFlowService = class SetupFlowService {
    createSequentialChoicePending(params) {
        ...
        const label = typeof params.labelForPlayer === 'function'
        ...
        const pending = {
            ...
            data: typeof params.dataBuilder === 'function'
    ...
    createSequentialPawnPending(params) {
        ...
        return this.createSequentialChoicePending({
            ...
            choices: pawns.map((pawn) => ({
                ...pawn,
                label: typeof params.choiceLabelBuilder === 'function'
                    ? String(params.choiceLabelBuilder(pawn) ?? pawn.label).trim()
                    : pawn.label,
            })),
            labelForPlayer: params.labelForPlayer ??
            ...
            dataBuilder: (availableChoices) => ({
                ...
                ...(params.includeChoiceMapData === true
                    ...
                    : {}),
                pawns: availableChoices.map((choice) => typeof params.pawnDataMapper === 'function'
                    ? params.pawnDataMapper(choice)
                    : this.defaultPawnData(choice)),
            }),
        });
    }
    toPlayerId(value) {
    ...
    normalizeChoices(choices) {
        return (Array.isArray(choices) ? choices : [])
            .map((choice) => ({
            ...choice,
            id: (0, string_value_utils_1.stringOrEmpty)(choice?.id).trim(),
            label: (0, string_value_utils_1.stringOrEmpty)(choice?.label).trim(),
        }))
    ...
    normalizePawnChoices(choices) {
        return (Array.isArray(choices) ? choices : [])
            .map((choice) => {
            const id = (0, string_value_utils_1.stringOrEmpty)(choice?.id).trim();
            const label = (0, string_value_utils_1.stringOrEmpty)(choice?.label ?? id).trim();
    ...
    defaultPawnData(choice) {
        return {
            id: (0, string_value_utils_1.stringOrEmpty)(choice?.id).trim(),
            label: (0, string_value_utils_1.stringOrEmpty)(choice?.label).trim(),
            description: (0, string_value_utils_1.stringOrEmpty)(choice?.description).trim(),
        };
    }
    playerLabel(player) {
        const username = (0, string_value_utils_1.stringOrEmpty)(player?.username).trim();
...
exports.SetupFlowService = SetupFlowService;
exports.SetupFlowService = SetupFlowService = __decorate([
    (0, common_1.Injectable)()
], SetupFlowService);
//# sourceMappingURL=setup-flow.service.js.map