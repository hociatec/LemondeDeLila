...
let DameNatureActionService = class DameNatureActionService {
    ...
    handleAskCard(state, action) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} récupère ${this.getCardName(cardId)} de ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)}.`);
    ...
    drawCardForPlayer(state, playerId) {
        ...
        if (!cardId) {
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} ne trouve plus aucune carte à piocher.`);
        }
