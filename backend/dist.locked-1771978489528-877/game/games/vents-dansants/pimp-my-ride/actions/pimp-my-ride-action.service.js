...
let PimpMyRideActionService = class PimpMyRideActionService {
    ...
    handleDiscardCard(state, action) {
        ...
        next = this.setMeta(next, updatedMeta);
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} jette ${this.getCardName(cardId)} à la défausse.`);
        next = this.turns.advanceTurn(next);
        return this.clearDrawn(next);
    }
    ...
    ensurePlayerDrawn(state, playerId) {
        ...
        if (cardId) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} ne trouve plus de cartes à piocher.`);
        }
        return next;
    }
    drawOneCard(meta) {
