...
let GameEngineService = class GameEngineService {
   ...
   */ async getStateForUser(roomId, gameType, userId) {
    ...
    exposeStateForUser(state, gameType, userId) {
        ...
        const finalState = (0, _mojibake.fixMojibakeDeep)(this.stripBoardAndGridIfNotStarted(withShortcuts));
        if (byState) {
   ...
   */ async restoreInternalState(roomId, gameType, state) {
    ...
    async scheduleBotTurn(roomId, gameType, state) {
        ...
        const fastPendingBotAction = Number.isFinite(pendingPlayerId) && pendingPlayerId === botActorId && (pendingType === 'draw' || pendingType === 'pick_pawn' || pendingType === 'choose_target');
    ...
    exposeState(state, gameType) {
        ...
        return (0, _mojibake.fixMojibakeDeep)(this.stripBoardAndGridIfNotStarted(withDescriptors));
    }
