...
let GameGateway = GameGateway_1 = class GameGateway {
    ...
    encodedStateMessageCache = new WeakMap();
    topLevelSnapshotCache = new WeakMap();
    lastPayloadBySocket = new WeakMap();
    pingIntervalMs = 25000;
    pongGraceMs = 4000;
    minPatchBytesSaved = 96;
    ...
    constructor(engine, auth, perf, clientUpdates, wsTickets, roomService, content) {
        ...
        (0, playing_logger_1.playingLog)('ws.game.gateway.init', {
    ...
    async handleConnection(client, ...args) {
        ...
        const interval = setInterval(() => {
            ...
            if (Date.now() - last > this.pingIntervalMs * 2) {
                ...
                (0, playing_logger_1.playingLog)('ws.game.heartbeat.timeout', {
    ...
    handleDisconnect(client) {
        ...
        if (meta) {
            (0, playing_logger_1.playingLog)('ws.game.disconnect', {
                userId: meta.userId,
    ...
    async handleMessage(client, raw) {
        ...
        try {
                ...
                case 'game.ping':
                    this.safeSend(client, {
                        ...
                        payload: {
                            ...
                            clientSentAtMs: typeof payload?.clientSentAtMs === 'number'
            ...
            (0, playing_logger_1.playingLog)('ws.game.error', {
    ...
    async handleJoin(client, meta, payload) {
        ...
        await this.perf.measure('ws.game.join.total', async () => {
            ...
            (0, playing_logger_1.playingLog)('ws.game.join', { userId: meta.userId, roomId, gameType });
    ...
    async handleState(client, meta, payload) {
        ...
        await this.perf.measure('ws.game.state.total', async () => {
            ...
            const state = await this.engine.getStateForUser(roomId, gameType, meta.userId);
            this.setRoom(meta, roomId, gameType, client);
            (0, playing_logger_1.playingLog)('ws.game.state.request', {
                userId: meta.userId,
                roomId,
                gameType,
            });
    ...
    async handleTurn(client, meta, payload) {
        ...
        await this.perf.measure('ws.game.turn.total', async () => {
            ...
            (0, playing_logger_1.playingLog)('ws.game.turn.request', {
                userId: meta.userId,
                roomId,
                gameType,
    ...
    async handleActions(client, meta, payload) {
        ...
        await this.perf.measure('ws.game.actions.total', async () => {
            ...
            (0, playing_logger_1.playingLog)('ws.game.actions', {
                userId: meta.userId,
                roomId,
                gameType,
    ...
    async handleBot(meta, payload) {
        ...
        await this.perf.measure('ws.game.bot.play.total', async () => {
            ...
            (0, playing_logger_1.playingLog)('ws.game.bot.play', {
                userId: meta.userId,
                roomId,
                gameType,
            });
    ...
    broadcastState(gameType, roomId, state) {
        ...
        (0, playing_logger_1.playingLog)('ws.game.broadcast', {
            roomId,
            gameType,
    ...
    broadcastEnded(gameType, roomId, state, payload) {
        ...
        for (const socket of Array.from(targets)) {
            ...
            const outcomesByPlayerId = payload.outcomesByPlayerId &&
        ...
        (0, playing_logger_1.playingLog)('ws.game.ended', {
            roomId,
            gameType,
            subscribers: targets ? targets.size : 0,
    ...
    sanitizeOutgoingPayload(payload) {
        return (0, mojibake_1.fixMojibakeDeep)(payload);
    }
    ...
    buildTopLevelPatch(previousPayload, nextPayload) {
        ...
        const baseTurnIndex = typeof rawBaseTurnIndex === 'number' && Number.isFinite(rawBaseTurnIndex)
            ? rawBaseTurnIndex
            : null;
        return {
            baseTurnIndex,
    ...
    async tryAutoJoinFromUrl(client, args) {
        ...
        await this.perf.measure('ws.game.auto_join.total', async () => {
            ...
            const state = await this.engine.getStateForUser(roomId, gameType, meta.userId);
            this.setRoom(meta, roomId, gameType, client);
            (0, playing_logger_1.playingLog)('ws.game.auto_join', {
                userId: meta.userId,
                roomId,
                gameType,
            });
    ...
    extractTrace(payload) {
        ...
        const traceId = typeof trace['id'] === 'string' && trace['id'].trim()
            ? trace['id'].trim()
            : null;
        const sentAtMs = typeof trace['sentAtMs'] === 'number' &&
...
exports.GameGateway = GameGateway = GameGateway_1 = __decorate([
    (0, websockets_1.WebSocketGateway)({ path: '/ws/game' }),
