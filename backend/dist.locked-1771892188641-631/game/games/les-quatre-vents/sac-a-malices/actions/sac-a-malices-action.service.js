...
exports.SacAMalicesActionService = void 0;
...
function getHouseCost(group, level) {
    ...
    if (Number.isFinite(perLevel ?? NaN))
        return Number(perLevel);
...
let SacAMalicesActionService = class SacAMalicesActionService {
    ...
    applyVariantConfig(state, action) {
        ...
        const candidateVariant = toStringValue(payload.variant ?? payload.variantId ?? payload.value) ??
            toStringValue(meta.variantId);
        const parsedCandidate = (0, sac_a_malices_variants_1.parseVariantInput)(candidateVariant ?? null);
        const parsed = parsedCandidate ?? 'classic';
        const variant = sac_a_malices_variants_1.SAC_VARIANT_BY_ID[parsed] ?? sac_a_malices_variants_1.SAC_VARIANT_BY_ID['classic'];
        ...
        if (!parsedCandidate) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, actorId)} choisit la variante : ${label}.`);
        }
        else {
    ...
    handleRoll(state) {
        ...
        if (jailTurns > 0) {
            if (rules.jail.allowDoubleEscape) {
                ...
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} lance les dés : "${d1}" + "${d2}" = "${sum}".`);
                if (!isDouble) {
    ...
    handleBuy(state, accept) {
        ...
        if (!pending || pendingRow.type !== 'buy')
            return state;
        ...
        if (!accept) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} n'achète pas.`);
            next = this.checkWinner(next);
            if (this.getMeta(next).winnerId != null)
                return { ...next, status: 'finished' };
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} achète "${tile.title}" pour ${price} €.`);
        next = this.checkWinner(next);
        if (this.getMeta(next).winnerId != null)
            return { ...next, status: 'finished' };
        return this.advanceTurnOrExtraRoll(next, playerId);
    ...
    openChooseProperty(state, kind) {
        ...
        for (let tileIndex = 0; tileIndex < tiles.length; tileIndex += 1) {
            ...
            if (!tile)
                continue;
            if (meta.ownership?.[tileIndex] !== playerId)
                continue;
            const b = this.getBuilding(meta, tileIndex);
            if (kind === 'build') {
                if (tile.type !== 'property')
                    continue;
                if (b.mortgaged || b.hotel)
                    continue;
                const group = this.getGroup(meta, tile.group ?? '');
                if (!group)
                    continue;
                if (!this.isGroupComplete(meta, playerId, group))
                    continue;
                const supportsHotel = Number(group?.hotelPrice ?? 0) > 0 &&
                ...
                const cost = supportsHotel && b.houses >= 4
                    ? Number(group.hotelPrice ?? 0) || 0
                    : houseCost;
                if (!Number.isFinite(cost) || cost <= 0 || myCash < cost)
                    continue;
            ...
            if (kind === 'mortgage') {
                if (b.mortgaged)
                    continue;
                if (tile.type === 'property' && (b.hotel || b.houses > 0))
                    continue;
    ...
    handleChooseProperty(state, action) {
        ...
        const pending = state.pending;
        const pendingRow = asRecord(pending);
        if (!pending || pendingRow.type !== 'choose_property')
            return state;
        const playerId = typeof pendingRow.playerId === 'number'
            ? pendingRow.playerId
            : (state.turn?.currentPlayerId ?? null);
        ...
        if (wanted == null)
            return state;
        if (!options.some((o) => o.tileIndex === wanted))
            return state;
        ...
        if (!tile)
            return next;
        if (meta.ownership?.[wanted] !== playerId)
            return next;
        if (kind === 'build') {
    ...
    applyLanding(state, playerId) {
        ...
        if (!tile)
            return next;
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} place ${this.pawnLabel(next, playerId)} en case ${pos + 1} (${tile.title}).`);
        ...
        if (tile.type === 'property' ||
            ...
            if (rent > 0) {
                next = this.core.appendLog(next, `Loyer : ${rent} € à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, owner)}.`);
    ...
    drawCard(meta, deckId) {
        const deck = meta.decks?.[deckId] ?? { cards: [], discard: [] };
    ...
    advanceTurnOrExtraRoll(state, playerId) {
        const meta = this.getMeta(state);
        if (meta.statuses?.eliminated?.[playerId])
            return this.advanceTurn(state);
    ...
    isGroupComplete(meta, ownerId, group) {
        const props = Array.isArray(group?.properties)
            ? group.properties
            : [];
        if (!props.length)
            return false;
        ...
        if (!idxs.length)
            return false;
        return idxs.every((idx) => meta.ownership?.[idx] === ownerId &&
            !this.getBuilding(meta, idx).mortgaged);
    }
    getMortgageValue(meta, tile) {
        if (tile.type === 'station')
            return meta.data?.stations?.mortgage ?? 0;
    ...
    buildOne(state, playerId, tileIndex) {
        ...
        if (!supportsHotel && b.houses >= 4)
            return state;
        const nextLevel = clamp(b.houses + 1, 1, 4);
        const houseCost = Number(group?.housePrices?.[String(nextLevel)] ??
            group?.housePrice ??
            0) || 0;
        const cost = supportsHotel && b.houses >= 4
            ? Number(group.hotelPrice ?? 0) || 0
            : houseCost;
        const cash = meta.money?.[playerId] ?? 0;
        if (!Number.isFinite(cost) || cost <= 0 || cash < cost)
            return state;
        let next = state;
        next = this.addMoney(next, playerId, -cost, { toPot: false });
        if (this.getMeta(next).statuses?.eliminated?.[playerId])
            return next;
        if (supportsHotel && b.houses >= 4) {
    ...
    addMoney(state, playerId, delta, options) {
        ...
        const nextMeta = {
            ...
            pot: rules.potEnabled && options.toPot
        ...
        if (nextMoney < 0) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} est en faillite !`);
    ...
    getPurchasePrice(meta, tile) {
        if (tile.type === 'station')
            return meta.data?.stations?.purchasePrice ?? 0;
        if (tile.type === 'utility') {
            const u = meta.data?.utilities?.find((x) => normalize(x.name) === normalize(tile.title));
    ...
    getRent(meta, tile, tileIndex, ownerId, lastRoll) {
        ...
        if (tile.type === 'property') {
            ...
            const b = this.getBuilding(meta, tileIndex);
            if (b.hotel)
                return Number(group.rents?.hotel ?? 0) || 0;
            const houses = clamp(Number(b.houses ?? 0) || 0, 0, 4);
            if (houses <= 0)
                return Number(group.rents?.base ?? 0) || 0;
            if (houses === 1)
                return Number(group.rents?.house1 ?? 0) || 0;
            if (houses === 2)
                return Number(group.rents?.house2 ?? 0) || 0;
            if (houses === 3)
                return Number(group.rents?.house3 ?? 0) || 0;
    ...
    checkWinner(state) {
        ...
        if (alive.length === 1) {
            ...
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, winnerId)} remporte la partie !`);
        }
        return state;
    }
    getMeta(state) {
...
exports.SacAMalicesActionService = SacAMalicesActionService;
exports.SacAMalicesActionService = SacAMalicesActionService = __decorate([
...
function extractSkipTurns(text) {
    ...
    if (/Passez deux tours/i.test(text))
        return 2;
    if (/Passez votre prochain tour/i.test(text) ||
...
function mentionsInfrastructureLoss(text) {
    return (/perd(?:ez|s)?\s+une\s+infrastructure/i.test(text) ||
        /perds\s+une\s+infrastructure/i.test(text));
}
//# sourceMappingURL=sac-a-malices-action.service.js.map