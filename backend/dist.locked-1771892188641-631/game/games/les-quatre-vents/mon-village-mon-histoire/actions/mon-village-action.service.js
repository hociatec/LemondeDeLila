...
exports.MonVillageActionService = void 0;
...
let MonVillageActionService = class MonVillageActionService {
    ...
    collectCard(state, playerId, tileNumber) {
        ...
        if (!card) {
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} n’a plus de cartes dans la zone ${zoneId}.`);
        }
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} collecte "${card.title}".`);
    ...
    finishGame(state, playerId) {
        ...
        if (!best)
            best = { id: playerId, total: 0, byZone: {} };
        ...
        if (tied.length > 1) {
            for (const zone of ZONE_MAP.map((range) => range.id)) {
                ...
                if (zoneBest && zoneBest.count > 0) {
                    ...
                    if (tied.some((entry) => entry.id !== best.id &&
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, best.id)} remporte la partie avec ${best.total} cartes !`);
        return next;
    }
...
exports.MonVillageActionService = MonVillageActionService;
exports.MonVillageActionService = MonVillageActionService = __decorate([
...
//# sourceMappingURL=mon-village-action.service.js.map