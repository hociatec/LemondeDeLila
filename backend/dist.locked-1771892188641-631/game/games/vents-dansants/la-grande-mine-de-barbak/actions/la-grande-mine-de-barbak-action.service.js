...
exports.LaGrandeMineDeBarbakActionService = void 0;
...
let LaGrandeMineDeBarbakActionService = class LaGrandeMineDeBarbakActionService {
    ...
    ensurePlayerDrawn(state, playerId) {
        ...
        const definition = la_grande_mine_cards_1.LA_GRANDE_MINE_CARD_BY_ID[cardId];
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} pioche ${definition?.name ?? 'une carte'}.`);
        if (!definition) {
            return next;
        }
    ...
    playTreasure(state, playerId, cardId, card) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} pose le trésor ${card.name} (+${card.points ?? 0} pts).`);
        return next;
    }
    playObject(state, playerId, cardId, card) {
        ...
        let next = this.setMeta(state, { ...meta, domains });
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} installe l'objet ${card.name}.`);
        return next;
    }
    applyEventEffect(state, playerId, card, played) {
        const message = played
            ? `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} utilise ${card.name} (${card.description}).`
            : `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} déclenche ${card.name} (${card.description}).`;
    ...
    applyMonsterEffect(state, playerId, targetId, card) {
        ...
        if (chosenId == null) {
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} tente de lancer ${card.name}, mais il n'y a personne.`);
        }
        next = this.removeRandomDomainCard(next, chosenId);
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} attaque ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, chosenId)} avec ${card.name}.`);
        return next;
    }
    applyCollapseEffect(state, cardId, playerId, played) {
        ...
        else if (cardId === 'barbak-collapse-3' ||
            cardId === 'barbak-collapse-4') {
            next = this.finishGame(next);
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} déclenche un effondrement final !`);
        }
        if (played) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} joue l'effondrement ${cardId}.`);
        }
        return next;
    }
    ...
    discardRandomFromHand(state, playerId, count) {
        ...
        for (let i = 0; i < count; i += 1) {
            ...
            if (cardId) {
                next = this.addCardToDiscard(next, cardId);
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} défausse ${cardId}.`);
            }
        }
        return next;
    }
    removeRandomTreasure(state, playerId, count) {
        ...
        for (let i = 0; i < count; i += 1) {
            ...
            if (cardId) {
                next = this.addCardToDiscard(next, cardId);
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} perd le trésor ${cardId}.`);
            }
        }
        return next;
    }
    removeRandomDomainCard(state, playerId) {
        ...
        if (cardId) {
            return this.addCardToDiscard(this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} perd ${cardId} face à une attaque.`), cardId);
        }
        return next;
    }
    trimHand(state, playerId) {
        ...
        while (hand.length > 5) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} réduit sa main et défausse ${removed}.`);
    ...
    finishGame(state) {
        ...
        return this.core.appendLog(next, winnerId
            ? `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, winnerId)} devient le Nain suprême !`
...
exports.LaGrandeMineDeBarbakActionService = LaGrandeMineDeBarbakActionService;
exports.LaGrandeMineDeBarbakActionService = LaGrandeMineDeBarbakActionService = __decorate([
...
//# sourceMappingURL=la-grande-mine-de-barbak-action.service.js.map