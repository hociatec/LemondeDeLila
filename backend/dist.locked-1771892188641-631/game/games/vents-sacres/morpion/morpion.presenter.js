...
exports.MorpionPresenter = void 0;
...
let MorpionPresenter = class MorpionPresenter extends base_presenter_service_1.BasePresenterService {
    gridCellActions;
    ...
    getAvailableActionsForUser(state, userId) {
        if (!this.isStarted(state))
            return [];
        if (state.turn?.currentPlayerId !== userId)
            return [];
        const meta = (state.metadata ?? {});
        const size = meta.size ?? 3;
        const board = Array.isArray(meta.board) ? meta.board : [];
    ...
    buildExtrasForUser(state, _metadata, _userId, currentPlayerId) {
        ...
        const who = typeof currentPlayerId === 'number'
            ? (players.find((p) => p?.id === currentPlayerId)?.username ??
                `#${currentPlayerId}`)
            : 'inconnu';
        const playInfo = String(state.status ?? '').toLowerCase() === 'started'
...
exports.MorpionPresenter = MorpionPresenter;
exports.MorpionPresenter = MorpionPresenter = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [grid_cell_actions_service_1.GridCellActionsService])
], MorpionPresenter);
//# sourceMappingURL=morpion.presenter.js.map