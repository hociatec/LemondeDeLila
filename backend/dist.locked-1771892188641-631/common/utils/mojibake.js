...
function normalizeFrenchDisplayTypos(value) {
    ...
    if (!out)
        return '';
    out = out
        .replace(/\bmise\s+a\s+jour\b/gi, (raw) => preserveWordCase(raw, 'mise à jour'))
        .replace(/\bmises\s+a\s+jour\b/gi, (raw) => preserveWordCase(raw, 'mises à jour'))
        .replace(/\bpublication\/distribution\b/gi, (raw) => preserveWordCase(raw, 'publication/distribution'))
        .replace(/\ba\s+l(?:['’]|\s+)echeance\b/gi, (raw) => preserveWordCase(raw, "à l'échéance"));
    const wordMap = {
        delai: 'délai',
        delais: 'délais',
        echeance: 'échéance',
