...
exports.DameNatureActionService = void 0;
...
let DameNatureActionService = class DameNatureActionService {
    ...
    handleAskCard(state, action) {
        ...
        if (!targetId || !cardId)
            return state;
        const cardDefinition = dame_nature_cards_1.DAME_NATURE_CARD_BY_ID[cardId];
        if (!cardDefinition || cardDefinition.type !== 'family')
            return state;
        const meta = this.getMeta(state);
        if (!this.playerHasCard(meta, targetId, cardId)) {
            return this.drawAndAdvance(state, currentId, `La carte ${this.getCardName(cardId)} n'est pas chez le joueur demand�.`);
        }
        let next = this.transferCardBetweenPlayers(state, targetId, currentId, cardId);
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} r�cup�re ${this.getCardName(cardId)} de ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)}.`);
    ...
    drawCardForPlayer(state, playerId) {
        ...
        if (!cardId) {
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} ne trouve plus aucune carte � piocher.`);
        }
        ...
        if (definition.type === 'quiz') {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} lit le quiz : ${definition.question}`);
    ...
    applyNatureEffect(state, playerId, delta, description) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} subit : ${description} (${delta >= 0 ? '+' : ''}${delta} jetons pollution).`);
        if (pollution >= 12) {
            next = {
                ...next,
                status: 'finished',
...
exports.DameNatureActionService = DameNatureActionService;
exports.DameNatureActionService = DameNatureActionService = __decorate([
...
//# sourceMappingURL=dame-nature-action.service.js.map