...
const {
  isEngineAudit,
  reconcileEngineAudit,
} = require('./current-engine-backlog.cjs');
if (isEngineAudit(backlog)) {
  const reconciliation = reconcileEngineAudit(backlog);
  fs.writeFileSync(
...
assert(
  open.every(
    (id, index) =>
      id >= 1 && id <= 744 && (index === 0 || id > open[index - 1]),
  ),
  'Original backlog IDs must remain ordered and within 1-744',
);
...
const isCurrentSnapshot =
  ...
  backlog.startsWith(`${snapshotHeader}\r\n`);
const numberedBody = isJsonSnapshot
  ? backlog.slice('Snapshot JSON complet :'.length)
  : isCurrentSnapshot
    ? backlog.slice(snapshotHeader.length)
    : backlog;
assert(
  isJsonSnapshot
    ? !numberedBody.trim() || /^\d+\. \S/.test(numberedBody.trim())
    : numberedBody
        .split(/\r?\n/)
...
if (isCurrentSnapshot) {
  ...
  const result = spawnSync(
    process.execPath,
    [path.join(__dirname, 'backlog-governance-check.cjs')],
    { encoding: 'utf8' },
  );
  ...
  console.log(
    JSON.stringify({ open: open.length, ...reconciliation }, null, 2),
  );
} else {
  ...
  for (const ids of Object.values(data.groups))
    for (const id of ids) record(String(id), jsonFile);
  for (const file of [
  ...
  record(
    '1, 2, 3, 4, 5, 7, 9, 26, 28, 31, 40-43, 55-57, 59-62, 66-67, 72-73, 105, 112, 116, 121-122, 124-128, 130, 135, 144-145, 147-150, 157, 159-160',
  ...
  record(
    '30, 32, 35',
    'corrections-lot-context-pattern-boundaries-2026-09-10.md',
  );
  ...
  const lotCount = [...closed.values()].filter((sources) =>
    sources.has(lotFile),
  ).length;
  const lotHeader = lot.match(
    /\*\*(\d+) cl(?:ôtures|Ã´tures) sur les 100 vis(?:ées|Ã©es)/,
  );
  ...
  console.log(
    JSON.stringify(
      { open: open.length, documentedClosed: closed.size, retainedClosed },
      null,
      2,
    ),
  );
  fs.writeFileSync(
    'logs/corrections-backlog-reconciliation.json',
    `${JSON.stringify({ checkedAt: new Date().toISOString(), openCount: open.length, retainedClosed, closedEvidence: [...closed].sort((a, b) => a[0] - b[0]).map(([id, sources]) => ({ id, sources: [...sources] })) }, null, 2)}\n`,
  );
  assert.equal(
    retainedClosed.length,
    0,
    'Verified closed points remain in backlog',
  );
}
