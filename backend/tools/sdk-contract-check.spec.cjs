...
const {
  captureContract,
  compareContracts,
} = require('./sdk-contract-check.cjs');
const {
  inspectCapabilities,
  assertSdkCapabilities,
} = require('./sdk-capability-contract.cjs');

test('author controllers require explicit names rather than exposing every runtime member', () => {
  assert.doesNotThrow(assertSdkCapabilities);
  assert.deepEqual(
    inspectCapabilities(
      "type API = PublicController<Runtime, 'read' | 'write'>",
    ),
    [],
  );
  for (const source of [
    'type API = PublicController<Runtime>',
    'type API = PublicController<Runtime, keyof Runtime>',
    'type API = PublicController<Runtime, string>',
  ])
    assert.equal(inspectCapabilities(source).length, 1);
});

function fixture(run) {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'lila-sdk-contract-'),
  );
  ...
  const capture = () =>
    captureContract({
      directory,
      entry: path.join(directory, 'sdk.ts'),
      options: {
...
test('detects transitive payload, return, readonly and optional changes without renaming exports', () => {
  fixture(({ write, capture }) => {
    ...
    write(
      'action.ts',
      "import type { Payload } from './payload'; export function execute(input: Payload): number { return input.value; }",
    );
    write(
      'payload.ts',
    ...
    write(
      'action.ts',
      "import type { Payload } from './payload'; export function execute(input: Payload): string { return String(input.value); }",
    );
    assert.deepEqual(compareContracts(before, capture().snapshot), [
      'action.ts',
    ]);
...
test('follows import types, re-export barrels and cyclic declaration dependencies', () => {
  fixture(({ write, capture }) => {
    ...
    write(
      'edge.ts',
      "export interface Edge { node: import('./node').Node; weight: number }",
    );
    const before = capture().snapshot;
    assert.equal(before.files.length, 4);
    write(
      'edge.ts',
      "export interface Edge { node: import('./node').Node; weight: string }",
    );
...
test('ignores implementation bodies and unrelated modules, and emits no files on disk', () => {
  fixture(({ directory, write, capture }) => {
    write(
      'sdk.ts',
      'export function score(value: number): number { return value + 1; }',
    );
    const before = capture().snapshot;
    write(
      'sdk.ts',
...
test('captures mapped author capabilities when an underlying controller signature changes', () => {
  fixture(({ write, capture }) => {
    write(
      'sdk.ts',
      "import type { Controller } from './controller'; export type Capability = Pick<Controller, keyof Controller>;",
    );
    write(
      'controller.ts',
      'export declare class Controller { private state; add(value: number): void; }',
    );
    const before = capture().snapshot;
    write(
      'controller.ts',
      'export declare class Controller { private state; add(value: string): void; }',
    );
    assert.deepEqual(compareContracts(before, capture().snapshot), [
      'controller.ts',
    ]);
