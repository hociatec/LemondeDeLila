...
function captureContract({ directory, entry, options, apiVersion = '10.0.0' }) {
  ...
  const result = program.emit(
    undefined,
    (_file, body, _bom, _error, sources) => {
      if (!_file.endsWith('.d.ts')) return;
      for (const source of sources || [])
        declarations.set(path.resolve(source.fileName), body);
    },
  );
  const diagnostics = [
  ...
  while (queue.length) {
    ...
    const body =
      ...
      (source?.isDeclarationFile ? source.text : undefined);
    ...
    for (const specifier of declarationImports(tree)) {
      ...
      if (!resolved)
        throw new Error(
          `Unresolved declaration dependency: ${specifier} from ${file}`,
        );
      if (
        resolved.isExternalLibraryImport ||
        normalize(resolved.resolvedFileName).includes('/node_modules/')
      ) {
        external.add(
          resolved.packageId
  ...
  const files = [...reached]
    .map(([file, body]) => {
      ...
      if (relative.startsWith('../') || path.isAbsolute(relative))
        throw new Error(`Declaration outside workspace: ${file}`);
...
function compareContracts(expected, actual) {
  ...
  const before = new Map(
    expected.files.map((file) => [file.file, file.sha256]),
  );
