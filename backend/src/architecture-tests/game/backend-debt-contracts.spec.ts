...
import {
  ...
  pawns,
  simultaneous,
} from '../../game/core/application/public-api';
import { GameConfigurationError } from '../../game/core/domain/errors/game-domain.errors';
import { GameTestKit } from '../../game/core/testing/game-test-kit';
import { DeclarativeGameRuntime } from '../../game/core/application/runtime/declarative-game.runtime';
import { GameSimulator } from '../../game/core/testing/game-simulator';
...
describe('backend debt contracts', () => {
  ...
  it('uses only the generated registry for runtime game discovery', () => {
    const source = readFileSync(
      resolve(__dirname, '../../game/composition/game-module-discovery.ts'),
      'utf8',
    );
...
function gameSources(): Array<{ file: string; source: string }> {
  const root = resolve(__dirname, '../../game/games');
