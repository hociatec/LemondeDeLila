...
const WINDOWS_1252_BYTES: Readonly<Record<number, number>> = {
  0x20ac: 0x80,
  0x201a: 0x82,
  0x0192: 0x83,
  0x201e: 0x84,
...
function mojibakeScore(value: string): number {
  ...
  return suspicious * 2 + (value.match(/\uFFFD/g) ?? []).length * 10;
}

function windows1252ToBytes(input: string): Uint8Array {
  const bytes: number[] = [];
  for (const character of input) {
    const codePoint = character.codePointAt(0) ?? 0;
    bytes.push(
      codePoint <= 0xff ? codePoint : (WINDOWS_1252_BYTES[codePoint] ?? 0x3f),
    );
  }
...
export function fixMojibakeString(value: string): string {
  ...
  const targetedOriginalScore = mojibakeScore(targetedOriginal);
  ...
  for (const c of candidates) {
    ...
    const normalizedScore = mojibakeScore(normalized);
