  ...
  implements OnGatewayConnection<WebSocket>, OnGatewayDisconnect<WebSocket>
{
  ...
  constructor(
    ...
    // Permet au backend (ex: moteur de jeu) de notifier les clients room sans dÃ©pendre du Gateway.
    ...
    // Permet Ã  l'admin (via RoomService) de forcer la suppression d'une room
    // en dÃ©connectant tous les clients WS connectÃ©s Ã  cette table.
    ...
    await this.enqueueClientMessage(client, async () => {
      ...
      try {
        ...
        await this.sendError(
          client,
          (err as Error).message || 'Erreur temps rÃ©el',
        );
      }
    });
  }
