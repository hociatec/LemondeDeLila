...
describe('GameWsStateMessagesPresenter', () => {
  it('announces drawing a question and recording an answer', () => {
    const messages = [
      { id: 'quiz:0', type: 'game.message', data: { key: 'game.quiz.started', params: { playerId: 1 } } },
      { id: 'quiz:1', type: 'game.message', data: { key: 'game.quiz.answer-recorded', params: { playerId: -2 } } },
    ];
    const state = new GameWsStateMessagesPresenter().withServerMessages(
      {
        match: { status: 'playing' },
        players: { all: [{ id: 1, username: 'Lila' }, { id: -2, username: 'Milou' }] },
        events: { recent: messages, latestByType: { 'game.message': messages[1] } },
      },
      1,
      {} as never,
    );

    expect((state.events as any).recent.map((event: any) => event.data.message)).toEqual([
      'Vous piochez une question.', 'Milou a répondu.',
    ]);
  });
