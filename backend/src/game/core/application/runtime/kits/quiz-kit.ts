...
export class GameQuizController {
  constructor(
    ...
    const legacy = this.state as QuizKitState & {
      banks?: Record<string, QuizQuestion[]>;
    };
    this.state.orders ??= Object.fromEntries(
      Object.entries(legacy.banks ?? {}).map(([bankId, questions]) => [
        bankId,
    ...
    for (const session of Object.values(this.state.sessions)) {
      const legacy = session as QuizSessionState & {
        question?: Omit<QuizQuestion, 'answerIndex'>;
        questionId?: string;
      };
      legacy.questionId ??= legacy.question?.id ?? '';
      delete legacy.question;
    }
  }
