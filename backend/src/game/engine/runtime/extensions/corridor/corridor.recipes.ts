...
import type { CorridorPosition, CorridorProgram } from './program';
...
type Wall = CorridorPosition & { orientation: Orientation };
...
export function corridorRules(source: CorridorProgram) {
  ...
  const move = defineAction<State, CorridorPosition>({
  ...
  function positions(ctx: Context): PlayerMap<CorridorPosition> {
  ...
  function legalMoves(actorId: number, ctx: Context): CorridorPosition[] {
  ...
  function inside(position: CorridorPosition) {
    return (
      position.x >= 0 &&
      position.y >= 0 &&
...
function same(a: CorridorPosition, b: CorridorPosition) {
  return a.x === b.x && a.y === b.y;
}
function sameWall(a: Wall, b: Wall) {
  return same(a, b) && a.orientation === b.orientation;
}
function key(position: CorridorPosition) {
  return `${position.x},${position.y}`;
}
function unique(items: CorridorPosition[]) {
