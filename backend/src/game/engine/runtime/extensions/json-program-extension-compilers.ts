...
import {
  jsonProgramExtensions,
  type RegisteredJsonProgramExtension,
} from './json-program-extension-registry';
...
type CompiledExtensionEntry<Extension> =
  Extension extends RegisteredJsonProgramExtension
    ? {
        readonly [Key in Extension['outputKey']]: ReturnType<
          Extension['compile']
        > | null;
      }
    : never;
...
export type CompiledJsonPrograms = UnionToIntersection<
  CompiledExtensionEntry<RegisteredJsonProgramExtension>
> & {
