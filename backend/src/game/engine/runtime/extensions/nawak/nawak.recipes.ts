...
import type { NawakProgram } from './program';
...
export function nawakRules(source: NawakProgram) {
...
function runtime(state: State): RuntimeState {
  if (!isRuntimeState(state))
    throw new TypeError('Invalid Nawak runtime state');
  return state;
}
