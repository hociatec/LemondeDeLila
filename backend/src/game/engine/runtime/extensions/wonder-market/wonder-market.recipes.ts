import type { WonderMarketProgram } from './program';
...
export function wonderMarketRules(source: WonderMarketProgram) {
...
function advance(program: WonderMarketProgram, ctx: Context): void {
...
function choose(program: WonderMarketProgram, playerId: number, ctx: Context) {
  const sell = program.goods.find((good) =>
    ctx.inventory.has(program.inventoryId, playerId, good),
  );
  if (sell)
    return { recipe: 'wonder-market-sell' as const, payload: { good: sell } };
  ...
  return buy
    ? { recipe: 'wonder-market-buy' as const, payload: { good: buy } }
    : { recipe: 'wonder-market-pass' as const, payload: {} };
}
