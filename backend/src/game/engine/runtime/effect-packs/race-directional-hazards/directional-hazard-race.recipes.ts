...
} from './directional-hazard-effects';
import {
  directionalHazardMirrorSource,
  incrementDirectionalHazardIdle,
} from './directional-hazard-turn';

type State = Record<string, never>;
type Context = GameContext<State>;

export function directionalHazardRaceRules(
  source: DirectionalHazardRaceProgram,
) {
  ...
  return {
        ...
        resolve: ({ value, ctx }) => {
          ...
          if (pending?.kind !== 'next-delta' || (value !== -1 && value !== 1))
            throw new GameRuleViolationError(
              'DIRECTIONAL_HAZARD_NEXT_DELTA_INVALID',
            );
    ...
    effects: directionalHazardEffects(program),
  };
}
function executeRoll(
  ...
  const source = directionalHazardMirrorSource(program, actorId, ctx);
  let value =
  ...
  incrementDirectionalHazardIdle(program, actorId, delta, ctx);
