...
import { drawAndResolve } from '../../recipes/gameplay/card-dice.recipes';
...
type Card = BounceQuizRaceProgram['cards'][number];
type Pending = { kind: 'quiz'; actorId: number; cardId: number };

export function bounceQuizRaceRules(source: BounceQuizRaceProgram) {
...
function effects(program: BounceQuizRaceProgram) {
  return {
    ...
    'race-bounce-quiz.roll': defineEffect<State, Record<string, never>>({
      input: gameInput.object({}),
    ...
    'race-bounce-quiz.gift': defineEffect<State, Record<string, never>>({
      input: gameInput.object({}),
    ...
    'race-bounce-quiz.swap': defineEffect<State, Record<string, never>>({
      input: gameInput.object({}),
      apply: ({ actorPlayerId, targetPlayerIds, ctx }) => {
        const targetId = targetPlayerIds[0];
    ...
    'race-bounce-quiz.swap-behind': defineEffect<State, Record<string, never>>({
      input: gameInput.object({}),
      apply: ({ actorPlayerId, ctx }) => {
        if (actorPlayerId == null) return;
        const current = ctx.movement.position(program.trackId, actorPlayerId);
