...
function effects(program: ChapterEncounterProgram) {
  ...
  return {
    ...
    'choice-chapter-encounter.swap-last-player': defineEffect<
      State,
      Record<string, never>
    >({
      input: gameInput.object({}),
...
function advanceCountdown(program: ChapterEncounterProgram, ctx: Context) {
...
function resource(
  program: ChapterEncounterProgram,
  kind: ChapterEncounterCollectionKind,
) {
