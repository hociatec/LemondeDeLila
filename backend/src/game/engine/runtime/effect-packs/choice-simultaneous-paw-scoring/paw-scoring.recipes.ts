...
} from './paw-scoring-rules';
import { powerIgnoresObstacle } from './paw-round-helpers';
...
export function pawScoringRules(source: PawScoringProgram) {
  ...
  function powers(playerId: number, ctx: Context): PawScoringPower[] {
    return ctx.status
      ...
      .filter((power): power is PawScoringPower =>
  ...
  function isObstacle(value: unknown): value is PawScoringObstacle {
    return (
      value === 'gamelle' ||
      value === 'pluie' ||
      value === 'chien' ||
    ...
    return {
      ...
      lifecycle: {
        ...
        onRoundStart: ({ ctx }: { state: State; ctx: Context }) =>
          resetPawScoringRound(program, status.turboPlayed, ctx),
      },
