...
import type {
  StoryChallengeDrawResolution as Resolution,
  StoryChallengePendingResolution as Pending,
} from './story-challenge-resolution.types';
...
export function createStoryChallengeResolution(program: StoryChallengeProgram) {
  ...
  function requestAbundance(actorId: number, ctx: Context) {
    const cards = [
      ctx.cards.drawOrRecycle<StoryChallengeCard>('bonus'),
      ctx.cards.drawOrRecycle<StoryChallengeCard>('bonus'),
