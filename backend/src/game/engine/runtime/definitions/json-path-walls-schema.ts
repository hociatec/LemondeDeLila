import type { PathWallsProgram } from '../effect-packs/board-path-walls/program';
import {
  type AuthorSchema,
...
export function assertPathWallsReferences(program: PathWallsProgram): void {
  if (
    ...
    throw new Error('PathWalls pawn identifiers must be unique');
  if (
    program.startPositions.some(
      (position) => position.x >= program.size || position.y >= program.size,
    )
  )
    throw new Error('PathWalls start position is outside the board');
}
