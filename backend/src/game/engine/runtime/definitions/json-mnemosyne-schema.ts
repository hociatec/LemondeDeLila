...
export const jsonMnemosyneSchema: AuthorSchema = object(
  {
    categories: array(
...
export function assertMnemosyneReferences(program: MnemosyneProgram): void {
  ...
  if (
    ...
    throw new Error(
      'Mnemosyne category identifiers must be unique and exclude all',
    );
  const allowed = new Set(categoryIds);
  if (!program.questions.some((question) => question.status === 'validated'))
    throw new Error('Mnemosyne requires validated questions');
  if (
    ...
    throw new Error('Mnemosyne question references an unknown category');
}
