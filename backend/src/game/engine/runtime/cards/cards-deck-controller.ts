...
export abstract class GameCardsDeckController extends GameCardsStateController {
  ...
  discardPile<TCard extends CardValue>(deckId: string): TCard[] {
    return (this.state.discards[deckId] ?? []).map((card) =>
      this.fromPersistentCard<TCard>(deckId, card),
    );
  }

  takeDiscard<TCard extends CardValue>(deckId: string, card: TCard): TCard {
  ...
  draw<TCard extends CardValue>(deckId: string): TCard | null {
  ...
  drawOrRecycle<TCard extends CardValue>(deckId: string): TCard | null {
    let card = this.drawOne<TCard>(deckId);
  ...
  drawToHand<TCard extends CardValue>(
    deckId: string,
    handId: string,
    playerId: number,
  ...
  drawManyToHand<TCard extends CardValue>(
    deckId: string,
    handId: string,
    playerId: number,
    count: number,
  ...
  drawThenResolve<TCard extends CardValue, TResult>(
    deckId: string,
  ...
  putOnTop<TCard extends CardValue>(
    deckId: string,
