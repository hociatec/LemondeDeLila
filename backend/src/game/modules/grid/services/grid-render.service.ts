...
export class GridRenderService {
  attachGridRenderDescriptors(state: GameStateWithActions): GameStateWithActions {
    ...
    const gridExisting = (extras as any).grid;
    if (
      !gridExisting ||
    ...
    const grid = { ...(gridExisting as Record<string, unknown>) };
    if ((grid as any).render !== undefined) {
      return state;
    }

    const blockedEdges = (grid as any).blockedEdges;
    if (
      !blockedEdges ||
    ...
    const sizeRaw = (grid as any).size;
    const size =
    ...
    const cells: Record<
      string,
      { walls: { n: boolean; e: boolean; s: boolean; w: boolean }; border: any }
    > = {};
    ...
    (grid as any).render = { cells, thick };
