import { BoardEffectsPoliciesService } from './board-effects-policies.service';

describe('BoardEffectsPoliciesService', () => {
  let service: BoardEffectsPoliciesService;

  beforeEach(() => {
    service = new BoardEffectsPoliciesService();
  });

  it('formats tile labels consistently', () => {
    expect(service.formatTileLabel(3, '')).toBe('Case 4');
    expect(service.formatTileLabel(3, 'Case 4 - Case neutre')).toBe(
      'Case 4 - Case neutre',
    );
    expect(service.formatTileLabel(3, 'Couloir des portraits')).toBe(
      'Case 4 - Couloir des portraits',
    );
  });
  ...
  it('builds placement log', () => {
    ...
    expect(line).toBe(
      'Lilas place sa citrouille rigolote en case 2 (Case 2 - Hall du manoir).',
    );
  });
  ...
  it('creates draw pending for configured tile type', () => {
    const landing = service.resolveLanding({
      position: 9,
      playerId: 7,
      tile: { type: 'symbol', description: 'Un vieux parquet craque.' },
      drawPolicies: {
        symbol: {
  ...
  it('marks finish without pending', () => {
    const landing = service.resolveLanding({
      position: 49,
      playerId: 3,
      tile: { type: 'finish', description: 'Arrivee.' },
    });
