import { drawAndResolve } from '../../../engine/sdk/public-api';
import type {
  PropertyEconomyCard,
...
import {
  ...
  type PropertyEconomyContext,
  type PropertyEconomyManagement,
  type PropertyEconomyState,
  type createPropertyEconomySupport,
} from './property-economy-support';

type Support = ReturnType<typeof createPropertyEconomySupport>;
const MAX_CHAIN_DEPTH = 24;

export function createPropertyEconomyBoard(support: Support) {
  function rollPair(ctx: PropertyEconomyContext): [number, number] {
  ...
  function resolveJailTurn(
    ...
    if (rules.jail.allowDoubleEscape) {
      ...
      if (first === second) {
        ctx.resources.set(playerId, PROPERTY_ECONOMY.jailTurns, 0);
        ctx.turn.extra();
    ...
    const remaining = Math.max(
      0,
      ctx.resources.get(playerId, PROPERTY_ECONOMY.jailTurns) - 1,
    );
    ctx.resources.set(playerId, PROPERTY_ECONOMY.jailTurns, remaining);
  ...
  function moveForward(
    ...
    const current = ctx.movement.position(PROPERTY_ECONOMY.track, playerId);
  ...
  function collectPot(
    ...
    const amount = ctx.counters.drain(PROPERTY_ECONOMY.pot);
  ...
  function resolveOwnable(
    ...
    const ownerId = ctx.ownership.ownerOf(PROPERTY_ECONOMY.properties, tile.id);
    if (ownerId == null) {
      const price = support.purchasePrice(support.current(ctx), tile);
      ctx.choice.one({
        id: 'board-property-economy.purchase',
        player: playerId,
        options: ctx.resources.has(playerId, PROPERTY_ECONOMY.money, price)
          ? ['buy', 'skip']
          : ['skip'],
    ...
    if (
      ownerId === playerId ||
      propertyEconomyBuildings(state)[tileIndex]?.mortgaged
    )
      return;
    const rules = support.current(ctx).rules;
    if (
      rules.rentBlockedInJail &&
      ctx.resources.get(ownerId, PROPERTY_ECONOMY.jailTurns) > 0
    )
      return;
    ...
    ctx.events.message('board-property-economy.rent.paid', {
      playerId,
      ownerId,
      amount: rent,
      tileId: tileIndex,
  ...
  function drawCard(
    ...
    drawAndResolve<PropertyEconomyState, PropertyEconomyCard, boolean>(ctx, {
      deckId,
      playerId,
      resolve: (card) => {
  ...
  function resolvePurchase(
    ...
    if (pending?.flow !== 'purchase')
      rejectRule('Achat PropertyEconomy à Malices absent');
    if (decision === 'buy')
  ...
  function buyTile(
    ...
    if (
      ctx.ownership.isOwned(PROPERTY_ECONOMY.properties, tile.id) ||
      price <= 0 ||
      !ctx.resources.has(playerId, PROPERTY_ECONOMY.money, price)
    )
      return;
    support.changeMoney(state, playerId, -price, false, ctx);
    ctx.ownership.claim(PROPERTY_ECONOMY.properties, tile.id, playerId);
    propertyEconomyBuildings(state)[tileIndex] = {
      houses: 0,
      hotel: false,
      mortgaged: false,
    };
    ctx.events.message('board-property-economy.property.bought', {
      playerId,
      tileId: tileIndex,
      amount: price,
    });
  ...
  function resolveManagement(
    ...
    if (pending?.flow !== 'management')
      rejectRule('Gestion PropertyEconomy à Malices absente');
    if (
      !support
        .managementOptions(state, pending.playerId, pending.kind, ctx)
        .includes(tileIndex)
    )
      rejectRule('Propriété PropertyEconomy à Malices invalide');
  ...
  function movementTarget(
    playerId: number,
    movement: Exclude<PropertyEconomyMovement, { kind: 'delta' }>,
    ctx: PropertyEconomyContext,
  ) {
    const variant = support.current(ctx);
    const current = ctx.movement.position(PROPERTY_ECONOMY.track, playerId);
...
function isOwnable(tile: PropertyEconomyTile) {
