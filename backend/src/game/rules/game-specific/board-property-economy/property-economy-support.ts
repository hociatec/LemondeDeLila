...
import { settleResourceDelta } from '../../recipes/resource-settlement';
import type {
...
export function createPropertyEconomySupport(source: PropertyEconomyProgram) {
  ...
  function current(ctx: PropertyEconomyContext): PropertyEconomyVariant {
    const id =
      ctx.config.get<PropertyEconomyVariantId>('variantId') ??
      program.defaultVariantId ??
      program.variants[0].id;
    return (
  ...
  function changeMoney(
    ...
    settleResourceDelta(
      ...
      delta,
      {
        ...
        onShortfall: () => bankrupt(state, playerId, ctx),
      },
    );
  }
  function bankrupt(
