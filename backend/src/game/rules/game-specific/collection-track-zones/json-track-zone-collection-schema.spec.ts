import { legacyExtensionFixture } from '../../../engine/testing/public-api';
import { compileJsonGame } from '../../public-api';
import manifest from '../../../games/les-quatre-vents/mon-village-mon-histoire/manifest.json';
import documentExtensionSource from '../../../games/les-quatre-vents/mon-village-mon-histoire/game.json';
import catalogue from '../../../games/les-quatre-vents/mon-village-mon-histoire/content/catalogue.json';
const document = legacyExtensionFixture(
  documentExtensionSource,
  'trackZoneCollection',
);

const assets = { 'content/catalogue.json': catalogue };

it.each([
  { trackId: 'missing' },
  { diceId: 'missing' },
  {
    zones: document.trackZoneCollection.zones.map((zone, index) =>
      index === 0 ? { ...zone, deckId: 'missing' } : zone,
    ),
  },
  {
    zones: document.trackZoneCollection.zones.map((zone, index) =>
      index === 0 ? { ...zone, resourceId: 'missing' } : zone,
    ),
  },
])('rejects invalid track-zone collection references %j', (extension) => {
  expect(() =>
    compileJsonGame(
      manifest,
      {
        ...document,
        trackZoneCollection: { ...document.trackZoneCollection, ...extension },
      },
      assets,
    ),
  ).toThrow();
});

it('rejects cards attached to another zone', () => {
  const zones = catalogue.zones.map((zone, index) =>
    index === 0
      ? {
          ...zone,
          cards: zone.cards.map((card) => ({
            ...card,
            attributes: { zoneId: 2 },
          })),
        }
      : zone,
  );
  expect(() =>
    compileJsonGame(manifest, document, {
      'content/catalogue.json': { ...catalogue, zones },
    }),
  ).toThrow(/matching zoneId/);
});
