...
import { resolveJsonContent } from '../../public-api';
...
import { parseJsonGame } from '../../public-api';
import {
  AuthoringError,
  authoringValueAt,
...
const precisePaths: Record<string, string> = {
  'duplicate tiles': 'board.tiles[1].id',
  'unknown track': 'board.trackId',
  'unknown dice': 'board.diceId',
  'unknown quiz bank': 'board.quiz.bankId',
  'unknown inventory': 'board.collection.requiredInventoryId',
  'unknown pawn set': 'board.pawnSelection.setId',
  'duplicate choices': 'board.exchange.giveChoiceId',
  'unknown phase': 'board.playingPhase',
  'unreachable phase': 'board.playingPhase',
  'missing quiz': 'board.quiz',
  'missing collection': 'board.collection',
  'missing exchange': 'board.exchange',
  'missing direction': 'board.directionChoiceId',
  'unknown source': 'board.collection.defaultSourceId',
  'unknown item': 'board.distribution.groups[0][0]',
  'inverted range': 'board.tiles[0].operations[0].minimum',
  'unknown deck': 'board.tiles[0].operations[0].deckId',
  'unknown collection source': 'board.tiles[0].operations[0].sourceId',
  'unknown tag': 'board.bindings["panier.nearest-stand"].tag',
};
