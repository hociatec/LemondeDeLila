...
export function bounceQuizRaceRules(source: BounceQuizRaceProgram) {
  ...
  function resolveAnswer(value: number, ctx: Context) {
  ...
  function resolveDestination(playerId: number, depth: number, ctx: Context) {
  ...
  function applyTile(playerId: number, depth: number, ctx: Context) {
    let current = ctx.movement.position(program.trackId, playerId);
    if (
      firstOtherPlayerAt(ctx, program.trackId, current, playerId) !== undefined
    ) {
  ...
  function drawCard(playerId: number, depth: number, ctx: Context) {
