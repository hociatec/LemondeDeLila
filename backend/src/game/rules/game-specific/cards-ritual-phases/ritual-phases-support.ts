import { gameEffects, drawEvent } from '../../../engine/sdk/public-api';
import type {
...
export function createRitualPhasesSupport(source: RitualPhasesProgram) {
  ...
  function freeFamily(playerId: number, ctx: RitualPhasesContext) {
    ...
    const visit = (
      ...
      if (selected.length === program.exchangeFamilyCount) {
        choices.push(selected);
        return;
      }
      for (let index = start; index < hand.length; index++) {
        const card = byId.get(hand[index]);
        if (card?.type === 'family' && !families.has(card.familyId))
          visit(
            index + 1,
            [...selected, card.id],
            new Set([...families, card.familyId]),
          );
      }
    };
    visit(0, [], new Set());
