...
export function ritualPhasesRules(source: RitualPhasesProgram) {
  ...
  const createActions = () => {
    ...
    const pass = defineEmptyAction<RitualPhasesState>({
  ...
  const createEffects = () => ({
    ...
    'cards-ritual-phases.dawn-cycle': defineEmptyEffect<RitualPhasesState>(
      ({ ctx }) => {
        for (const player of ctx.players.all()) {
          const cardId = ctx.cards.hand<string>(
            RITUAL_PHASES.hands,
...
function effect(
  ...
  return defineEmptyEffect<RitualPhasesState>(({ targetPlayerIds, ctx }) => {
