import { gameInput } from '../../../engine/sdk/public-api';
...
export function createStoryChallengeEffects(
  ...
  const movementEffects = () => ({
    ...
    >({
      ...
      apply: ({ state, targetPlayerIds, data, ctx }) => {
        for (const playerId of targetPlayerIds) {
          const roll = ctx.dice.roll(program.diceId).total;
          const delta =
            data.mode === 'double'
              ? roll * 2
              : data.mode === 'half'
    ...
    >({
      input: gameInput.object({
        type: gameInput.enum(Object.keys(program.decks)),
      }),
      apply: ({ state, targetPlayerIds, data, ctx }) => {
        for (const playerId of targetPlayerIds)
    ...
    >({
      input: gameInput.object({
        effect: gameInput.enum(Object.keys(program.targetRules)),
      }),
    ...
    >({
      input: gameInput.object({
        types: gameInput.array(
          gameInput.enum([
            program.deckRoles.reward,
            program.deckRoles.penalty,
            program.deckRoles.event,
            program.deckRoles.story,
          ]),
          { min: 1 },
        ),
      }),
    ...
    'choice-story-challenge.queue-random-draws': defineEmptyEffect<State>(
      ({ targetPlayerIds, ctx }) => {
        const playerId = targetPlayerIds[0];
        if (playerId == null) return;
        const types: StoryChallengeCardType[] = [...program.randomDrawDecks];
        resolution.queueDraws(
          playerId,
          ctx.random.shuffle(types).slice(0, program.randomDrawCount),
          ctx,
        );
      },
    ),
  ...
  const storyEffects = () => ({
    'choice-story-challenge.force-one-others': defineEmptyEffect<State>(
      ({ targetPlayerIds, ctx }) => {
        const playerId = targetPlayerIds[0];
        if (playerId == null) return;
    ...
    'choice-story-challenge.skip-if-low-roll': emptyEffect(
      ({ targetPlayerIds, ctx }) => {
        ...
        if (
          playerId != null &&
          ctx.dice.roll(program.diceId).total < program.lowRollThreshold
        )
    ...
    'choice-story-challenge.option': defineEffect<State, { effect: string }>({
      input: gameInput.object({
        effect: gameInput.enum(Object.keys(program.optionRules)),
      }),
      apply: ({ targetPlayerIds, data, ctx }) => {
        const playerId = targetPlayerIds[0];
        if (playerId != null)
    ...
    'choice-story-challenge.conte': emptyEffect(({ targetPlayerIds, ctx }) => {
      ...
      if (playerId != null && ctx.status.has(playerId, statuses.keyOfGold))
        resolution.scheduleTarget(
          playerId,
          program.conditionalTargetEffect,
          ctx,
        );
    }),
...
function emptyEffect(
  ...
  return defineEmptyEffect<State>(apply);
}
