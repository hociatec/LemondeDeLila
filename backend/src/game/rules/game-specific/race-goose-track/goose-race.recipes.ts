...
import type { GooseRaceProgram, GooseTile } from './program';
    ...
    roll: defineAction<State, Record<string, never>>({
      ...
      execute: ({ actor, ctx }) => {
        ...
        if (ctx.status.has(actor.id, program.wellStatus)) {
          if (!program.escapeRolls.includes(value)) {
...
function land(
  ...
  const operation = program.tileRules[tile.type];
  if (operation === 'finish')
    ...
    land(
      program,
      playerId,
      tile.backTo ?? program.defaultReturn,
      rollValue,
      depth + 1,
      ctx,
    );
  else if (operation === 'skip')
    ctx.turn.skip(playerId, tile.turnsToSkip ?? program.defaultSkip);
  else if (operation === 'roll-directed') {
    const magic = ctx.dice.roll(program.diceId).total;
    const delta = magic <= program.forwardRollMaximum ? magic : -magic;
    land(
      program,
      playerId,
