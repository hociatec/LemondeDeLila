...
export function assertProtectedHauntedRaceReferences(
  ...
  for (const rule of program.protections)
    if (!program.cards.some((card) => card.category === rule.category))
      fail('unknown protected category');
  if (
    new Set(program.protections.map((rule) => rule.status)).size !==
    program.protections.length
  )
    fail('duplicate protection status');
