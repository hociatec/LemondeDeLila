...
export function assertPublicDomainCardsReferences(
  ...
  if (
    new Set(program.collectibleCategories).size !==
      program.collectibleCategories.length ||
    !program.collectibleCategories.includes(program.lossCategory)
  )
    fail('invalid collectible categories');
