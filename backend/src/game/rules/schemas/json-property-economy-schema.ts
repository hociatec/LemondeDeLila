...
} from '../effect-packs/board-property-economy/program';
...
export function assertPropertyEconomyReferences(
  ...
  const fail = (reason: string): never => {
    throw new GameConfigurationError('PropertyEconomy à Malices: ' + reason);
  };
  if (
    new Set(program.variants.map((candidate) => candidate.id)).size !==
      program.variants.length ||
    (program.defaultVariantId != null &&
      !program.variants.some(
        (candidate) => candidate.id === program.defaultVariantId,
      ))
  )
