...
export function assertSpeciesTroopsReferences(
  ...
  if (new Set(program.species).size !== program.species.length)
    fail('duplicate collection kind');
  if (program.species.some((value) => value.includes(':')))
    fail('collection kind cannot contain the inventory separator');
  for (const card of program.cards) {
    if (card.species != null && !program.species.includes(card.species))
      fail('unknown collection kind on ' + card.id);
