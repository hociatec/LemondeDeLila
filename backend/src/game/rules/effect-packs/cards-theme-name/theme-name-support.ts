...
export function createThemeNameCardsSupport(
        ...
        for (const player of ctx.players.all()) {
          ctx.cards.drawManyToHand(
            ...
            Math.max(
              0,
              program.handSize -
                ctx.cards.hand(THEME_NAME.names, player.id).length,
            ),
            { recycle: true },
          );
          ctx.cards.drawManyToHand(
  ...
  function specialInputs(
    ...
    const targeting = program.specialRules.find(
      (rule) => rule.id === effect,
    )?.targeting;
    if (targeting === 'held-card')
      return ctx.cards
        .hand<ThemeNameCardsNameCard>(THEME_NAME.names, actorId)
        .map((name) => ({ cardId, name: name.id }));
    const targeted =
      targeting === 'opponent' ||
      targeting === 'two-opponents' ||
      targeting === 'pending-opponent';
    const targets = targeted
      ? ctx.players.otherIds(actorId).filter((playerId) => {
          if (targeting !== 'pending-opponent') return true;
  ...
  function validateTargets(
    ...
    for (const target of [input.targetPlayerId, input.secondaryTargetId])
      if (target != null && (target === actorId || !ctx.players.get(target)))
        rejectRule('Cible de carte invalide');
  }
  function allowedNameCount(playerId: number, ctx: ThemeNameCardsContext) {
    return Math.min(
      program.maximumSubmission,
      1 + ctx.resources.get(playerId, THEME_NAME.extraNames),
    );
  }
