...
export function propertyEconomyRules(source: PropertyEconomyProgram) {
  ...
  >({
    ...
    defaults: { variantId: program.defaultVariantId ?? variantIds[0] },
  ...
  const createTurnActions = () => {
    const roll = defineEmptyAction<PropertyEconomyState>({
    ...
    function management(kind: PropertyEconomyManagement) {
      return defineEmptyAction<PropertyEconomyState>({
    ...
    const payFine = defineEmptyAction<PropertyEconomyState>({
    ...
    const useJailCard = defineEmptyAction<PropertyEconomyState>({
  ...
  const createEffects = () => ({
    'board-property-economy.lose-infrastructure':
      defineActorEffect<PropertyEconomyState>(({ state, actorPlayerId, ctx }) =>
