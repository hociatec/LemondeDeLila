...
describe('pending-actions-rulebook.helper', () => {
  ...
  it('handles indexed pending choices from data and root', () => {
    ...
    expect(getPendingIndexedChoiceActionsForPlayer(fromData, 9)).toEqual([
      { type: 'choose_option', payload: { choiceIndex: 0 } },
      { type: 'choose_option', payload: { choiceIndex: 1 } },
    ]);
    expect(
      validatePendingIndexedChoiceActionForActor({
        pending: fromData,
      ...
      ok: true,
      choiceIndex: 1,
      action: { type: 'choose_option', payload: { choiceIndex: 1 } },
    });
    ...
    expect(
      getPendingIndexedChoiceActionsForPlayer(fromRoot, 4, {
        pendingType: 'pick',
      ...
      { type: 'pick_choice', payload: { index: 0 } },
      { type: 'pick_choice', payload: { index: 1 } },
      { type: 'pick_choice', payload: { index: 2 } },
    ]);
    ...
    ).toEqual({ ok: false, reason: 'invalid_choice', choiceIndex: 3 });
  });
  ...
  it('handles string pending choices from root choices', () => {
    ...
    expect(getPendingStringChoiceActionsForPlayer(pending, 2)).toEqual([
      { type: 'choose_option', payload: { option: 'A' } },
      { type: 'choose_option', payload: { option: 'B' } },
    ]);
    ...
    expect(
      ...
      ok: true,
      option: 'B',
      action: { type: 'choose_option', payload: { option: 'B' } },
    });
    ...
    ).toEqual({ ok: false, reason: 'invalid_option', option: 'C' });
  });
  ...
  it('handles number pending choices from min/max range', () => {
    ...
    expect(getPendingNumberChoiceActionsForPlayer(pending, 5)).toEqual([
      { type: 'choose_number', payload: { value: 2 } },
      { type: 'choose_number', payload: { value: 3 } },
      { type: 'choose_number', payload: { value: 4 } },
    ]);
    ...
    expect(
      ...
      ok: true,
      value: 3,
      action: { type: 'choose_number', payload: { value: 3 } },
    });
    ...
    ).toEqual({ ok: false, reason: 'invalid_value', value: 1 });
  });
  ...
  it('handles number pending choices from explicit values list', () => {
    ...
    expect(
      getPendingNumberSetChoiceActionsForPlayer(pending, 1, {
      ...
      { type: 'choose_next_player', payload: { playerId: 3 } },
      { type: 'choose_next_player', payload: { playerId: 5 } },
    ]);
    ...
    expect(
      ...
      ok: true,
      value: 5,
      action: { type: 'choose_next_player', payload: { playerId: 5 } },
    });
    ...
    ).toEqual({ ok: false, reason: 'invalid_value', value: 7 });
  });
  ...
  it('handles pending choose_card actions and validation', () => {
    const pending = {
      ...
      data: {
        cards: [
          ...
          { cardType: 'malus', cardId: 22 },
        ],
      },
    };

    expect(getPendingCardChoiceActionsForPlayer(pending, 4)).toEqual([
      { type: 'choose_card', payload: { cardType: 'bonus', cardId: 10 } },
      { type: 'choose_card', payload: { cardType: 'malus', cardId: 22 } },
    ]);
    ...
    expect(
      validatePendingCardChoiceActionForActor({
        ...
        actorId: 4,
        actionType: 'choose_card',
        payload: { cardType: 'bonus', cardId: 99 },
      }),
    ).toEqual({ ok: false, reason: 'invalid_card' });
  });
});
