...
import type {
  GameDefinition,
  GameRulesAdapter,
} from '../../engine/interfaces/game-rules-adapter.interface';
...
jest.mock('../../engine/services/game-registry.service', () => {
  ...
  return { GameRegistryService: GameRegistryServiceMock };
});

jest.mock('../../engine/game-registry.module', () => {
  const { Module } = jest.requireActual('@nestjs/common');
  const { GameRegistryService } = jest.requireMock(
    '../../engine/services/game-registry.service',
  );
  ...
  return { GameRegistryModule: GameRegistryModuleMock };
});
              ...
              for (const candidateState of stateVariants) {
                for (
                  ...
                  const actionWithMeta = {
                    ...action,
                    meta: {
                      ...asRecord((action as Record<string, unknown>).meta),
                    ...
                    for (const actorVariant of [
                      ...
                      try {
                        ...
                        attempts.unshift({
                          ...
                          meta: {
                            ...asRecord(
                              (validated as Record<string, unknown>).meta,
                            ),
                            actorId: candidate.actorId,
                          },
                        } as GameSingleActionDto);
