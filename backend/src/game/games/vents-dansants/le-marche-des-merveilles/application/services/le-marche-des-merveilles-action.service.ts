...
import {
  describeLeMarcheGoodLabel,
  determineLeMarcheWinner,
} from './le-marche-des-merveilles-action.utils';
...
export class LeMarcheDesMerveillesActionService {
  ...
  private handleBuy(
    ...
    const label = describeLeMarcheGoodLabel(good);
  ...
  private finishGame(state: GameStateEntity): GameStateEntity {
    const meta = this.getMeta(state);
    const winnerId = determineLeMarcheWinner(meta);
    const next = {
      ...state,
      status: 'finished',
