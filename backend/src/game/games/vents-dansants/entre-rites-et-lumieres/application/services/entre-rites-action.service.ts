...
import { ENTRE_RITES_TOTAL_FAMILIES } from '../../model/entre-rites-state.model';
import {
  addEntreRitesCardToDiscard,
  addEntreRitesCardToHand,
  drawEntreRitesOneCard,
  recordEntreRitesSpecial,
  removeEntreRitesCardFromHand,
} from './entre-rites-action.utils';
...
export class EntreRitesActionService {
  ...
  private transferCard(
    ...
    meta = removeEntreRitesCardFromHand(meta, fromId, cardId);
    meta = addEntreRitesCardToHand(meta, toId, cardId);
  ...
  private handleDrawnCard(
    ...
    if (card.type === 'family') {
      ...
      const meta = addEntreRitesCardToHand(this.getMeta(next), playerId, card.id);
  ...
  private effectCollectFromOthers(
    ...
    for (const player of players) {
      ...
      let updatedMeta = removeEntreRitesCardFromHand(meta, opponentId, cardId);
      updatedMeta = addEntreRitesCardToHand(updatedMeta, playerId, cardId);
  ...
  private effectRevealAndSteal(
    ...
    let updatedMeta = removeEntreRitesCardFromHand(
      this.getMeta(next),
      targetId,
      cardId,
    );
  ...
  private discardOneCard(
    ...
    const cardId = hand.shift()!;
    let updatedMeta = removeEntreRitesCardFromHand(meta, playerId, cardId);
    updatedMeta = addEntreRitesCardToDiscard(updatedMeta, cardId);
    const next = this.setMeta(state, updatedMeta);
    return this.core.appendLog(
      next,
  ...
  private discardCard(
    ...
    const meta = this.getMeta(state);
    const updatedMeta = addEntreRitesCardToDiscard(
      removeEntreRitesCardFromHand(meta, playerId, cardId),
      cardId,
    );
    const next = this.setMeta(state, updatedMeta);
    return this.core.appendLog(
  ...
  private recordSpecial(
    ...
    return this.setMeta(
      state,
      recordEntreRitesSpecial(this.getMeta(state), playerId, cardId),
    );
  }
  ...
  private drawOneCard(meta: EntreRitesMetadata): {
    ...
    return drawEntreRitesOneCard(this.deckPolicies, meta);
  }
