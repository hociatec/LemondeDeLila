...
export class GerardPresidentActionService {
  ...
  private handlePlayName(
    ...
    const nextState = this.appendLog(
      ...
      `Le joueur ${this.formatPlayer(playerId)} a joué ${removed.join(', ')}.`,
    );
  ...
  private handleChooseWinner(
    ...
    nextState = this.appendLog(
      nextState,
      `${this.formatPlayer(winnerId)} remporte la manche et atteint ${winnerScore} point(s).`,
    );
  ...
  private handlePass(state: GameStateEntity): GameStateEntity {
    ...
    const nextState = this.appendLog(
      this.setCurrentPlayer(state, players, nextId),
      `${this.formatPlayer(playerId)} passe son tour.`,
    );
  ...
  private cloneMetadata(state: GameStateEntity): GerardPresidentMetadata {
  ...
  private formatPlayer(playerId: number | null): string {
  ...
  private filterPlayableNames(names: string[]): string[] {
    const unique: string[] = [];
    names.forEach((name) => {
