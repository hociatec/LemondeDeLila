...
export class BandeABananeActionService {
  ...
  private playTrap(
    ...
    if (card.trap === 'tigre-rodeur') {
      ...
      next = this.core.appendLog(
        next,
        `${this.playerName(next, playerId)} chute sur un tigre et lâche une carte.`,
      );
      return next;
    }
    return state;
  ...
  private playVol(
    ...
    next = this.core.appendLog(
      next,
      `${this.playerName(next, playerId)} vole ${this.getCardName(stolen)} à ${
  ...
  private playCris(
    ...
    if (originalTargetHand.length) {
      ...
      next = this.core.appendLog(
        next,
        `${this.playerName(next, playerId)} échange ${this.getCardName(
          returned,
    ...
    next = this.core.appendLog(
      next,
      `${this.playerName(next, playerId)} donne une carte à ${
        this.playerName(next, targetId)
      }.`,
    );
    return next;
  ...
  private addCardToTroop(
    ...
    if (this.hasWinningTroupe(this.getMeta(next), playerId)) {
      next = this.core.appendLog(
        next,
        `${this.playerName(next, playerId)} crie « BANAAAANE ! » et devient le chef de la Bande à Banane !`,
      );
      next = {
        ...next,
        status: 'finished',
  ...
  private enforceHandLimit(state: GameStateEntity, playerId: number): GameStateEntity {
    ...
    while (hand.length > BandeABananeActionService.HAND_LIMIT) {
      ...
      next = this.core.appendLog(
        next,
        `${this.playerName(next, playerId)} dépasse 7 cartes et défausse ${
