...
export class DameNatureActionService {
  ...
  private handleAskCard(
    ...
    next = this.core.appendLog(
      next,
      `${this.playerName(next, currentId)} récupère ${this.getCardName(cardId)} de ${this.playerName(next, targetId)}.`,
    );
  ...
  private drawCardForPlayer(state: GameStateEntity, playerId: number): GameStateEntity {
    ...
    if (!cardId) {
      return this.core.appendLog(
        next,
        `${this.playerName(next, playerId)} ne trouve plus aucune carte à piocher.`,
      );
    }
