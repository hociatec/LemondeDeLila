...
export class NawakChallengeService {
  ...
  constructor(private readonly random: RandomService) {
    ...
    if (!this.challenges.length) {
      this.logger.warn(
        'Aucun defi Nawak charge. Ajoutez des donnees dans data/nawak-defis.txt.',
      );
    }
  }
  ...
  loadChallenge(meta: NawakMetadata): {
    ...
    if (!this.challenges.length) {
      throw new GameNotFoundError('Pas de defi disponible pour Nawak !');
    }
    const seed = meta.rng ?? {};
  ...
  private loadChallenges(): NawakChallenge[] {
    ...
    this.logger.error(
      'Impossible de charger les defis Nawak : aucun fichier data/nawak-defis.txt accessible.',
    );
    return [];
  }
