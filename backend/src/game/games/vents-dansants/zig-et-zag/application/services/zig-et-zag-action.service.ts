...
import {
  asZigEtZagRecord,
  normalizeZigEtZagIdList,
  normalizeZigEtZagRoundState,
  pickZigEtZagNextCurrentPlayerId,
} from './zig-et-zag-action.utils';
...
export class ZigEtZagActionService {
  ...
  private handleSelectCard(
    ...
    const cardId = String(asZigEtZagRecord(action.payload).cardId ?? '').trim();
    if (!cardId) {
      return state;
    }
  ...
  private prepareBattle(
    ...
    return {
      ...
      triggerColors,
      triggerFamilies,
      battleLog: [...round.battleLog, 'Bataille d̩clench̩e !'],
    };
  }
  ...
  private appendRevealLogs(
    ...
    for (const play of round.plays ?? []) {
      ...
      if (play.faceUpCard) {
        next = this.core.appendLog(
          next,
          `${resolvePlayerName(players, play.playerId)} d̩voile ${this.formatCardLabel(play.faceUpCard)}.`,
        );
      }
    }
  ...
  private ensureRoundState(
    ...
    const meta = this.getMeta(state);
    if (meta.roundState) {
      const normalized = normalizeZigEtZagRoundState(meta.roundState);
    ...
    const normalized = normalizeZigEtZagRoundState(round);
    const nextState = this.setRoundState(state, meta, normalized);
    return { state: nextState, round: normalized };
  }
  ...
  private playCardWithId(
    ...
    nextState = this.setCurrentPlayerWithAnnouncement(
      nextState,
      players,
      pickZigEtZagNextCurrentPlayerId(nextRound, playerId),
    );
  ...
  private normalizeWaitingPlayers(round: ZigEtZagRoundState): number[] {
    return normalizeZigEtZagIdList(round.waitingPlayers);
  }
  ...
  private getActorId(
    ...
    const actorFromMeta = asZigEtZagRecord(action.meta).actorId;
