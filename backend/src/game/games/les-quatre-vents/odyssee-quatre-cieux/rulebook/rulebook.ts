...
import { ODYSSEE_GAME, type OdysseeActionType } from '../definitions/odyssee.definition';
  ...
  if (pending) {
    ...
    if (pending.type === 'choose_pawn') {
      const moves: Array<{ pawnIndex: number; targetProgress: number }> = Array.isArray(pending?.data?.moves)
        ? pending.data.moves
        : [];
      return moves.map((m) => ({ type: 'move_pawn', payload: { pawnIndex: m.pawnIndex, targetProgress: m.targetProgress } }));
    }
    return [];
  }
...
export function validateAction(state: GameStateEntity, action: GameSingleActionDto, actorId: number | null): GameSingleActionDto {
  ...
  const type = (rawType === 'roll_dice' ? 'ROLL_DICE' : rawType) as OdysseeActionType;
  ...
  if (actorId == null) throw new PlayerActionError('Acteur requis.', { gameType: 'odyssee-quatre-cieux' });
  if (String(state.status ?? '').toLowerCase() !== 'started') {
    throw new PlayerActionError("La partie n'est pas démarrée.", { gameType: 'odyssee-quatre-cieux' });
  }

  const pending = state.pending as any;
  if (pending) {
    if (pending.playerId !== actorId) throw new PlayerActionError("Ce n'est pas votre action.", { gameType: 'odyssee-quatre-cieux' });
    if (pending.type !== 'choose_pawn' || type !== 'move_pawn') {
      throw new PlayerActionError('Action non disponible.', { gameType: 'odyssee-quatre-cieux' });
    }
    ...
    if (!Number.isFinite(pawnIndex) || !Number.isFinite(targetProgress)) {
      throw new GameValidationError('Payload invalide.', { gameType: 'odyssee-quatre-cieux', payload: action.payload });
    }
    const moves: Array<{ pawnIndex: number; targetProgress: number }> = Array.isArray(pending?.data?.moves)
      ? pending.data.moves
      : [];
    if (!moves.some((m) => m.pawnIndex === pawnIndex && m.targetProgress === targetProgress)) {
      throw new GameValidationError('Coup invalide.', { gameType: 'odyssee-quatre-cieux', pawnIndex, targetProgress });
    }
