import { loadGameContent, rejectContent } from '../../../engine/sdk/public-api';
...
export const MON_VILLAGE_CONTENT = loadContent();
...
function loadContent(): GameContent<VillageContent> {
  ...
  return loadGameContent(
    ...
    { board, cards },
    {
      parse: (value) => {
        ...
        return {
          tiles: value.board.tiles.map((tile) => ({ ...tile })),
          zones: value.cards.zones.map((zone) => ({
            id: zone.id,
            title: zone.title,
