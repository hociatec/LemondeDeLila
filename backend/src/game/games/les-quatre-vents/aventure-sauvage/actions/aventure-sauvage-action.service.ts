...
export class AventureSauvageActionService {
  ...
  private applyLanding(state: GameStateEntity, playerId: number, position: number): GameStateEntity {
    let next = state;
  ...
  private drawAndApplyCard(
    ...
    if (card.reroll) {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} rejoue.`);
    }

    return next;
  }

  private moveBy(state: GameStateEntity, playerId: number, delta: number): GameStateEntity {
  ...
  private addSkipTurns(state: GameStateEntity, playerId: number, turns: number): GameStateEntity {
