...
import type { CaMetadata, CaTile } from '../model/ca.types';
...
export class CaSetupService {
  hydrateInitialState(base: GameStateEntity): GameStateEntity {
    ...
    const meta: CaMetadata = {
      ...
      lastMoveDelta,
      turnsSinceMoved,
      statuses: { skipTurn: {}, ignoreNextPenalty: {}, doubleNextMove: {}, doubleNextRoll: {} },
...
function buildDeck(): any[] {
  // Deck fidèle au fichier (80), mais effets appliqués dans le service via (id, kind).
  // Avances simples (1-12)
  const cards: any[] = [];
  const push = (id: number, title: string, category: string, kind: string, text: string, moveDelta?: number, keepTurn?: boolean) =>
  ...
  simple.forEach(([t, d], i) => push(i + 1, t, 'avances_simples', 'move', `Avance de ${d} case(s).`, d));
  ...
  spectacular.forEach(([t, d], i) => push(13 + i, t, 'avances_spectaculaires', 'move', `Avance de ${d} case(s).`, d));
  ...
  losses.forEach(([t, k, d], i) => {
    ...
    else push(id, t, 'reculs', 'move', d === 0 ? "Pas de déplacement ce tour-ci." : `Recule de ${Math.abs(d)} case(s).`, d as any);
  });

  const special = [
    ['Raccourci secret', 'rule', 'Tu prends la première place.'],
    ['Chemin des cactus sympathiques', 'rule', 'Avance de 4 cases et ignore la prochaine pénalité.'],
    ['Saute-mouton', 'rule', 'Tu dépasses d’1 place le joueur devant toi.'],
    ['Saut quantique non prévu', 'rule', 'Avance jusqu’à la prochaine case multiple de 5.'],
    ['Chemin de traverse', 'rule', 'Avance de 3 cases et rejoue.'],
    ['Trompe-l’œil du décor', 'swap', 'Avance de 2 cases et échange ta position avec un joueur.'],
    ['Trappe bienveillante', 'move', 'Avance de 5 cases.'],
    ['Virage à 90° trop efficace', 'move', 'Avance de 3 cases.'],
  ] as const;
  special.forEach(([t, kind, text], i) => push(33 + i, t, 'special', kind, text as any));
  ...
  const shared = [
    ...
    ['Pause hydratation collective', 'global', 'Personne ne se déplace ce tour-ci.'],
    ...
    ['Brouhaha inexplicable', 'global', 'Chacun échange avec celui juste derrière lui.'],
    ['La danse des mollets heureux', 'global', 'Tous rejouent immédiatement.'],
  ] as const;
  shared.forEach(([t, kind, text], i) => push(41 + i, t, 'chaos', kind, text as any));
  ...
  conditional.forEach((text, i) => push(51 + i, `Condition ${i + 1}`, 'conditionnel', 'conditional', text));
  ...
  rules.forEach((text, i) => push(61 + i, `Règle idiote ${i + 1}`, 'regle', 'rule', text));
  ...
  ambience.forEach((text, i) => push(71 + i, `Ambiance ${i + 1}`, 'ambiance', 'neutral', text));
