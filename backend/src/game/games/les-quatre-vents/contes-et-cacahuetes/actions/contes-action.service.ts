...
export class ContesActionService {
  ...
  private handleRoll(state: GameStateEntity): GameStateEntity {
    ...
    const rollOut = forced > 0 ? { roll: 1, meta: meta as any } : this.random.rollDice(meta as any, 6);
    next = { ...next, metadata: { ...(next.metadata ?? {}), ...rollOut.meta }, lastRoll: rollOut.roll };

    if (forced > 0) {
      next = this.setStatusCount(next, 'forcedRollOneTurns', currentId, forced - 1);
    }

    next = this.core.appendLog(next, `${this.playerName(next, currentId)} lance le dé : \"${rollOut.roll}\".`);
  ...
  private handleRerollDecision(state: GameStateEntity, reroll: boolean): GameStateEntity {
    ...
    if (reroll) {
      ...
      next = { ...next, metadata: { ...(next.metadata ?? {}), ...out.meta }, lastRoll: out.roll };
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} relance le dé : \"${out.roll}\".`);
      next = this.applyMoveFromRoll(next, playerId, out.roll, 0);
    } else {
      const roll = Number(pending.data.baseRoll);
      next = { ...next, lastRoll: roll };
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} garde le résultat \"${roll}\".`);
  ...
  private handleChooseTarget(state: GameStateEntity, action: GameSingleActionDto): GameStateEntity {
    ...
    if (ctx === 'move_other_2') {
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} fait avancer ${this.playerName(next, targetPlayerId)} de 2 cases.`);
    ...
    if (ctx === 'grimoire_voyageur') {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, targetPlayerId)} avance d’1 case.`);
  ...
  private handleChooseNumber(state: GameStateEntity, action: GameSingleActionDto): GameStateEntity {
    const pending = state.pending as any as ContesPending;
  ...
  private handleChooseOption(state: GameStateEntity, action: GameSingleActionDto): GameStateEntity {
    const pending = state.pending as any as ContesPending;
  ...
  private handleChooseCard(state: GameStateEntity, action: GameSingleActionDto): GameStateEntity {
    const pending = state.pending as any as ContesPending;
    ...
    const pick = pending.data.cards.find((c) => c.cardType === (cardType as any) && c.cardId === cardId);
    ...
    if (ctx.startsWith('abondance_keep_one:')) {
      next = this.core.appendLog(next, `Corne d’abondance : ${this.playerName(next, playerId)} garde \"${pick.title}\".`);
  ...
  private applyMoveFromRoll(state: GameStateEntity, playerId: number, roll: number, depth: number): GameStateEntity {
    let next = state;
    const meta = this.getMeta(next);
    ...
    if (roll === 1 && replace && this.canUseBonusCards(next, playerId)) {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} utilise Feuille magique : 1 devient 4.`);
    }
  ...
  private moveBy(state: GameStateEntity, playerId: number, delta: number, depth: number): GameStateEntity {
    ...
    let next: GameStateEntity = {
      ...state,
      metadata: { ...(state.metadata ?? {}), ...meta, positions: { ...(meta.positions ?? {}), [playerId]: nextPos } },
    };

    next = this.onAnyPlayerPassedBlocked(next, playerId, nextPos);

    const tile = (this.getMeta(next).tiles ?? [])[nextPos] as ContesCacahuetesTile | undefined;
    next = this.core.appendLog(next, `${this.playerName(next, playerId)} arrive sur ${tile?.label ?? `Case ${nextPos + 1}`}.`);

    if (raw >= finishIndex) {
      next = this.setWinner(next, playerId);
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} remporte la partie !`);
  ...
  private applyTileEffect(state: GameStateEntity, playerId: number, tile: ContesCacahuetesTile, depth: number): GameStateEntity {
  ...
  private applyConteTile(state: GameStateEntity, playerId: number, depth: number): GameStateEntity {
    const meta = this.getMeta(state);
    ...
    if (ignore && this.canUseBonusCards(state, playerId)) {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} ignore l’effet Conte (Cape d’invisibilité) et avance d’1 case.`);
  ...
  private drawAndApply(
    ...
    if (card.type === 'bonus') return this.applyBonusEffectById(next, playerId, card.id, depth);
    if (card.type === 'malus') return this.applyMalusEffectById(next, playerId, card.id, depth);
    if (card.type === 'surprise') return this.applySurpriseEffectById(next, playerId, card.id, depth);
    return next;
  }

  // --- Effects + helpers (added in next patches) ---

  private applyBonusEffectById(state: GameStateEntity, playerId: number, id: number, depth: number): GameStateEntity {
    let next = state;
    switch (id) {
      case 1:
      ...
      case 2:
        ...
        return this.core.appendLog(next, `${this.playerName(next, playerId)} gagne un parchemin enchanté (1 relance).`);
      case 3:
        next = this.addStatusCount(next, 'shieldMalus', playerId, 1);
        return this.core.appendLog(next, `${this.playerName(next, playerId)} obtient une Amulette protectrice (1 protection).`);
      case 4:
        next = this.setStatusBool(next, 'ignoreNextConteAndAdvance', playerId, true);
        return this.core.appendLog(next, `${this.playerName(next, playerId)} obtient une Cape d’invisibilité (prochaine case Conte ignorée).`);
      case 5:
      ...
      case 7:
        ...
        return this.core.appendLog(next, `${this.playerName(next, playerId)} obtient la Clé d’or (sur Conte : Bonus/Malus pour un autre joueur).`);
      case 8:
      ...
      case 13:
        next = this.core.appendLog(next, `${this.playerName(next, playerId)} avance de 5 cases mais passera son prochain tour.`);
      ...
      case 14:
        ...
        return this.core.appendLog(next, `${this.playerName(next, playerId)} pose Feuille magique (1 devient 4 une fois).`);
      case 15:
  ...
  private applyMalusEffectById(state: GameStateEntity, playerId: number, id: number, depth: number): GameStateEntity {
    let next = state;
    switch (id) {
      case 1:
        return this.addStatusCount(next, 'skipTurn', playerId, 1);
      ...
      case 15:
        ...
        return this.core.appendLog(next, `${this.playerName(next, playerId)} ne peut plus utiliser de cartes Bonus pendant 2 tours.`);
      default:
        return next;
    }
  }

  private applySurpriseEffectById(state: GameStateEntity, playerId: number, id: number, depth: number): GameStateEntity {
    let next = state;
    switch (id) {
      case 1:
      ...
      case 8:
        ...
        return this.core.appendLog(next, `${this.playerName(next, playerId)} lira à l’envers : prochain tour en reculant.`);
      case 9:
        return this.setPending(next, {
          type: 'choose_option',
      ...
      case 10:
        ...
        return this.core.appendLog(next, `${this.playerName(next, playerId)} est protégé(e) de la prochaine carte Malus.`);
      case 11:
  ...
  private applyAbondance(state: GameStateEntity, playerId: number): GameStateEntity {
    let next = state;
    ...
    if (cards.length === 1) return this.applyBonusEffectById(next, playerId, cards[0].id, 0);
    ...
    return this.setPending(next, {
      ...
      data: {
        ...
        cards: cards.map((c) => ({ cardType: 'bonus', cardId: c.id, title: c.title })),
      },
    });
  }

  private applyCoffreMerveilles(state: GameStateEntity, playerId: number, depth: number): GameStateEntity {
    let next = state;
    ...
    const t = (v: number) => (v === 0 ? 'bonus' : v === 1 ? 'malus' : 'surprise');
  ...
  private drawCard(
    ...
    return { state: { ...state, metadata: { ...(state.metadata ?? {}), ...nextMeta } }, card };
  }

  private maybeProtectFromMalus(state: GameStateEntity, playerId: number): { protected: boolean; state: GameStateEntity } {
    let next = state;
    const meta = this.getMeta(next);
  ...
  private startGiveBonusChoice(state: GameStateEntity, giverId: number, targetId: number): GameStateEntity {
    ...
    return this.setPending(state, {
      ...
      data: {
        ...
        cards: tokens.map((t) => ({ cardType: 'bonus', cardId: t.cardId, title: t.title })),
      },
    });
  }

  private listBonusTokens(meta: ContesCacahuetesMetadata, playerId: number): Array<{ cardId: number; title: string }> {
    const out: Array<{ cardId: number; title: string }> = [];
    const shield = Number(meta.statuses.shieldMalus?.[playerId] ?? 0);
    if (shield > 0) out.push({ cardId: 3, title: `Amulette protectrice (${shield})` });
    if (Boolean(meta.statuses.ignoreNextConteAndAdvance?.[playerId])) out.push({ cardId: 4, title: 'Cape d’invisibilité' });
    if (Boolean(meta.statuses.keyOfGold?.[playerId])) out.push({ cardId: 7, title: 'Clé d’or universelle' });
    if (Boolean(meta.statuses.replaceOneOn1By4?.[playerId])) out.push({ cardId: 14, title: 'Feuille magique' });
  ...
  private transferBonusToken(state: GameStateEntity, fromId: number, toId: number, bonusId: number): GameStateEntity {
    ...
    if (bonusId === 3) {
      ...
      return this.core.appendLog(next, `${this.playerName(next, fromId)} donne une Amulette protectrice à ${this.playerName(next, toId)}.`);
    }
    if (bonusId === 4) {
      if (!Boolean(meta.statuses.ignoreNextConteAndAdvance?.[fromId])) return next;
      ...
      return this.core.appendLog(next, `${this.playerName(next, fromId)} donne une Cape d’invisibilité à ${this.playerName(next, toId)}.`);
    }
    if (bonusId === 7) {
      if (!Boolean(meta.statuses.keyOfGold?.[fromId])) return next;
      next = this.setStatusBool(next, 'keyOfGold', fromId, false);
      next = this.setStatusBool(next, 'keyOfGold', toId, true);
      return this.core.appendLog(next, `${this.playerName(next, fromId)} donne la Clé d’or à ${this.playerName(next, toId)}.`);
    }
    if (bonusId === 14) {
      if (!Boolean(meta.statuses.replaceOneOn1By4?.[fromId])) return next;
      next = this.setStatusBool(next, 'replaceOneOn1By4', fromId, false);
      next = this.setStatusBool(next, 'replaceOneOn1By4', toId, true);
      return this.core.appendLog(next, `${this.playerName(next, fromId)} donne Feuille magique à ${this.playerName(next, toId)}.`);
    }
    if (bonusId === 2) {
      ...
      return this.core.appendLog(next, `${this.playerName(next, fromId)} donne un Parchemin enchanté à ${this.playerName(next, toId)}.`);
    }
    return next;
  }
  ...
  private swapPositions(state: GameStateEntity, aId: number, bId: number): GameStateEntity {
    const meta = this.getMeta(state);
    ...
    positions[aId] = b;
    positions[bId] = a;
    let next: GameStateEntity = { ...state, metadata: { ...(state.metadata ?? {}), ...meta, positions } };
    next = this.core.appendLog(next, `${this.playerName(next, aId)} échange sa position avec ${this.playerName(next, bId)}.`);
    return next;
  }

  private setTurnSwap(state: GameStateEntity, aId: number, bId: number): GameStateEntity {
    let next = state;
  ...
  private onAnyPlayerPassedBlocked(state: GameStateEntity, moverId: number, moverPos: number): GameStateEntity {
    ...
    for (const pid of toClear) {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, pid)} n’est plus bloqué(e).`);
    }
    return {
      ...next,
      metadata: { ...(next.metadata ?? {}), ...meta, statuses: { ...meta.statuses, blockedUntilPassed: blocked } },
    };
  }
  ...
  private applyTurnSwapIfNeeded(state: GameStateEntity): GameStateEntity {
    ...
    next = { ...next, turn: { ...(next.turn ?? { direction: 1 }), currentPlayerId: swapWith } };
  ...
  private swapWithClosestBehind(state: GameStateEntity, playerId: number): GameStateEntity {
    const meta = this.getMeta(state);
  ...
  private blockUntilPassed(state: GameStateEntity, playerId: number): GameStateEntity {
    const meta = this.getMeta(state);
    const pos = meta.positions?.[playerId] ?? 0;
    let next = state;
    next = this.setStatusCount(next, 'blockedUntilPassed', playerId, pos);
    next = this.setStatusCount(next, 'skipTurn', playerId, 999);
    return this.core.appendLog(next, `${this.playerName(next, playerId)} est bloqué(e) jusqu’à ce qu’un autre joueur atteigne ou dépasse sa case.`);
  }

  private goToPreviousMalusAndApply(state: GameStateEntity, playerId: number, depth: number): GameStateEntity {
    const meta = this.getMeta(state);
    const pos = meta.positions?.[playerId] ?? 0;
  ...
  private teleport(state: GameStateEntity, playerId: number, pos: number): GameStateEntity {
    const meta = this.getMeta(state);
  ...
  private autoSkipIfBlocked(state: GameStateEntity, currentId: number): GameStateEntity {
    const meta = this.getMeta(state);
  ...
  private setPending(state: GameStateEntity, pending: Exclude<ContesPending, null>): GameStateEntity {
    return { ...state, pending };
  }

  private setStatusCount(state: GameStateEntity, key: string, playerId: number, value: number): GameStateEntity {
    const meta = this.getMeta(state) as any;
    const statuses = meta.statuses ?? {};
    const map = { ...(statuses[key] ?? {}) };
    if (!value) delete map[playerId];
    else map[playerId] = value;
    return { ...state, metadata: { ...(state.metadata ?? {}), ...meta, statuses: { ...statuses, [key]: map } } };
  }

  private setStatusBool(state: GameStateEntity, key: string, playerId: number, value: boolean): GameStateEntity {
    const meta = this.getMeta(state) as any;
    const statuses = meta.statuses ?? {};
    const map = { ...(statuses[key] ?? {}) };
    if (!value) delete map[playerId];
  ...
  private addStatusCount(state: GameStateEntity, key: string, playerId: number, delta: number): GameStateEntity {
    const meta = this.getMeta(state) as any;
    const statuses = meta.statuses ?? {};
    const map = { ...(statuses[key] ?? {}) };
