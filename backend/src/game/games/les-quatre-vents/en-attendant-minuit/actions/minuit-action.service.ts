...
import type { MinuitCard, MinuitMetadata, MinuitPendingQuiz, MinuitTile } from '../model/minuit.types';
...
export class MinuitActionService {
  ...
  private handleAnswerQuiz(state: GameStateEntity, action: GameSingleActionDto): GameStateEntity {
    ...
    if (correct) {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, currentId)} réussit le quiz.`);
      if (delta > 0) {
        next = this.move(next, currentId, delta);
      }
    } else {
      next = this.core.appendLog(next, `${this.playerName(next, currentId)} rate le quiz.`);
    }
  ...
  private handleChooseTarget(state: GameStateEntity, action: GameSingleActionDto): GameStateEntity {
    ...
    if (ctx.kind === 'swap') {
      meta = { ...meta, positions: { ...(meta.positions ?? {}), [currentId]: targetPos, [targetPlayerId]: actorPos } };
    ...
    if (ctx.kind === 'gift') {
      let next: GameStateEntity = { ...state, pending: null, metadata: { ...(state.metadata ?? {}), ...meta, pendingContext: null } };
      next = this.core.appendLog(next, `${this.playerName(next, currentId)} offre un cadeau à ${this.playerName(next, targetPlayerId)}.`);
  ...
  private applyLanding(state: GameStateEntity, playerId: number): GameStateEntity {
    ...
    if (occupant != null) {
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} arrive sur une case occupée : recul d'une case.`);
    ...
    if (afterPos === 55) {
      ...
      next = this.core.appendLog(next, `${this.playerName(next, playerId)} atteint Minuit !`);
    ...
    if (tile.type === 'move') {
      ...
      if (ignore && delta < 0) {
        meta = { ...meta, statuses: { ...meta.statuses, ignoreNextMalus: { ...(meta.statuses.ignoreNextMalus ?? {}), [playerId]: false } } };
        next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
        return this.core.appendLog(next, 'Malus ignoré.');
      }
      if (delta !== 0) {
        next = this.core.appendLog(next, `${tile.title} : déplacement ${delta}.`);
    ...
    if (tile.type === 'skip') {
      ...
      if (ignore) {
        meta = { ...meta, statuses: { ...meta.statuses, ignoreNextSkip: { ...(meta.statuses.ignoreNextSkip ?? {}), [playerId]: false } } };
        next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
      ...
      next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
      return this.core.appendLog(next, `${tile.title} : ${this.playerName(next, playerId)} passe ${turns} tour(s).`);
    }
  ...
  private applyCard(state: GameStateEntity, playerId: number, card: MinuitCard): GameStateEntity {
    let next = state;
    let meta = this.getMeta(next);
    ...
    if (/Ignorez la prochaine case malus/i.test(text)) {
      meta = { ...meta, statuses: { ...meta.statuses, ignoreNextMalus: { ...(meta.statuses.ignoreNextMalus ?? {}), [playerId]: true } } };
      next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
      return this.core.appendLog(next, 'Protection malus activée.');
    }

    if (/Ignorez la prochaine case.*Passe ton tour/i.test(text)) {
      meta = { ...meta, statuses: { ...meta.statuses, ignoreNextSkip: { ...(meta.statuses.ignoreNextSkip ?? {}), [playerId]: true } } };
      next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
    ...
    if (skip > 0) {
      ...
      next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
      return this.core.appendLog(next, `${this.playerName(next, playerId)} passe ${skip} tour(s).`);
    }

    if (/jusqu['’]à la prochaine Carte Noël/i.test(text)) {
      const nextPos = findNext(meta.tiles, meta.positions[playerId] ?? 0, (t) => t.type === 'card');
      if (nextPos != null) {
        next = this.core.appendLog(next, "Avance jusqu'à la prochaine Carte Noël.");
    ...
    if (/position avec le joueur juste derrière/i.test(text)) {
      ...
      if (behind != null) {
        ...
        meta = { ...meta, positions: { ...meta.positions, [playerId]: behindPos, [behind]: actorPos } };
        next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
        next = this.core.appendLog(next, `${this.playerName(next, playerId)} échange sa position avec ${this.playerName(next, behind)}.`);
        return next;
      }
    }
  ...
  private setPos(state: GameStateEntity, playerId: number, pos: number): GameStateEntity {
    ...
    const nextMeta: MinuitMetadata = { ...meta, positions: { ...(meta.positions ?? {}), [playerId]: nextPos } };
    return { ...state, metadata: { ...(state.metadata ?? {}), ...nextMeta } };
  }

  private drawCard(meta: MinuitMetadata): { card: MinuitCard | null; meta: MinuitMetadata } {
    ...
    if (!deck.length && discard.length) {
      ...
      const reshuffled: MinuitMetadata = { ...meta, ...(shuffled.meta as any), decks: { cards: shuffled.values as any, discard: [] } };
  ...
  private parseQuizCard(playerId: number, card: MinuitCard): MinuitPendingQuiz | null {
    ...
    const anyCorrect = lines.some((l) => /Les trois réponses sont juste/i.test(l));
  ...
  private findOccupant(meta: MinuitMetadata, me: number, pos: number): number | null {
...
function extractMoveDelta(text: string): number {
  ...
  const forward = text.match(/avancez\s+de\s+([0-9]+|un|une|deux|trois|quatre|cinq|six)\s+case/i);
  if (forward) return parse(forward[1]);
  const backward = text.match(/reculez\s+de\s+([0-9]+|un|une|deux|trois|quatre|cinq|six)\s+case/i);
...
function findNext<T>(items: T[], start: number, predicate: (v: any) => boolean): number | null {
...
function findPrev<T>(items: T[], start: number, predicate: (v: any) => boolean): number | null {
...
function findBehind(positions: Record<number, number>, playerId: number): number | null {
  const entries = Object.entries(positions).map(([id, pos]) => ({ id: Number(id), pos: Number(pos) }));
