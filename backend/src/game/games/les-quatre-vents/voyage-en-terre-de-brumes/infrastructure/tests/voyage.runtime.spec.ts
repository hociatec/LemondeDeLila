...
import { createVoyageRuntime } from '../../voyage.runtime';

describe('Voyage runtime', () => {
  it('hydrates initial state without Nest module', () => {
    const { service } = createVoyageRuntime({
      contentLoader: {
        validators: {
          version: () => () => undefined,
        ...
        loadContent: ({ filename }: { filename: string }) => {
          ...
          return { version: 1, cards: [{ id: `${filename}-1`, title: filename }] };
        },
      },
      random: {
