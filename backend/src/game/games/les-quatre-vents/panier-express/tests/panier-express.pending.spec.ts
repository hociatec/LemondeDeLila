...
describe('PanierExpress pending scenarios', () => {
  ...
  it('tirage chanceux: consomme 3 cartes du deck bonus et expose offered', () => {
    ...
    (state.metadata.decks as any).events = { deck: ['tirage-chanceux'], discards: [] };
    (state.metadata.decks as any)['courses-bonus'] = {
    ...
    expect((after.pending as any)?.data?.kind).toBe('event.tirage_chanceux');
    expect((after.pending as any)?.choices).toEqual(['amande', 'noix', 'pomme']);
    expect((after.pending as any)?.data?.offered).toEqual(['amande', 'noix', 'pomme']);
    expect((after.metadata as any)?.decks?.['courses-bonus']?.deck).toEqual(['banane']);
  });
  ...
  it('bot: résout un tirage chanceux en 1 pick_choice (anti-boucle)', () => {
    const state = makeStartedState(
      game,
      [
        { id: 1, username: 'Nuggets', isBot: true, inventory: [], basket: [], shoppingList: [] },
        { id: 2, username: 'Humain', inventory: [], basket: [], shoppingList: [] },
      ],
      1,
    );

    (state.metadata.decks as any).events = { deck: ['tirage-chanceux'], discards: [] };
    (state.metadata.decks as any)['courses-bonus'] = { deck: ['amande', 'noix', 'pomme'], discards: [] };
  ...
  it('producteur-genereux: 2 étapes (choix carte -> choix joueur)', () => {
    const state = makeStartedState(
      game,
      [
        { id: 1, username: 'A', inventory: ['amande'], basket: [], shoppingList: [] },
        { id: 2, username: 'B', inventory: [], basket: [], shoppingList: [] },
      ],
      1,
    );
    state.pending = {
      ...
      label: 'x',
      choices: ['amande'],
      data: { kind: 'event.producteur_genereux.choose_card', cards: ['amande'], targets: [{ playerId: 2, username: 'B' }] },
    };
    ...
    expect((step1.pending as any)?.data?.kind).toBe('event.producteur_genereux.choose_target');
  ...
  it('panier-bonus: vole une carte unique au joueur cible', () => {
    const state = makeStartedState(
      ...
      [
        { id: 1, username: 'A', inventory: [], basket: [], shoppingList: [] },
        { id: 2, username: 'B', inventory: ['amande'], basket: [], shoppingList: [] },
      ],
      1,
    );
    state.pending = {
      ...
      label: 'x',
      choices: ['B'],
      data: { kind: 'event.panier_bonus.choose_target', targets: [{ playerId: 2, username: 'B' }] },
    };
  ...
  it('échange spontané: choix cible -> choix carte -> échange', () => {
    const state = makeStartedState(
      ...
      [
        { id: 1, username: 'A', inventory: ['amande'], basket: [], shoppingList: [] },
        { id: 2, username: 'B', inventory: ['noix'], basket: [], shoppingList: [] },
      ],
      1,
    );
    state.pending = {
      ...
      label: 'x',
      choices: ['B'],
      data: { kind: 'event.echange_spontane.choose_target', targets: [{ playerId: 2, username: 'B' }] },
    };

    const step1 = game.applyActions(state, [
      { type: 'pick_choice', payload: { index: 0 }, meta: { actorId: 1 } } as any,
    ]);
    expect((step1.pending as any)?.data?.kind).toBe('event.echange_spontane.choose_give');
  ...
  it('conseil de voisinage: prend une carte utile et donne une carte en retour', () => {
    const state = makeStartedState(
      ...
      [
        ...
        { id: 1, username: 'A', inventory: ['noix'], basket: [], shoppingList: ['amande'] },
        { id: 2, username: 'B', inventory: ['amande'], basket: [], shoppingList: [] },
      ],
      1,
    );
    state.pending = {
      ...
      label: 'x',
      choices: ['B: amande'],
      data: { kind: 'event.conseil_voisinage.pick', candidates: [{ targetPlayerId: 2, card: 'amande', label: 'B: amande' }] },
    };
