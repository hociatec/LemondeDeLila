...
describe('PanierExpressService', () => {
  ...
  it("l'échange imposé ne rend pas aléatoirement la carte donnée au joueur ciblé", () => {
    const state: any = service.hydrateInitialState({
      ...
      pending: {
        ...
        blocking: true,
        label:
          'Choisissez une carte à donner à Azrael, puis Entrée.',
        choices: ['nashi'],
        data: {
          kind: 'exchange.impose.choose_card',
          initiatorId: 2,
  ...
  it("évite le doublon de libellé sur l'erreur de livraison", () => {
    ...
    expect(
      logs.some((message: string) =>
        message.includes(
          'Erreur de livraison : A défausse',
        ),
      ),
    ).toBe(false);
  });
  ...
  it("journalise l'intempérie avant les déplacements des joueurs", () => {
    ...
    const weatherIndex = logs.findIndex(
      (message) => message.includes('Intempérie') && message.includes('recul'),
    );
