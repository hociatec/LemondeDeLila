...
type PanierTurnFlowEntry = {
  id?: number | null;
  remainingAfter?: number | null;
};

type PanierTurnFlowState = {
  skipped?: PanierTurnFlowEntry[];
};
  ...
  advanceTurn(state: GameStateEntity): GameStateEntity {
    ...
    const turnFlow =
      meta && typeof meta === 'object'
        ? ((meta as PanierExpressMetadata & { turnFlow?: PanierTurnFlowState })
            .turnFlow ?? null)
        : null;
    ...
    playingLog('panier.advanceTurn', {
      ...
      skipTurn: this.getMetadata(withMeta).statuses?.skipTurn ?? {},
    });
