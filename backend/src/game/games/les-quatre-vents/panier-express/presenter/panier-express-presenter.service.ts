    ...
    if (params.rawPending && params.rawPending.type === 'exchange') {
      ...
      if (exchangePending.step === 'choose_target') {
        ...
        return {
          ...
          blocking: true,
          question: "Choisir un joueur pour l'�change.",
          choices,
      ...
      if (exchangePending.step === 'choose_give') {
        ...
        return {
          ...
          question: targetUsername
            ? `Choisir une carte � donner � ${targetUsername}.`
            : 'Choisir une carte � donner.',
          choices,
          data: {
            step: 'choose_give',
      ...
      if (exchangePending.step === 'confirm') {
        ...
        const take =
          exchangePending.take != null
            ...
            : '';
        const question = take
          ? `${initiator} vous propose un �change : il vous donne "${give}" et vous lui donnez "${take}". (A = accepter, R = refuser)`
          : `${initiator} vous propose un �change : il vous donne "${give}". (A = accepter, R = refuser)`;
        return {
          type: 'exchange',
          playerId: exchangePending.playerId,
          blocking: true,
