...
import {
  describeToutPresDeMamanPawnLabel,
  getToutPresDeMamanPlayerPosition,
  getToutPresDeMamanTileByIndex,
} from './tout-pres-de-maman-action.utils';
...
export class ToutPresDeMamanActionService {
  ...
  private handleRoll(state: GameStateEntity): GameStateEntity {
    ...
    next = this.core.appendLog(
      next,
      `${resolvePlayerNameFromState(next, playerId)} avance de ${total} case(s) et place ${describeToutPresDeMamanPawnLabel(next, playerId)} en case ${target + 1} (${tile?.title ?? `case ${target + 1}`}).`,
    );
  ...
  private getTileByIndex(
    ...
    return getToutPresDeMamanTileByIndex(meta, index);
  }

  private getPlayerPosition(state: GameStateEntity, playerId: number): number {
    return getToutPresDeMamanPlayerPosition(this.getMeta(state), playerId);
  }
