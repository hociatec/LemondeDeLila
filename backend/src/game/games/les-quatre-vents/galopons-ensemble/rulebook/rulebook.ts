...
import { GALOPONS_GAME, type GaloponsActionType } from '../definitions/galopons.definition';
...
export function validateAction(state: GameStateEntity, action: GameSingleActionDto, actorId: number | null): GameSingleActionDto {
  ...
  const type = (rawType === 'roll_dice' ? 'ROLL_DICE' : rawType) as GaloponsActionType;
  ...
  if (actorId == null) throw new PlayerActionError('Acteur requis.', { gameType: 'galopons-ensemble' });

  if (String(state.status ?? '').toLowerCase() !== 'started') {
    throw new PlayerActionError("La partie n'est pas démarrée.", { gameType: 'galopons-ensemble' });
  }

  const pending = state.pending as any;
  if (pending) {
    if (pending.playerId !== actorId) {
      throw new PlayerActionError("Ce n'est pas votre action.", { gameType: 'galopons-ensemble' });
    }
    if (pending.type === 'choose_target') {
      if (type !== 'choose_target') {
        throw new PlayerActionError('Choix invalide.', { gameType: 'galopons-ensemble' });
      }
      const targets: Array<{ targetPlayerId: number }> = Array.isArray(pending?.data?.targets)
        ? pending.data.targets
        : [];
      ...
      if (!Number.isFinite(targetPlayerId) || !targets.some((t) => t.targetPlayerId === targetPlayerId)) {
        throw new GameValidationError('Cible invalide.', { gameType: 'galopons-ensemble', targetPlayerId });
      }
      return { type: 'choose_target', payload: { targetPlayerId } };
    }
    throw new PlayerActionError('Action non disponible.', { gameType: 'galopons-ensemble' });
  }
