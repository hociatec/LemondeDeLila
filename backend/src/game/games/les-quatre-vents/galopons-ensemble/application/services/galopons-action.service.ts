...
import { assignConfiguredBotPawns } from '../../../../../application/helpers/configured-pawn-setup.helper';
import type {
  GaloponsCard,
  GaloponsCardEffect,
  GaloponsMetadata,
...
import {
  asGaloponsPartialMeta as asPartialMeta,
  asGaloponsRecord as asRecord,
  clampGaloponsPosition as sharedClamp,
  extractGaloponsMoveDelta as sharedExtractMoveDelta,
  findNextGaloponsTile as sharedFindNext,
  normalizeGaloponsPawnChoiceLabel,
  resolveGaloponsPawnName,
  resolveGaloponsPawnLabel,
  resolveOtherGaloponsPlayers,
  toGaloponsText as sharedToText,
  type GaloponsChooseTargetContext,
} from './galopons-action.utils';
...
export class GaloponsActionService {
  ...
  private handleChoosePawn(
    ...
    const applied = applyConfiguredPawnSelection({
      ...
      playerPawnLabelResolver: (choice, currentState) =>
        resolveGaloponsPawnName(this.getMeta(currentState).pawns, toText(choice.id)) ||
        normalizeGaloponsPawnChoiceLabel(toText(choice.label)) ||
        toText(choice.id) ||
        'pion',
  ...
  private applyLanding(
    ...
    next = this.core.appendLog(
      next,
      `${resolvePlayerNameFromState(next, playerId)} place ${resolveGaloponsPawnLabel(next, meta, playerId)} en case ${tile.n} (${tile.title}).`,
    );
  ...
  private otherPlayers(
    ...
    return resolveOtherGaloponsPlayers(state, me);
  }
...
function extractMoveDelta(text: string): number {
  return sharedExtractMoveDelta(text);
}
...
function findNext(
  ...
  return sharedFindNext(tiles, start, direction, predicate);
}

function toText(value: unknown): string {
  return sharedToText(value);
}
