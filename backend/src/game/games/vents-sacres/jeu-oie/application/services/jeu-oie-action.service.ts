...
import {
  ...
  describeJeuOiePawnPossessiveLabel,
  lowercaseJeuOieFirst,
  normalizeJeuOiePawnChoice,
} from './jeu-oie-action.utils';
export class JeuOieActionService {
  ...
  private normalizePawnChoice(pawn: JeuOiePawn | null | undefined): JeuOiePawn {
    return normalizeJeuOiePawnChoice(pawn);
  }
  ...
  private handleChoosePawn(
    ...
    const choicesForPending = allPawnsForPending
      .map((p: JeuOiePawn) => this.normalizePawnChoice(p))
    ...
    let started = continueSequentialPawnSelection({
      ...
      pawnDataMapper: (p: JeuOiePawn) => this.normalizePawnChoice(p),
      starterId:
  ...
  private applyLanding(
    ...
    const compactLabel = compactJeuOieTileLabel(label, position);
    next = this.core.appendLog(
      next,
      `${resolvePlayerNameFromState(next, playerId)} place ${describeJeuOiePawnPossessiveLabel(next, playerId)} en case ${position} (${compactLabel}).`,
    );
  ...
  private getMeta(state: GameStateEntity): JeuOieMetadata {
    return (state.metadata ?? {}) as JeuOieMetadata;
  }
  ...
  private ensurePawnSelectionPrompt(state: GameStateEntity): GameStateEntity {
    ...
    const availablePawns = (Array.isArray(meta.pawns) ? meta.pawns : [])
      .map((pawn: JeuOiePawn) => this.normalizePawnChoice(pawn))
    ...
    return continueSequentialPawnSelection({
      ...
      isAssigned,
      pawns,
      pawnDataMapper: (choice: JeuOiePawn) => this.normalizePawnChoice(choice),
