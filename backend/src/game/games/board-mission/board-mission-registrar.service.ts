...
import { buildBoardMissionShortcuts } from '../../engine/board-mission/board-mission.shortcuts';
import {
  getBoardMissionAvailableActions,
  validateBoardMissionAction,
} from '../../engine/board-mission/board-mission.rulebook';
...
import type { BoardMissionGameMetadata } from '../../engine/board-mission/board-mission.types';
...
@Injectable()
export class BoardMissionRegistrarService implements OnModuleInit {
  private readonly logger = new Logger(BoardMissionRegistrarService.name);
      ...
      exposeStateForUser: (
        ...
        this.presenter.exposeStateForUser(
          ...
          this.modelLoader.load(entry.id, entry.gameDir),
          {
            ...
            actionsLabel: ui.actionLabel ?? 'Lancer le dé',
            scorePanelTitle: ui.scorePanelTitle ?? 'Trajets',
            idleClientText: ui.idleClientText ?? 'Aucun client à bord.',
            idleRouteText: ui.idleRouteText ?? 'Aucun trajet en cours.',
            idleEventText: ui.idleEventText ?? 'Pas d’obstacle identifié.',
          },
        ),
