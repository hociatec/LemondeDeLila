...
export class DameNatureService implements GameRulesAdapter, OnModuleInit {
  ...
  private dispatchAction(
    ...
    if (next.turn?.currentPlayerId != null) {
      ...
      if (pendingQuiz && (!isQuizAnswer || !isQuizTarget)) {
        dameNatureLog('block.pending_quiz', {
          actorId,
          currentId,
          pendingQuiz,
        });
      ...
      if (
        ...
        dameNatureLog('block.not_current_player', {
          actorId,
          currentId,
          action: action.type,
        });
    ...
    switch (action.type.toLowerCase()) {
      ...
      case 'answer_ask_card':
        dameNatureLog('action.answer_ask', {
          actorId,
      ...
      case 'answer_quiz_wrong':
        dameNatureLog('action.answer_quiz', {
          actorId,
  ...
  getAvailableActions(
      ...
      if (meta.pendingAsk.targetId === playerId) {
        ...
        const hasRequested =
          requestedMemberId != null && me != null
            ? (me.hand ?? []).some((c) => c.memberId === requestedMemberId)
            : false;
    ...
    const progress =
      typeof playerId === 'number'
        ? this.getTurnProgress(meta, playerId)
        : { playerId, drew: false, discarded: false, asked: false };
  ...
  private handleDraw(
    ...
    if (progress.asked) {
      return this.appendAction(
        this.core.appendLog(state, `Demande impossible : vous avez déjà demandé une carte ce tour.`),
  ...
  private handleQuizAnswer(
    ...
    const correctFromPayload =
      typeof action.payload?.correct === 'boolean'
  ...
  exposeState(state: GameStateEntity): GameStateWithActions {
    ...
    const actionPlayerId =
      ...
      (typeof currentId === 'number' ? currentId : null);
    const actions =
      typeof actionPlayerId === 'number'
        ? this.getAvailableActions(state, actionPlayerId)
        : [];
    const players = this.ensurePlayers(state);
    const families = this.setup.families();
    ...
    const extrasFromState = (state as GameStateEntity & { extras?: unknown })
      .extras;
    const baseExtras =
      extrasFromState && typeof extrasFromState === 'object'
        ? (extrasFromState as Record<string, unknown>)
        : {};
    const currentPlayerView =
      typeof currentId === 'number'
        ? (playerViews.find((v) => v.id === currentId) ?? null)
        : null;
      ...
      started && typeof currentId === 'number'
        ? players
            ...
            .reduce(
              ...
              {} as Record<
                string,
                { familyId: string; memberId: string; label: string }[]
              >,
            )
        : {};
    const shortcuts: any[] = [
    ...
    if (
      pending &&
      actionPlayerId != null &&
      pending.targetId === actionPlayerId
    ) {
      shortcuts.push({
        key: 'pressed A',
        type: 'action',
    ...
    if (
      pendingQuiz &&
      actionPlayerId != null &&
      pendingQuiz.playerId === actionPlayerId
    ) {
      shortcuts.push({
        key: 'pressed A',
        type: 'action',
        actionType: 'answer_quiz_correct',
      });
      shortcuts.push({
        key: 'pressed R',
        type: 'action',
        actionType: 'answer_quiz_wrong',
      });
    }
    ...
    return {
      ...
      extras: {
        ...baseExtras,
        catalog: this.setup.families().reduce(
          (acc, f) => ({
            ...acc,
        ...
        askTargetHands,
        handCards:
          started && currentPlayerView && currentId != null
            ? (players
