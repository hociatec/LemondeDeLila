...
Object.defineProperty(exports, "GameStatsService", {
    enumerable: true,
    get: function() {
...
let GameStatsService = class GameStatsService {
    ...
    async finalizeFinished(roomId, state) {
        ...
        match.winnerUser = winnerId != null ? await this.users.findOne({
            where: {
                id: winnerId
            }
        }) : null;
    ...
    async getLeaderboardGames() {
        const rows = await this.matches.createQueryBuilder('m').select('DISTINCT m.game_type', 'gameType').where('m.ended_at IS NOT NULL').orderBy('m.game_type', 'ASC').getRawMany();
        const list = [];
        for (const r of rows){
    ...
    async getTop10(gameType) {
        ...
        const rows = await this.players.createQueryBuilder('p').innerJoin('p.match', 'm').select('p.user_id', 'userId').addSelect('MAX(p.username)', 'username').addSelect("SUM(CASE WHEN p.outcome = 'won' THEN 1 ELSE 0 END)", 'wins').addSelect("SUM(CASE WHEN p.outcome = 'lost' THEN 1 ELSE 0 END)", 'losses').addSelect("SUM(CASE WHEN p.outcome IN ('won','lost','draw') THEN 1 ELSE 0 END)", 'finished').addSelect("SUM(CASE WHEN p.outcome = 'quit' THEN 1 ELSE 0 END)", 'quit').where('m.game_type = :gameType', {
            gameType: normalized
        }).andWhere('m.ended_reason = :reason', {
            reason: 'finished'
        }).groupBy('p.user_id').orderBy('wins', 'DESC').addOrderBy('finished', 'DESC').addOrderBy('losses', 'ASC').limit(10).getRawMany();
        return rows.map((r)=>({
    ...
    async resetAllStats() {
        const deletedPlayers = await this.players.createQueryBuilder().delete().execute();
        const deletedMatches = await this.matches.createQueryBuilder().delete().execute();
        return {
    ...
    constructor(matches, players, users, catalog){
        ...
        this.logger = new _common.Logger(GameStatsService.name);
    }
};
GameStatsService = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_param(0, (0, _typeorm.InjectRepository)(_gamematchentity.GameMatch)),
    _ts_param(1, (0, _typeorm.InjectRepository)(_gamematchplayerentity.GameMatchPlayer)),
