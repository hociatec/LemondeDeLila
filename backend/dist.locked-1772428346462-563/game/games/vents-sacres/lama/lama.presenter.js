...
let LamaPresenter = class LamaPresenter extends _basepresenterservice.BasePresenterService {
    ...
    buildExtrasForUser(state, metadata, userId, currentPlayerId) {
        ...
        players.filter((p)=>typeof p?.id === 'number').forEach((p)=>pushScoreLine(p.id));
        Object.keys(scoreBy).map((key)=>Number(key)).filter((pid)=>Number.isFinite(pid) && !seenIds.has(pid)).sort((a, b)=>a - b).forEach((pid)=>pushScoreLine(pid));
