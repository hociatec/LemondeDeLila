...
Object.defineProperty(exports, "AFondLesBallonsSetupService", {
    enumerable: true,
    get: function() {
        return AFondLesBallonsSetupService;
    }
});
...
let AFondLesBallonsSetupService = class AFondLesBallonsSetupService {
    loadPawns() {
    ...
    hydrateInitialState(baseState) {
        ...
        const pawnByPlayerId = normalizePawnAssignments(players, metaSeed.pawnByPlayerId ?? metaSeed.charactersByPlayerId, pawns);
        const setupStarterId = typeof metaSeed.setupStarterId === 'number' ? metaSeed.setupStarterId : resolveSeededStarterId(players, baseState.metadata ?? {}, baseState.turn?.currentPlayerId ?? null);
        ...
        next = this.core.appendLog(next, 'Objectif : atteindre exactement la case 40 (la Grosse Noix Dorée). Si vous dépassez, vous reculez du surplus.');
        return next;
    }
    constructor(core, random, contentLoader, setupFlow){
        this.core = core;
        this.random = random;
...
AFondLesBallonsSetupService = _ts_decorate([
...
function resolveSeededStarterId(players, meta, fallbackId) {
    ...
    const shuffled = (0, _seededshuffle.seededShuffle)(players, seed, 'a-fond-les-ballons:setup-starter');
...
function buildTiles() {
    ...
    for(let i = 0; i < 40; i += 1){
        ...
        const label = type === 'start' ? 'Départ - La Tanière à Tartines' : type === 'finish' ? 'La Grosse Noix Dorée' : type === 'bonus' ? 'Bonus' : type === 'folie' ? 'Folie' : type === 'piege' ? 'Piège' : type === 'glissade' ? 'Glissade' : type === 'tornade' ? 'Tornade' : type === 'chaton' ? 'Chaton' : 'Neutre';
        tiles.push({
            type,
...
function normalizePawnAssignments(players, raw, pawns) {
    const byId = {};
...
function buildCharactersByPlayerId(pawnByPlayerId, pawns) {
    const byId = {};
...
function resolvePawnIdFromChoices(raw, pawns) {
    if (raw == null) return null;
    const rawRecord = asRecord(raw);
    const value = typeof raw === 'object' ? rawRecord.id ?? rawRecord.pawnId ?? rawRecord.value ?? raw : raw;
