...
describe('MinuitActionService', ()=>{
            ...
            pending: {
                ...
                data: {
                    pawns: [
                        ...
                        {
                            id: 'La FÃ©e des Flocons',
                            label: 'La FÃ©e des Flocons'
                        }
                    ]
                }
            },
    ...
    it('logs canonical pawn names (not pawn ids/slugs) when choosing a pawn', ()=>{
            ...
            pending: {
                ...
                data: {
                    pawns: [
                        {
                            id: 'fee-des-flocons',
                            label: 'La FÃ©e des Flocons: Agile'
                        },
                        {
                            id: 'lutin',
        ...
        const messages = (next.log ?? []).map((l)=>String(l.message ?? ''));
        expect(messages.some((m)=>m.includes('Lilas a choisi le pion: La FÃ©e des Flocons.'))).toBe(true);
                    ...
                    cards: [
                        {
                            ...
                            kind: 'Surprise',
                            lines: [
                                'Avancez jusquÃ  la prochaine Carte NoÃ«l.'
                            ]
                        }
                    ],
                    discard: []
    ...
    it('stops landing loops without overflowing the call stack', ()=>{
        ...
        const messages = (next.log ?? []).map((l)=>String(l.message ?? ''));
        expect(messages.some((m)=>m.includes('EnchaÃ®nement de cases interrompu pour Ã©viter une boucle infinie.'))).toBe(true);
    });
});
