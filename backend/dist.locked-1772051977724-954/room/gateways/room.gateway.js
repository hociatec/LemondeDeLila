...
let RoomGateway = class RoomGateway {
    ...
    async handleCommand(client, meta, payload) {
        ...
        if (type === 'room.start' || type === 'room.reset' || type === 'bot.add' || type === 'bot.remove' || type === 'room.toggle-privacy' || type === 'room.kick' || type === 'room.ban' || type === 'room.set-owner' || type === 'room.set-ambience') {
            this.safeSend(client, {
                type: 'room.ack',
                roomId: meta.roomId,
                payload: {
