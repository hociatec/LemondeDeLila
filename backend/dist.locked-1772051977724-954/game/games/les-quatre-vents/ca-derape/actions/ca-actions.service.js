...
Object.defineProperty(exports, "CaActionService", {
    enumerable: true,
    get: function() {
        return CaActionService;
    }
});
...
let CaActionService = class CaActionService {
    ...
    handleRoll(state) {
        ...
        const deltaFromPrevious = typeof meta.statuses?.nextPlayerDelta === 'number' ? meta.statuses.nextPlayerDelta : 0;
        ...
        next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, currentId) + ' lance le dé : "' + String(roll) + '".');
        // Move by roll, plus optional +/-1.
        meta = this.getMeta(next);
        let move = roll + deltaFromPrevious;
        if (deltaFromPrevious !== 0) {
            next = this.core.appendLog(next, 'Déplacement ' + (deltaFromPrevious > 0 ? '+1' : '-1') + ' appliqué au lancer.');
        }
        ...
        if (move !== 0) {
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, currentId) + ' ' + verb + ' de ' + String(Math.abs(move)) + ' ' + casesWord + '.');
        } else {
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, currentId) + ' ne se déplace pas.');
        }
        ...
        next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, currentId) + ' met ' + this.pawnLabel(next, currentId) + ' en case ' + String(after + 1) + (label ? ' (' + label + ')' : '') + (isNeutral ? ' (neutre).' : ' (carte).'));
        if (desc) {
        ...
        if (after >= meta.tiles.length - 1) {
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, currentId) + ' remporte la partie !');
            return {
                ...next,
                status: 'finished',
                metadata: {
    ...
    handleDraw(state) {
        ...
        next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, playerId) + ' pioche : "' + String(card.title) + '".');
        if (card.text) {
        ...
        const override = this.getMeta(next).pendingContext ?? null;
    ...
    handleChooseTarget(state, action) {
        ...
        let meta = this.getMeta(state);
        const context = meta.pendingContext ?? null;
        if (!context || context.actorId !== currentId) return {
            ...state,
            pending: null
        };
        ...
        if (context.kind === 'mirror_next_roll') {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} copiera le prochain lancer de ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId)}.`);
            return this.advanceTurnWithNextDelta(next);
        }
        return {
            ...state,
    ...
    handleChooseNextPlayer(state, action) {
        if (String(state.status ?? '').toLowerCase() !== 'started') return state;
        const currentId = state.turn?.currentPlayerId ?? null;
        if (currentId == null) return state;
        const pending = state.pending;
        ...
        next = this.core.appendLog(next, `Prochain joueur choisi : ${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)}.`);
        return {
            ...next,
            turnIndex: idx,
            turn: {
    ...
    handleChooseNextDelta(state, action) {
        if (String(state.status ?? '').toLowerCase() !== 'started') return state;
        const currentId = state.turn?.currentPlayerId ?? null;
        if (currentId == null) return state;
        const pending = state.pending;
        ...
        if (!Number.isFinite(delta) || delta !== -1 && delta !== 1) return {
            ...state,
            pending: null
        };
        let meta = this.getMeta(state);
        ...
        next = this.core.appendLog(next, `Le prochain joueur aura un déplacement ${delta > 0 ? '+1' : '-1'}.`);
        return this.advanceTurnWithNextDelta(next);
    }
    applyCardEffects(state, actorId, card) {
        let next = state;
        let meta = this.getMeta(next);
        const lastIndex = Math.max(0, (meta.tiles?.length ?? 0) - 1);
        let movedByCard = false;
        let lastLandingPos = null;
        let lastLandingIsNeutral = true;
        const logLanding = (pos)=>{
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' met ' + this.pawnLabel(next, actorId) + ' en case ' + String(pos + 1) + (label ? ' (' + label + ')' : '') + (isNeutral ? ' (neutre).' : ' (carte).'));
            if (desc) {
                next = this.core.appendLog(next, desc);
            }
        };
        const finalize = (state)=>{
            ...
            if (movedByCard && lastLandingPos != null && !lastLandingIsNeutral && !state.pending) {
                return {
                    ...state,
                    pending: {
                        type: 'draw',
        ...
        const applyMove = (delta, reason)=>{
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' ' + verb + ' de ' + String(Math.abs(delta)) + ' ' + casesWord + ' (' + reason + ').');
            logLanding(after);
            movedByCard = true;
            if (after >= lastIndex) {
                meta = {
                ...
                next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' remporte la partie !');
            }
        };
        const applySkip = (count, reason)=>{
            const ignorePenalty = meta.statuses?.ignoreNextPenalty?.[actorId] === true;
            if (ignorePenalty) {
                meta = {
                    ...meta,
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' devra passer ' + String(count) + ' tour(s) (' + reason + ').');
        };
        // No-op cards
        ...
        if (card.kind === 'rule') {
            ...
            if (card.id === 61) {
                ...
                next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' lance deux dés: ' + String(r1.roll) + ' et ' + String(r2.roll) + '.');
    ...
    applyGlobal(state, _actorId, card) {
        let next = state;
        let meta = this.getMeta(next);
        ...
        if (reached.length) {
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, winner) + ' remporte la partie !');
        }
        return {
            ...next,
            metadata: {
    ...
    applyConditional(state, actorId, card) {
        let next = state;
        let meta = this.getMeta(next);
        const lastIndex = Math.max(0, (meta.tiles?.length ?? 0) - 1);
        ...
        const applyMove = (delta, note)=>{
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' ' + (delta >= 0 ? 'avance' : 'recule') + ' de ' + String(Math.abs(delta)) + ' case(s) (' + note + ').');
        };
        if (card.id === 51) {
            ...
            if (same != null) {
                ...
                next = this.core.appendLog(next, 'Condition : ' + (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' et ' + (0, _playernamehelper.resolvePlayerNameFromState)(next, same) + ' avancent de 2 cases.');
            }
        } else if (card.id === 58) {
            ...
            if (aheadId != null && (positions[aheadId] ?? 0) === pos + 1) {
                ...
                next = this.core.appendLog(next, 'Condition : ' + (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' rejoint ' + (0, _playernamehelper.resolvePlayerNameFromState)(next, aheadId) + '.');
            }
        } else if (card.id === 60) {
    ...
    applySpecialAfterMove(state, actorId, card) {
        ...
        if (card.id === 33) {
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' prend la premi?re place.');
            return next;
        }
        ...
        if (card.id === 35) {
            ...
            const ahead = others.map((id)=>({
                    id,
                    pos: positions[id] ?? 0
                })).filter((x)=>x.pos > myPos).sort((a, b)=>a.pos - b.pos)[0];
            if (!ahead) {
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + ' saute au-dessus de ' + (0, _playernamehelper.resolvePlayerNameFromState)(next, ahead.id) + '.');
            return next;
        }
        ...
        if (card.id === 36) {
            ...
            next = this.core.appendLog(next, (0, _playernamehelper.resolvePlayerNameFromState)(next, actorId) + " avance jusqu'à une case multiple de 5.");
            return next;
        }
        return next;
    }
    advanceTurnWithNextDelta(state) {
    ...
    pawnLabel(state, id) {
        ...
        const pawn = typeof playerRecord.pawn === 'string' ? playerRecord.pawn.trim() : '';
        const resolved = pawn || (0, _playernamehelper.resolvePlayerNameFromState)(state, id);
        return `"${resolved}"`;
    }
