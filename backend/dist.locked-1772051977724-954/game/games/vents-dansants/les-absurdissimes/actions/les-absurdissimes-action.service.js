...
Object.defineProperty(exports, "AbsurdissimesActionService", {
    enumerable: true,
    get: function() {
        return AbsurdissimesActionService;
    }
});
...
let AbsurdissimesActionService = class AbsurdissimesActionService {
    ...
    handlePlayCard(state, action) {
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(next.players, currentPlayerId)} propose ${cardId}.`);
        ...
        next = {
            ...next,
            turn: {
                currentPlayerId: remainingPlayers[0],
                direction: 1
            }
        };
        return next;
    ...
    handleJudgePick(state, action) {
        ...
        let next = this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerName)(state.players, winnerId)} remporte la manche avec la réponse ${meta.submissions[winnerId] ?? ''}.`);
        ...
        if (hasWinner) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerName)(next.players, winnerId)} atteint ${target} points !`);
            return next;
        }
    ...
    prepareNextRound(state, meta) {
        ...
        return this.core.appendLog(nextState, `Nouvelle manche : ${(0, _playernamehelper.resolvePlayerName)(nextState.players, judgeId)} est juge.`);
    }
...
AbsurdissimesActionService = _ts_decorate([
