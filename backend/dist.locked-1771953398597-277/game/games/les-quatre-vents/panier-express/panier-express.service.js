...
var PanierExpressService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.PanierExpressService = void 0;
...
const panier_express_shortcuts_1 = require("./panier-express.shortcuts");
let PanierExpressService = class PanierExpressService extends abstract_game_service_1.AbstractGameService {
    static { PanierExpressService_1 = this; }
    core;
    deckPool;
    movement;
    tileRegistry;
    ...
    description = 'Course au marché : compléter sa liste puis revenir pile sur la case départ.';
    minPlayers = 2;
    maxPlayers = 10;
    static SHOPPING_LIST_SIZE = 3;
    ...
    exposeStateForUser(state, userId) {
        const ensured = this.ensureMetadata(state);
        const viewerId = typeof userId === 'number' &&
            (ensured.players ?? []).some((p) => p && p.id === userId)
            ? userId
            : null;
        const actions = typeof viewerId === 'number'
            ? this.getAvailableActions(ensured, viewerId)
            : [];
        const meta = this.getMetadata(ensured);
        const rawPending = ensured.pending ?? null;
        const pendingQuiz = typeof viewerId === 'number'
    ...
    hydrateInitialState(baseState) {
        ...
        assignmentOrder.forEach((p) => {
            ...
            const list = existingList.length > 0
                ? existingList
        ...
        hydratedPlayers.forEach((p, idx) => {
            ...
            const hadList = Array.isArray(originalPlayer?.shoppingList) &&
            ...
            withLogs = this.core.appendLog(withLogs, '[Panier Express] ' +
                label +
    ...
    getAvailableActions(state, playerId) {
        return PanierExpressRulebook.getAvailableActions(this.ensureMetadata(state), playerId);
    }
    validateAction(state, action, actorId) {
        return PanierExpressRulebook.validateAction(this.ensureMetadata(state), action, actorId);
    }
    ...
    hydrateMetadataCollections(state, meta, players) {
        ...
        const movementDirection = meta.movementDirection === -1 || meta.movementDirection === 1
            ? meta.movementDirection
            : 1;
        const movementDirectionOwnerId = typeof meta.movementDirectionOwnerId === 'number'
    ...
    queuePawnSelection(state) {
        ...
        const withClearedBots = normalizedPlayers === players
            ? state
            : { ...state, players: normalizedPlayers };
        const taken = new Set(normalizedPlayers
        ...
        if (!chooser)
            return withClearedBots;
        const pawns = choices.map((pawn) => ({
            id: pawn.name,
            label: pawn.description && pawn.description.length > 0
    ...
    handleRoll(state, action) {
        ...
        (0, playing_logger_1.playingLog)('panier.roll', {
        ...
        const hasBlockingExchange = postActions.some((a) => ['exchange_choose_target', 'exchange_choose_give'].includes((a.type || '').toLowerCase()));
    ...
    handleDraw(state, action) {
        ...
        const actorId = typeof actionMeta.actorId === 'number'
            ? actionMeta.actorId
            : Number(pending.playerId);
        const pendingPlayerId = Number(pending.playerId);
        if (Number.isFinite(pendingPlayerId) &&
    ...
    movePlayer(state, playerId, roll) {
        const ensured = this.ensureMetadata(state);
        const meta = this.getMetadata(ensured);
        const tiles = Array.isArray(meta.tiles) && meta.tiles.length
            ? meta.tiles
            : this.buildTiles();
        ...
        const nextState = { ...ensured, metadata: nextMeta };
    ...
    resolveTile(state, playerId) {
        ...
        const announced = this.core.appendLog(ensured, description
    ...
    registerTileHandlers() {
        ...
        this.tileRegistry.register('event', (s, ctx) => this.startDrawPending(s, ctx.playerId, { kind: 'event.card' }, 'Piocher une carte Événement (Espace).'));
        ...
        this.tileRegistry.register('bonus_course', (s, ctx) => this.queueCourseDraws(s, [{ playerId: ctx.playerId, standId: 'bonus' }], 'Piocher une course bonus (Espace).'));
    ...
    registerStandHandlers() {
        this.standEffects.registerStand('stand', (s, ctx) => this.queueCourseDraws(s, [{ playerId: ctx.playerId, standId: ctx.standId }], 'Piocher une course (Espace).'));
        this.standIds().forEach((id) => {
            this.standEffects.registerStand(id, (s, ctx) => this.queueCourseDraws(s, [{ playerId: ctx.playerId, standId: ctx.standId }], 'Piocher une course (Espace).'));
        });
    }
    applyEvent(state, playerId) {
        ...
        if (!drawn.card) {
            const refilled = this.deckPool.set(meta.decks, 'events', this.deckPool.shuffle([...this.setup.eventCards()]));
            drawn = this.drawFromPool({ ...meta, decks: refilled }, 'events');
        ...
        const addOneCourseToPlayer = (pid, card) => {
            ...
            const hasCard = this.utils.toStringArray(playerNow?.basket).includes(trimmed) ||
        ...
        const buildTargets = (excludePlayerId) => (next.players ?? [])
            .filter((p) => p.id !== excludePlayerId)
            .map((p) => ({ playerId: p.id, username: p.username }));
        const buildTargetChoices = (targets) => targets
        ...
        switch (event) {
            ...
            case 'stand-exceptionnel':
                ...
                next = this.queueCourseDraws(next, [{ playerId, standId: 'bonus' }], 'Piocher une course bonus (Espace).');
                next = this.appendActionLog(next, playerId, 'event', {
                    event,
                    effect: 'draw',
                });
            ...
            case 'tirage-chanceux': {
                next = this.startDrawPending(next, playerId, { kind: 'event.tirage_chanceux' }, 'Tirage chanceux : piocher 3 cartes (Espace).');
                next = this.appendActionLog(next, playerId, 'event', {
                    event,
                    effect: 'pick',
                });
            ...
            case 'producteur-genereux': {
                next = this.startDrawPending(next, playerId, { kind: 'event.producteur_genereux' }, 'Producteur généreux : piocher 2 courses bonus (Espace).');
                next = this.appendActionLog(next, playerId, 'event', {
                    event,
                    effect: 'pick',
                });
            ...
            case 'recette-express': {
                ...
                const requirementLabel = requiredItems > 1
            ...
            case 'stand-en-fete': {
                ...
                next = this.queueCourseDraws(next, targets.map((p) => ({ playerId: p.id, standId: 'bonus' })), 'Piocher une course bonus (Espace).');
            ...
            case 'stand-surprise': {
                ...
                const roll = rng.roll;
                const matcher = roll <= 2
                    ? (id) => id.startsWith('bio')
                    : roll <= 4
                        ? (id) => id === 'fruitier'
                        : (id) => id.startsWith('primeur');
                const metaNow = this.getMetadata(next);
                const tiles = Array.isArray(metaNow.tiles) ? metaNow.tiles : [];
                const total = tiles.length;
                const current = metaNow.positions?.[playerId] ?? 0;
            ...
            case 'echange-obligatoire': {
                ...
                if (giveA)
                    removeOneCourseFromPlayer(playerId, giveA);
                if (giveB)
                    removeOneCourseFromPlayer(targetId, giveB);
                if (giveA)
                    addOneCourseToPlayer(targetId, giveA);
                if (giveB)
                    addOneCourseToPlayer(playerId, giveB);
            ...
            case 'inversion-de-panier': {
                ...
                const swapped = playersWithInventory.map((p) => {
                    if (p.id === playerId)
                        return { ...p, inventory: theirInventory };
                    if (p.id === targetId)
                        return { ...p, inventory: myInventory };
                    return p;
                });
            ...
            default:
                if (event === 'erreur-de-livraison' ||
                    event === 'produit-avarie' ||
                    event === 'emballage-oublie') {
                    const discarded = discardRandomCourse(playerId);
                    next = this.core.appendLog(next, discarded
    ...
    handleMerchantRequestAccept(state, action) {
        ...
        const pending = this.getPendingRecord(state);
        const pendingData = asRecord(pending?.data);
        const ingredient = pending && toText(pending.type) === 'merchant_request'
    ...
    handleSkipTurn(state, action) {
        const playerId = this.getActorIdFromAction(action) ??
    ...
    handleChoosePawn(state, action) {
        ...
        let next = {
            ...state,
            pending: null,
            players: this.getPlayers(state).map((player) => player.id === resolved.playerId ? { ...player, pawn: chosen } : player),
        };
    ...
    handlePickChoice(state, action) {
        ...
        const removeCourseFromPlayer = (base, playerId, card) => {
            const trimmed = String(card ?? '').trim();
            if (!trimmed)
                return { state: base, removed: false };
            let removed = false;
        ...
        if (kind === 'event.tirage_chanceux') {
            const offered = Array.isArray(pendingData.offered)
        ...
        const buildCourseSets = () => {
            ...
            const winterVegStand = (id) => id === 'primeur-hivernal' ||
                id === 'bio-legumes' ||
                id === 'champignons' ||
        ...
        if (kind === 'exchange.strategique.confirm') {
            ...
            const alreadyResolved = Array.isArray(meta.actionLog)
                ? meta.actionLog.some((entry) => entry?.type === 'exchange' &&
            ...
            if (accepted) {
                ...
                next = removedTake.state;
                if (removedGive.removed)
                    next = addCourseToPlayer(next, actorId, give);
                if (removedTake.removed)
        ...
        if (kind === 'exchange.echange_saison.choose_target') {
            ...
            if (!fruitCards.length) {
                ...
                return this.queueCourseDraws(next, [{ playerId: actorId, standId: 'bonus' }], 'Piocher une course bonus (Espace).');
            }
            return {
                ...state,
                pending: {
    ...
    buildShoppingList(used) {
        ...
        if (!items.length)
            return [];
        const size = Math.min(PanierExpressService_1.SHOPPING_LIST_SIZE, items.length);
        const attempt = () => {
...
exports.PanierExpressService = PanierExpressService;
exports.PanierExpressService = PanierExpressService = PanierExpressService_1 = __decorate([
...
//# sourceMappingURL=panier-express.service.js.map