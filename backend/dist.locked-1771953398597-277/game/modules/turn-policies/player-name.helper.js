...
exports.resolvePlayerNameFromState = resolvePlayerNameFromState;
function resolvePlayerName(players, playerId, options) {
    const safePlayers = Array.isArray(players) ? players : [];
    const match = safePlayers.find((player) => {
        const id = player?.id;
        if (options?.coerceNumericIds)
            return Number(id) === playerId;
        return id === playerId;
    });
    let username = match?.username && String(match.username).trim()
...
//# sourceMappingURL=player-name.helper.js.map