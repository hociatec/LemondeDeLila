...
let JeuOieActionService = class JeuOieActionService {
    ...
    handleRoll(state) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)} lance le d̩ : "${roll}".`);
        if (inWell) {
            if (roll !== 1) {
                const logged = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} reste bloqu̩ dans le puits.`);
    ...
    applyLanding(state, playerId, position, roll) {
        ...
        if (tile.type === 'inn' || tile.type === 'prison') {
            ...
            const suffix = turns === 1 ? '' : ` (passera ses ${turns} prochains tours).`;
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} perd ${turns} tour(s).${suffix}`);
            meta = this.getMeta(next);
        ...
        if (tile.type === 'well') {
            ...
            well[playerId] = true;
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} est bloqu̩ dans le puits (il faut faire 1 pour sortir).`);
            return {
                ...next,
                metadata: {
                    ...(next.metadata ?? {}),
