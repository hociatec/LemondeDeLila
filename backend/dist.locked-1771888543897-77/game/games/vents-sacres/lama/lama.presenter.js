...
let LamaPresenter = class LamaPresenter extends base_presenter_service_1.BasePresenterService {
    shared;
    constructor(shared) {
        super();
        this.shared = shared;
    ...
    getAvailableActionsForUser(state, userId) {
        ...
        const handValues = ((meta.handsByPlayerId ?? {})[String(userId)] ?? []).filter((v) => typeof v === 'number' && v >= 1 && v <= lama_model_1.LAMA_VALUE);
        ...
        if (step === 'return_token') {
            if (meta.pendingReturnPlayerId !== userId)
                return [];
            const score = Number((meta.scoresByPlayerId ?? {})[String(userId)] ?? 0);
            if (score >= 1)
                out.push({ type: 'lama_return', payload: { value: 1 } });
            if (score >= 10)
        ...
        const trackerPlayerId = this.shared.asNumberOrNull(tracker?.playerId);
        ...
        const drawCount = this.shared.getCurrentTurnDrawCount(meta, userId, turnIndex);
        ...
        if (!drawLocked && (meta.deck ?? []).length > 0 && drawCount < maxDraws) {
    ...
    buildPendingStateForUser(state, metadata, userId, currentPlayerId) {
                ...
                data: {
                    ...
                    fields: [
                        ...
                        },
                        {
                            ...
                            label: 'Rejouer après pioche',
                            kind: 'boolean',
                            initialText: metadata.allowPlayAfterDraw ? 'oui' : 'non',
                        },
                        {
                            key: 'maxDrawsPerTurn',
                            label: 'Pioches max par tour (1-3)',
                            kind: 'number',
                            min: 1,
                            max: 3,
                            initialText: String(this.shared.getMaxDrawsPerTurn(metadata)),
                        },
                    ],
                },
            };
        ...
        if ((metadata.step ?? '') === 'round_pause') {
            const until = typeof metadata.roundPauseUntilMs === 'number'
        ...
        if (step === 'return_token') {
            if (metadata.pendingReturnPlayerId !== userId)
                return null;
            const score = Number((metadata.scoresByPlayerId ?? {})[String(userId)] ?? 0);
            const choices = [];
            if (score >= 1)
                choices.push('Rendre 1 jeton');
            if (score >= 10)
                choices.push('Rendre 1 diamant (10 jetons)');
        ...
        const turnIndex = Number(state.turnIndex ?? 0);
        const drawCount = this.shared.getCurrentTurnDrawCount(metadata, userId, turnIndex);
        const maxDraws = this.shared.getMaxDrawsPerTurn(metadata);
        const canDraw = !drawLocked && (metadata.deck ?? []).length > 0 && drawCount < maxDraws;
        const top = this.topDiscard(metadata);
        if (!top)
            return null;
        const choices = hand
            .slice()
            .filter((v) => typeof v === 'number' && v >= 1 && v <= lama_model_1.LAMA_VALUE)
            .sort((a, b) => a - b)
            .map(lama_model_1.lamaCardLabel);
        const meScore = Number((metadata.scoresByPlayerId ?? {})[String(userId)] ?? 0);
        const discardTop = (0, lama_model_1.lamaCardLabel)(top);
        const handScore = [...new Set(hand)].reduce((sum, v) => sum + (0, lama_model_1.lamaCardScore)(v), 0);
        const drawHint = drawLocked
            ? '↑/↓ choisir, Entrée jouer, P/Q passer, C défausse, E mains, S jetons'
            : canDraw
                ? `↑/↓ choisir, Entrée jouer, Espace piocher (${drawCount}/${maxDraws}), P/Q passer, C défausse, E mains, S jetons`
                : `↑/↓ choisir, Entrée jouer, P/Q passer, C défausse, E mains, S jetons`;
        return {
            ...
            label: droppedOut
                ...
                : currentPlayerId === userId
                    ? `Défausse : ${discardTop}. Main : ${hand.length} cartes (${handScore} jetons). (${drawHint})`
    ...
    getActionLabel(actionType) {
        if (actionType === 'lama_play')
            return 'Jouer';
        if (actionType === 'draw')
            return 'Piocher';
        if (actionType === 'lama_set_config')
            return 'Configuration';
        if (actionType === 'lama_quit')
            return 'Passer (se retirer de la manche)';
        if (actionType === 'lama_pass')
            return 'Passer (se retirer de la manche)';
        if (actionType === 'lama_return')
            return 'Rendre jetons';
        if (actionType === 'lama_peek_discard')
            return 'Voir défausse';
    ...
    buildExtrasForUser(state, metadata, userId, currentPlayerId) {
        ...
        const discardTop = top ? (0, lama_model_1.lamaCardLabel)(top) : '(vide)';
        const drawLocked = Object.values(metadata.droppedOutByPlayerId ?? {}).some((isOut) => Boolean(isOut));
        const turnIndex = Number(state.turnIndex ?? 0);
        const drawCount = this.shared.getCurrentTurnDrawCount(metadata, userId, turnIndex);
        const maxDraws = this.shared.getMaxDrawsPerTurn(metadata);
        ...
        const playableText = (() => {
            ...
            for (const [value, count] of [...counts.entries()].sort((a, b) => a[0] - b[0])) {
                if (!allowed.has(value))
                    continue;
                parts.push(`${(0, lama_model_1.lamaCardLabel)(value)}×${count}`);
            }
            return `Défausse : ${discardTop}. (${canDraw ? `↑/↓ choisir, Entrée jouer, Espace piocher (${drawCount}/${maxDraws}), P/Q passer, C défausse, E mains, S score` : '↑/↓ choisir, Entrée jouer, P/Q passer, C défausse, E mains, S score'})`;
        })();
        return {
            ...base,
            hand,
            ...
            ui: {
                panels: {
                    ...
                    score: {
                        ...
                        message: (() => {
                            ...
                            const loseAt = metadata.loseAtScore != null
                                ...
                                : null;
                            const loseText = loseAt != null && Number.isFinite(loseAt)
                    ...
                    table: {
                        title: 'Table',
                        message: metadata.loseAtScore != null
    ...
    topDiscard(meta) {
        ...
        if (!top)
            return null;
        if (top < 1 || top > lama_model_1.LAMA_VALUE)
            return null;
        return top;
    }
...
exports.LamaPresenter = LamaPresenter;
exports.LamaPresenter = LamaPresenter = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [lama_shared_service_1.LamaSharedService])
], LamaPresenter);
//# sourceMappingURL=lama.presenter.js.map