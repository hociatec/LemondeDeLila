export type SacTileType = 'start' | 'property' | 'station' | 'utility' | 'chance' | 'community' | 'tax' | 'jail' | 'go_to_jail' | 'free' | 'neutral';
export type SacVariantId = 'classic' | 'gaia' | 'violette-boussole' | 'sabord-quai' | 'route-des-flandres' | 'cosmos-credit' | 'pintzel-couronnes';
export type SacTile = {
    n: number;
    title: string;
    description?: string;
...
export type SacBoardJsonV1 = {
    version: 1;
    tiles: SacTile[];
};
export type SacCardsJsonV1 = {
    version: 1;
    cards: SacCard[];
};
