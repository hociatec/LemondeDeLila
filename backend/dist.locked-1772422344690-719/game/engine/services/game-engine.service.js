...
let GameEngineService = class GameEngineService {
   ...
   */ async restoreInternalState(roomId, gameType, state) {
    ...
    async scheduleBotTurn(roomId, gameType, state) {
        ...
        const fastPendingBotAction = Number.isFinite(pendingPlayerId) && pendingPlayerId === botActorId && (pendingType === 'draw' || pendingType === 'choose_pawn' || pendingType === 'pick_pawn' || pendingType === 'swap' || pendingType === 'choose_target');
