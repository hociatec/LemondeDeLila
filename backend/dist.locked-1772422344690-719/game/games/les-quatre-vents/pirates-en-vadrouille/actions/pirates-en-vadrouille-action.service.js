...
Object.defineProperty(exports, "PiratesEnVadrouilleActionService", {
    enumerable: true,
    get: function() {
        return PiratesEnVadrouilleActionService;
    }
});
...
const _piratecardeffects = require("./pirate-card-effects");
...
let PiratesEnVadrouilleActionService = class PiratesEnVadrouilleActionService {
    ...
    handleRoll(state) {
        ...
        if (updatedMeta.keepTurn) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} rejoue.`);
            return next;
        }
    ...
    handleChooseTarget(state, action) {
        ...
        if (!pending || !(0, _pendingactionservice.isPendingType)(state, 'choose_target')) return state;
        const playerId = state.turn?.currentPlayerId ?? null;
        if (playerId == null) return state;
        const payload = asRecord(action.payload);
        const targetId = Number(payload.targetPlayerId);
        const pendingData = asRecord(pending.data);
        const optionsRaw = Array.isArray(pendingData.options) ? pendingData.options : [];
        ...
        if (!Number.isFinite(targetId) || !options.some((opt)=>opt.targetPlayerId === targetId)) {
            return state;
        }
        const meta = this.getMeta(state);
        const ctx = meta.pendingContext;
        if (!ctx || ctx.actorId !== playerId) {
            return (0, _pendingactionservice.clearPendingState)(state);
        }
        let next = this.applyTargetEffect(state, targetId, ctx);
        const updatedMeta = this.getMeta(next);
        next = {
            ...(0, _pendingactionservice.clearPendingState)(next),
            metadata: {
                ...next.metadata ?? {},
                ...updatedMeta,
                pendingContext: null
    ...
    drawCard(state, playerId, deckName) {
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} pioche la carte "${card.title}".`);
    ...
    applyBonusCard(state, playerId, card) {
        let next = state;
        const effect = _piratecardeffects.BONUS_CARD_EFFECTS[card.id];
    ...
    applyObstacleCard(state, playerId, card) {
        ...
        if (immunity > 0) {
            ...
            return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} est protégé et ignore l'obstacle "${card.title}".`);
        }
        const effect = _piratecardeffects.OBSTACLE_CARD_EFFECTS[card.id];
        if (!effect) return next;
        next = this.core.appendLog(next, this.formatActionMessage(next, effect, playerId));
        switch(effect.kind){
            case 'move':
    ...
    applyTargetEffect(state, targetPlayerId, ctx) {
        ...
        if (ctx.kind === 'target_move' && ctx.actorId != null) {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, ctx.actorId)} applique ${ctx.delta >= 0 ? 'un boost' : 'un ralentissement'} à ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId)} (${ctx.delta}).`);
        ...
        if (ctx.kind === 'steal_treasure' && ctx.actorId != null) {
            ...
            if (!stolen) {
                return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, ctx.actorId)} tente de voler un trésor mais ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId)} n'en possède pas.`);
            }
            ...
            return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, ctx.actorId)} dérobe "${stolen.title}" à ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId)}.`);
        }
        return next;
    }
    promptTargetSelection(state, playerId, effect) {
        const players = Array.isArray(state.players) ? state.players : [];
        const targets = players.filter((p)=>p?.id != null && p.id !== playerId).map((p)=>({
                targetPlayerId: p.id
            }));
        ...
        return {
            ...(0, _pendingactionservice.createPendingState)(state, pending),
            metadata: {
                ...state.metadata ?? {},
                ...meta,
                pendingContext
    ...
    handleFinish(state, playerId) {
        ...
        if (collection.treasures.length >= 3 || collection.goldPieces >= 3) {
            ...
            return this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} ouvre le coffre légendaire et gagne la partie !`);
        }
        const next = this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerNameFromState)(state, playerId)} n'a pas assez de trésors ou pièces d'or et recule de deux cases.`);
    ...
    addCardToCollection(state, playerId, deck, card, force = false) {
        ...
        if (!force && total >= 5) {
            return this.core.appendLog(state, `${(0, _playernamehelper.resolvePlayerNameFromState)(state, playerId)} a déjà cinq cartes et ne peut pas en ajouter.`);
        }
        const updated = {
            ...collection
        };
    ...
    formatActionMessage(state, effect, playerId) {
        ...
        return `${(0, _playernamehelper.resolvePlayerNameFromState)(state, playerId)} ${description}.`;
    }
    ...
    getTotalCards(collection) {
        return (collection.treasures?.length ?? 0) + (collection.obstacles?.length ?? 0) + (collection.bonus?.length ?? 0);
    }
...
PiratesEnVadrouilleActionService = _ts_decorate([
