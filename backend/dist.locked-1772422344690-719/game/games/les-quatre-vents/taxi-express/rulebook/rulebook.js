...
function validateAction(state, action, actorId) {
    ...
    if (!_taxiexpressdefinition.TAXI_EXPRESS_GAME.actions.includes(type)) {
        throw new _gameerrors.GameValidationError(`Action inconnue: ${rawType || '(vide)'}`, {
            gameType: _taxiexpressdefinition.TAXI_EXPRESS_GAME.id,
            action: rawType,
            allowedActions: _taxiexpressdefinition.TAXI_EXPRESS_GAME.actions
        });
    }
    if (actorId == null) {
