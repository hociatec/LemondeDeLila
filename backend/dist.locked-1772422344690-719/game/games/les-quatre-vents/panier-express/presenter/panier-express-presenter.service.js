...
Object.defineProperty(exports, "PanierExpressPresenterService", {
    enumerable: true,
    get: function() {
        return PanierExpressPresenterService;
    }
});
...
let PanierExpressPresenterService = class PanierExpressPresenterService extends _basepresenterservice.BasePresenterService {
    exposeState(params) {
    ...
    buildCatalog() {
        return {
            phases: _rulesdefinition.PANIER_EXPRESS_PHASES.map((p)=>p.id),
    ...
    buildExtras(state, metadata, currentPlayerId) {
        ...
        const sanitizedViews = typeof currentPlayerId === 'number' ? playerViews.map((v)=>v.id === currentPlayerId ? v : {
                ...v,
    ...
    buildPendingView(params) {
        ...
        if (params.rawPending && params.rawPending.type === 'exchange') {
            ...
            if (typeof params.currentId === 'number' && exchangePlayerId != null && exchangePlayerId !== params.currentId) {
                return null;
            }
            if (toText(exchangePending.step) === 'choose_target') {
                const targets = toExchangeTargetArray(exchangePending.targets);
                const choices = targets.map((t)=>(0, _sanitizetext.sanitizeText)(t.targetUsername)).filter((c)=>c.length > 0);
                return {
                    type: 'exchange',
            ...
            if (toText(exchangePending.step) === 'choose_give') {
                ...
                const choices = giveChoices.map((c)=>this.utils.formatCourseLabel((0, _sanitizetext.sanitizeText)(toText(c)))).filter((c)=>c.length > 0);
                const targetUsername = (0, _sanitizetext.sanitizeText)(toText(exchangePending.targetUsername));
                return {
                    type: 'exchange',
                    playerId: exchangePlayerId,
                    targetPlayerId: toNumber(exchangePending.targetPlayerId),
                    blocking: true,
                    question: targetUsername ? `Choisir une carte à donner à ${targetUsername}.` : 'Choisir une carte à donner.',
                    choices,
                    data: {
                        step: 'choose_give',
                        targetPlayerId: toNumber(exchangePending.targetPlayerId),
            ...
            if (toText(exchangePending.step) === 'confirm') {
                const initiator = (0, _sanitizetext.sanitizeText)(toText(exchangePending.initiatorUsername));
                const give = this.utils.formatCourseLabel((0, _sanitizetext.sanitizeText)(toText(exchangePending.give)));
                const take = exchangePending.take != null ? this.utils.formatCourseLabel((0, _sanitizetext.sanitizeText)(toText(exchangePending.take))) : '';
                const question = take ? `${initiator} vous propose un échange : il vous donne "${give}" et vous lui donnez "${take}". (A = accepter, R = refuser)` : `${initiator} vous propose un échange : il vous donne "${give}". (A = accepter, R = refuser)`;
                return {
                    type: 'exchange',
                    playerId: exchangePlayerId,
                    blocking: true,
        ...
        if (params.rawPending && params.rawPending.type && params.rawPending.type !== 'quiz') {
            ...
            const rawChoices = Array.isArray(pendingRecord.choices) ? pendingRecord.choices : null;
            if (!rawChoices) {
                return params.rawPending;
            }
            return {
                ...params.rawPending,
                choices: rawChoices.map((c)=>this.utils.formatCourseLabel((0, _sanitizetext.sanitizeText)(toText(c)))).filter((c)=>c.length > 0)
            };
        }
        return null;
    }
    normalizeQuizPending(pendingQuiz) {
        ...
        const question = (0, _sanitizetext.sanitizeText)(pendingQuiz.question ?? '');
        const rawChoices = Array.isArray(pendingQuiz.choices) && pendingQuiz.choices.length ? pendingQuiz.choices : pendingQuiz.answer ? [
            pendingQuiz.answer
        ] : [];
        const choices = rawChoices.map((choice)=>this.utils.formatCourseLabel((0, _sanitizetext.sanitizeText)(String(choice)))).filter((choice)=>choice.length > 0);
    ...
    buildPlayerViews(state) {
        return (state.players ?? []).map((p)=>toPanierPlayerLike(p)).filter((p)=>p != null).map((p)=>this.buildPlayerView(p)).filter((view)=>Boolean(view));
    }
    ...
    buildExtrasView(state, params) {
        ...
        const currentPlayerView = typeof params.currentId === 'number' ? params.playerViews.find((view)=>view.id === params.currentId) ?? null : null;
        const listMessage = (title, items)=>{
            const values = Array.isArray(items) ? items.map((x)=>String(x ?? '').trim()).filter((x)=>x.length > 0) : [];
            if (values.length === 0) return `${title}: (vide)`;
            const max = 12;
            const shown = values.length > max ? values.slice(0, max) : values;
            const body = shown.join(', ');
            return values.length > max ? `${title}: ${body}, ... (+${values.length - max})` : `${title}: ${body}`;
        };
        const shoppingAllMessage = ()=>{
            ...
            const lines = params.fullPlayerViews.map((p)=>{
                ...
                const body = shown.join(', ');
                return items.length > max ? `${name} : liste ${body}, ... (+${items.length - max})` : `${name} : liste ${body}`;
            });
            return lines.join('\n');
        };
        const inventoryAllMessage = ()=>{
            ...
            const lines = params.playerViews.map((p)=>{
                const name = typeof p.username === 'string' && p.username.trim().length > 0 ? p.username.trim() : `Joueur ${p.id}`;
                const canSee = currentId != null ? p.id === currentId || params.revealInventory[p.id] > 0 : params.revealInventory[p.id] > 0;
                if (!canSee) {
                ...
                const shown = items.length > max ? items.slice(0, max) : items;
                const body = shown.join(', ');
                return items.length > max ? `${name} : inventaire ${body}, ... (+${items.length - max})` : `${name} : inventaire ${body}`;
            });
            return lines.join('\n');
        };
        const scoreMessage = ()=>{
            const views = Array.isArray(params.fullPlayerViews) && params.fullPlayerViews.length ? params.fullPlayerViews : params.playerViews;
    ...
    buildPositionPanelMessage(meta, playerViews) {
        ...
        const lines = playerViews.map((view)=>{
            const name = typeof view.username === 'string' && view.username.trim().length > 0 ? view.username.trim() : `Joueur ${view.id}`;
            const pos = meta.positions?.[view.id];
            const lap = meta.laps?.[view.id];
            const caseNumber = typeof pos === 'number' && Number.isFinite(pos) ? Math.max(1, Math.trunc(pos) + 1) : null;
            const lapText = typeof lap === 'number' && Number.isFinite(lap) ? `tour plateau ${Math.trunc(lap)}` : 'tour plateau ?';
            const caseText = caseNumber ? totalTiles ? `case ${caseNumber}/${totalTiles}` : `case ${caseNumber}` : 'case (inconnue)';
    ...
    constructor(utils, boardPayload){
        super(), this.utils = utils, this.boardPayload = boardPayload, this.rawPendingRef = null;
    }
};
PanierExpressPresenterService = _ts_decorate([
...
function toExchangeTargetArray(value) {
    ...
    return value.map((item)=>asRecord(item)).map((record)=>({
            targetPlayerId: toNumber(record.targetPlayerId),
            targetUsername: (0, _sanitizetext.sanitizeText)(toText(record.targetUsername))
