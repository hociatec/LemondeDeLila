...
Object.defineProperty(exports, "PANIER_EXPRESS_VICTORY", {
    enumerable: true,
    get: function() {
        return PANIER_EXPRESS_VICTORY;
    }
});
const PANIER_EXPRESS_VICTORY = [
    {
        id: 'shopping-complete',
        description: 'Liste de courses complete + retour pile sur la case depart.',
        check: (state)=>{
            const players = state.players ?? [];
            const positions = state.metadata?.positions && typeof state.metadata.positions === 'object' ? state.metadata.positions : {};
            for (const player of players){
                ...
                const pos = typeof positions?.[player.id] === 'number' ? positions[player.id] : -1;
                const atStart = pos === 0;
