...
const _vaultroomsnapshotsservice = require("./vault-room-snapshots.service");
    ...
    it('supports save -> restore -> save (overwrite restored snapshot)', async ()=>{
        ...
        const snapshots = {
            find: jest.fn(async ({ where })=>{
                ...
                return Array.from(snapshotStore.values()).filter((s)=>s.ownerUserId === ownerUserId);
            }),
            ...
            delete: jest.fn(async ({ id, ownerUserId })=>{
            ...
            save: jest.fn(async (entity)=>{
                snapshotStore.set(String(entity.id), entity);
                return entity;
            })
        };
        const roomBots = {
        ...
        const rooms = {
            ...
            saveRoom: jest.fn(async (room)=>room),
        ...
        const bots = {
            addBotSystem: jest.fn(async ()=>({
                    id: 99,
                    name: 'bot-system'
                }))
        };
        const engine = {
            exportInternalState: jest.fn().mockResolvedValueOnce({
                status: 'started',
                metadata: {
                    roomId: 10,
                    roomOwnerId: 1,
        ...
        const service = new _vaultroomsnapshotsservice.VaultRoomSnapshotsService(snapshots, roomBots, rooms, bots, engine, registry, notifications, presence);
        ...
        const firstSnapshot = JSON.parse(String(firstSnapshotRaw?.snapshotJson ?? '{}'));
        ...
        const restoredState = engine.restoreInternalState.mock.calls[0][2];
        expect(restoredState.players.some((p)=>p.id === -99)).toBe(true);
        ...
        expect(notifications.notifyUser).toHaveBeenCalledWith(1, 'rooms.restore.ready', expect.objectContaining({
            roomId: 20
        }));
        expect(notifications.notifyUser).toHaveBeenCalledWith(2, 'rooms.restore.ready', expect.objectContaining({
            roomId: 20
        }));
        ...
        const overwritten = JSON.parse(String(overwrittenRaw?.snapshotJson ?? '{}'));
