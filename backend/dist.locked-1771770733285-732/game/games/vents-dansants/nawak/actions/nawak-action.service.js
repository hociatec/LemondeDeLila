...
let NawakActionService = class NawakActionService {
    ...
    handleChooseAnswer(state, action) {
        ...
        const answerLabel = meta.currentChallenge.answers?.[answerIndex] ?? 'r�ponse inconnue';
        ...
        if (allChosen) {
            ...
            next = this.core.appendLog(next, 'Tous les choix sont faits : votez maintenant pour une r�ponse �trang�re !');
        }
    ...
    finishVoting(state, playerIds, votes) {
        ...
        if (tie) {
            next = this.core.appendLog(next, '�galit� d�tect�e : un nouveau d�fi va d�partager les joueurs.');
        }
        return this.turns.advanceTurn(next);
    }
    getMeta(state) {
