...
exports.EntreRitesActionService = void 0;
...
const entre_rites_state_entity_1 = require("../model/entre-rites-state.entity");
let EntreRitesActionService = class EntreRitesActionService {
    ...
    handleAskCard(state, action) {
        ...
        if (!targetHand.includes(cardId)) {
            let next = this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)} demande ${cardId} à ${(0, player_name_helper_1.resolvePlayerNameFromState)(state, targetId)} sans succès et doit piocher.`);
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} récupère ${cardId} de ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)} et continue.`);
    ...
    handlePass(state, _action) {
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} passe son tour.`);
    }
    advanceTurn(state) {
    ...
    drawCardForPlayer(state, playerId) {
        ...
        if (!cardId || !card) {
            return this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} ne peut plus piocher, la pioche est vide.`);
        }
    ...
    drawSingleCard(state) {
        ...
        const card = cardId ? entre_rites_cards_1.ENTRE_RITES_CARD_BY_ID[cardId] : undefined;
        return { state: next, cardId, card };
    }
    handleDrawnCard(state, playerId, card, allowSpecial = true) {
        if (card.type === 'family') {
            let next = this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} pioche ${card.name}.`);
    ...
    handleSpecialEffect(state, playerId, card) {
        ...
        if (this.isSilenced(next, playerId)) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} pioche ${card.name} mais ses pouvoirs sont désormais muets.`);
    ...
    effectCollectFromOthers(state, playerId) {
        let next = this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} invoque la Bénédiction et réclame une carte à chaque adversaire.`);
        ...
        for (const player of players) {
            ...
            next = this.rebuildCollections(next, playerId);
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} prend ${cardId} à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, opponentId)}.`);
        }
        return next;
    }
    effectTakeFromDiscard(state, playerId) {
        ...
        if (!discard.length) {
            return this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} cherche dans la défausse mais rien n’y est.`);
        }
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} reprend ${cardId} depuis la défausse.`);
    }
    effectMuteSpecials(state, playerId) {
        ...
        return this.core.appendLog(this.setMeta(state, nextMeta), `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} impose le Silence Sacré.`);
    }
    effectSwapHands(state, playerId) {
        ...
        if (!target || target.id == null) {
            return this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} invoque l’Envol Mystique sans adversaire disponible.`);
        }
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} échange sa main avec ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)}.`);
    }
    effectFreeFamily(state, playerId) {
        ...
        const familyKeys = Object.keys(entre_rites_cards_1.ENTRE_RITES_CUSTOM_FAMILY_SIZE);
        const pending = familyKeys.find((familyId) => !completed.has(familyId));
        if (!pending) {
            return this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} active la Clé du Jardin Caché mais toutes les familles sont déjà complètes.`);
        }
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} pose une famille secrète grâce à la Clé du Jardin Caché.`);
    }
    effectReshuffleCycle(state, playerId) {
        let next = this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} déclenche L’Aube Nouvelle : tout le monde défausse puis pioche.`);
    ...
    effectRevealAndSteal(state, playerId) {
        ...
        for (const player of players) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, player.id)} révèle sa main : ${hand.join(', ') || 'vide'}.`);
        }
        ...
        next = this.rebuildCollections(next, targetId);
        next = this.rebuildCollections(next, playerId);
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} s’empare de ${cardId} grâce au Chant du Coq.`);
    }
    discardOneCard(state, playerId) {
        ...
        const next = this.setMeta(state, updatedMeta);
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} défausse ${cardId}.`);
    }
    ...
    rebuildCollections(state, playerId) {
        ...
        for (const [familyId, values] of Object.entries(group)) {
            const needed = entre_rites_cards_1.ENTRE_RITES_CUSTOM_FAMILY_SIZE[familyId] ?? 7;
    ...
    checkVictory(state, playerId) {
        ...
        if (totalFamilies >= entre_rites_state_entity_1.ENTRE_RITES_TOTAL_FAMILIES) {
            const winnerId = this.findWinner(meta) ?? playerId;
            const next = this.core.appendLog(state, `Toutes les familles sont complétées. ${(0, player_name_helper_1.resolvePlayerNameFromState)(state, winnerId)} remporte la partie !`);
...
exports.EntreRitesActionService = EntreRitesActionService;
exports.EntreRitesActionService = EntreRitesActionService = __decorate([
...
//# sourceMappingURL=entre-rites-action.service.js.map