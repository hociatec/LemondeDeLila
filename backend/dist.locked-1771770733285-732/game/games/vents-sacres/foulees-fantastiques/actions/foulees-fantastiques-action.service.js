...
exports.FouleesFantastiquesActionService = void 0;
...
let FouleesFantastiquesActionService = class FouleesFantastiquesActionService {
    ...
    ensureFamilyPending(state) {
        ...
        if (allChosen) {
            let next = { ...state, phase: 'turn', pending: null };
        ...
        const taken = new Set(Object.values(familyIdByPlayer)
            .filter((v) => typeof v === 'string')
            .map((v) => v.trim().toLowerCase())
            .filter(Boolean));
        const available = family_definition_1.FOULEES_FAMILY_PACKS.filter((f) => !taken.has(f.id));
        const usable = available.length > 0 ? available : family_definition_1.FOULEES_FAMILY_PACKS;
        const usableChoices = usable.map(family_definition_1.toFouleesFamilyChoice);
        ...
        const prompt = `${(0, player_name_helper_1.resolvePlayerNameFromState)(withPending, currentId)} doit choisir une famille d'animaux.`;
    ...
    handleChooseFamily(state, action) {
        ...
        const withPrompt = this.appendLogOnce(state, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)} doit choisir une famille d'animaux.`);
        const meta = (withPrompt.metadata ??
            {});
        const rawFamily = action.payload?.familyId ?? action.payload?.value;
        const selected = this.setupFlow.resolveChoice(rawFamily, family_definition_1.FOULEES_FAMILY_PACKS.map(family_definition_1.toFouleesFamilyChoice));
        if (!selected) {
            return this.core.appendLog(state, 'Famille invalide.');
        }
        const familyId = String(selected.id ?? '')
            .trim()
            .toLowerCase();
        const pack = family_definition_1.FOULEES_FAMILY_PACKS.find((f) => f.id === familyId);
        if (!pack) {
            return this.core.appendLog(state, 'Famille invalide.');
        }
        const familyIdByPlayer = meta.familyIdByPlayer ?? {};
        const takenByOther = Object.entries(familyIdByPlayer).some(([pid, fid]) => {
            ...
            if (!Number.isFinite(otherId) || otherId === currentId)
                return false;
            return (String(fid ?? '')
                .trim()
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} choisit la famille des ${pack.family} (${pack.habitat}).`);
    ...
    handleRoll(state) {
        ...
        if (moves.length === 0) {
            ...
            next = this.core.appendLog(next, blockInfo ??
                `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)} ne peut jouer aucun pion.`);
        ...
        const hasStableExit = roll === 6 &&
            moves.some((m) => typeof m?.targetProgress === 'number' && m.targetProgress === 0);
        const label = hasStableExit && moves.every((m) => m.targetProgress === 0)
            ? `C'est à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} de choisir un animal à sortir dans la liste, puis Entrée.`
            : hasStableExit
                ? `C'est à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} de choisir un animal à sortir ou à jouer dans la liste, puis Entrée.`
                : `C'est à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} de choisir un animal à jouer dans la liste, puis Entrée.`;
        const pending = {
            type: 'choose_pawn',
            label,
            playerId: currentId,
    ...
    handleMovePawn(state, action) {
        ...
        const pawnIndex = typeof payload.pawnIndex === 'number'
            ? payload.pawnIndex
            : Number(payload.pawnIndex);
        const targetProgress = typeof payload.targetProgress === 'number'
    ...
    computeMoves(state, playerId, roll) {
        ...
        for (const pawn of pawns) {
            ...
            const prog = typeof pawn?.progress === 'number' ? pawn.progress : -1;
            if (typeof pawnIndex !== 'number')
                continue;
            if (prog >= arrivalProgress)
                continue;
            ...
            if (prog < 0) {
                ...
                const homeIndex = prog - meta.trackLength + 1;
                if (homeIndex >= 1 && homeIndex < meta.homeLength) {
                    const required = homeIndex + 1;
                    if (roll === required) {
    ...
    findBlockingOpponent(state, playerId, roll) {
        ...
        if (bestDistance == null)
            return null;
        const who = (0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId);
    ...
    applyMove(state, playerId, move, _roll) {
        ...
        const updatedPawns = pawns.map((p) => p?.pawnIndex === move.pawnIndex ? { ...p, progress: nextProg } : p);
        let next = {
            ...state,
            metadata: {
                ...(state.metadata ?? {}),
        ...
        if (prevProg < 0 && nextProg === 0) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} sort ${pawnLabel} ${this.fromHabitat(habitat)} et le place en case ${pos + 1}.`);
        }
        else {
            if (nextProg >= 0 && nextProg < meta.trackLength) {
                const pos = (offset + nextProg) % meta.trackLength;
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} place ${pawnLabel} en case ${pos + 1}.`);
            }
            else {
                const homeIndex = nextProg - meta.trackLength + 1;
                if (homeIndex >= 1 && homeIndex <= meta.homeLength) {
                    next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} met ${pawnLabel} dans l'abri (${homeIndex}/${meta.homeLength}).`);
                }
            }
        }
        if (prevProg >= 0 &&
            ...
            const homeIndex = nextProg - meta.trackLength + 1;
            if (homeIndex >= 1 && homeIndex <= meta.homeLength) {
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} entre ${pawnLabel} dans l'abri (${homeIndex}/${meta.homeLength}).`);
            }
        }
        const arrivalProgress = meta.trackLength + meta.homeLength - 1;
        if (prevProg < arrivalProgress && nextProg >= arrivalProgress) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} met ${pawnLabel} à l'arrivée.`);
        }
        next = this.applyCapture(next, playerId, move.pawnIndex, nextProg);
        if (this.isWinner(next, playerId, arrivalProgress)) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, playerId)} a gagné !`);
            return {
                ...next,
                status: 'finished',
    ...
    applyCapture(state, moverId, _moverPawnIndex, moverProgress) {
        const baseMeta = (state.metadata ??
            {});
        if (!(typeof moverProgress === 'number'))
            return state;
        ...
        const isSafe = Array.isArray(baseMeta.safeTiles) &&
        ...
        for (const p of players) {
            ...
            const updated = pawns.map((pawn) => {
                ...
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, moverId)} capture ${(0, player_name_helper_1.resolvePlayerNameFromState)(state, p.id)} (${capturedLabel}) : retour au départ.`);
                changed = true;
    ...
    endTurn(state, extraTurn) {
        if (extraTurn) {
            const currentId = state.turn?.currentPlayerId ?? null;
            const who = currentId != null
                ? (0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)
                : 'Le joueur';
    ...
    appendTurnAnnouncement(state) {
        ...
        const message = `C'est au tour de ${(0, player_name_helper_1.resolvePlayerNameFromState)(state, currentId)}.`;
    ...
    pawnLabel(state, playerId, pawnIndex) {
        ...
        const name = Array.isArray(list) && typeof list[pawnIndex] === 'string'
    ...
    habitatLabel(state, playerId) {
        const meta = (state.metadata ?? {});
        const habitat = typeof meta?.habitatByPlayer?.[playerId] === 'string'
    ...
    fromHabitat(habitat) {
        ...
        if (!raw)
            return "de l'abri de départ";
        if (h === 'écurie' || h === 'ecurie')
            return "de l'écurie";
        if (h === 'volière' || h === 'voliere')
            return 'de la volière';
        if (h === 'primaterie')
            return 'de la primaterie';
        if (h === 'aquarium')
            return "de l'aquarium";
...
exports.FouleesFantastiquesActionService = FouleesFantastiquesActionService;
exports.FouleesFantastiquesActionService = FouleesFantastiquesActionService = __decorate([
...
//# sourceMappingURL=foulees-fantastiques-action.service.js.map