...
exports.InteractiveExchangeService = void 0;
...
let InteractiveExchangeService = class InteractiveExchangeService {
    ...
    chooseGive(state, playerId, give, adapter) {
        ...
        if (pending.playerId !== playerId)
            return { kind: 'invalid', state };
        if (pending.step !== 'choose_give')
            return { kind: 'invalid', state };
        const targetPlayerId = typeof pending.targetPlayerId === 'number'
        ...
        const offer = {
            ...
            initiatorUsername: typeof initiator?.username === 'string' && initiator.username.trim()
                ? initiator.username.trim()
                : `Joueur ${playerId}`,
            targetPlayerId,
            targetUsername: typeof target?.username === 'string' && target.username.trim()
        ...
        return {
            kind: 'offered',
            state: { ...picked.state, pending: offer },
            offer,
        };
    }
    acceptOffer(state, targetPlayerId, adapter) {
        ...
        if (pending.playerId !== targetPlayerId)
            return { kind: 'invalid', state };
        const offer = pending;
...
exports.InteractiveExchangeService = InteractiveExchangeService;
exports.InteractiveExchangeService = InteractiveExchangeService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [random_service_1.RandomService])
], InteractiveExchangeService);
//# sourceMappingURL=interactive-exchange.service.js.map