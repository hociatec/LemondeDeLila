...
let CatalogService = class CatalogService {
    async getAllGames() {
        if (this.cachedGames && (this.cacheTtlMs === 0 || Date.now() < this.cacheExpiresAt)) {
            return this.cachedGames;
        }
        const games = await this.loadFromRegistry();
        this.cachedGames = games.filter((game)=>!!game.id);
        this.cacheExpiresAt = this.cacheTtlMs === 0 ? Number.MAX_SAFE_INTEGER : Date.now() + this.cacheTtlMs;
        return this.cachedGames;
    }
    async getCategories() {
    ...
    async loadFromRegistry() {
        ...
        const mapped = definitions.map((def)=>{
            ...
            return {
                ...
                chatSoundsEnabled: typeof def.chatSoundsEnabled === 'boolean' ? def.chatSoundsEnabled : true,
    ...
    slugify(value) {
        ...
        const noAccent = value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
    ...
    constructor(registry){
        ...
        this.cacheTtlMs = Number.isFinite(ttlCandidate) && ttlCandidate >= 0 ? ttlCandidate : 30000;
    }
};
