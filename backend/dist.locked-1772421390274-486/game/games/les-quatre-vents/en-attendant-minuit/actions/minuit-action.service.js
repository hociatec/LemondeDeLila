...
let MinuitActionService = class MinuitActionService {
    ...
    handleRoll(state) {
        ...
        if (meta.statuses?.forceDrawNextTurn?.[currentId] === true) {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} pioche une carte au lieu de lancer le de.`);
            return {
                ...next,
                pending: {
                    type: 'draw',
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} lance le dÃ© : "${roll}".`);
    ...
    handleChooseTarget(state, action) {
        ...
        if (ctx.kind === 'swap') {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} Ã©change sa position avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId, MINUIT_PLAYER_NAME_OPTIONS)}.`);
            return this.advanceTurnOrKeep(next, currentId);
        }
        if (ctx.kind === 'gift') {
            let next = {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} offre un cadeau a ${(0, _playernamehelper.resolvePlayerNameFromState)(next, targetPlayerId, MINUIT_PLAYER_NAME_OPTIONS)}.`);
    ...
    assignBotPawns(state) {
        ...
        for (const bot of assignedBots){
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, bot.id, MINUIT_PLAYER_NAME_OPTIONS)} a choisi le pion: ${this.resolvePawnName(nextMeta, bot.pawn)}.`);
        }
        return next;
    }
    handlePickPawn(state, action) {
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} a choisi le pion: ${resolvedPawnName}.`);
    ...
    applyLanding(state, playerId) {
        ...
        for(let step = 0; step < MINUIT_MAX_LANDING_STEPS; step += 1){
            ...
            if (occupant != null) {
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} place ${this.pawnPossessiveLabel(next, playerId)} sur une case occupÃ©e : recul d'une case.`);
        ...
        return this.core.appendLog(next, 'EnchaÃ®nement de cases interrompu pour Ã©viter une boucle infinie.');
    }
    applyCard(state, playerId, card) {
        ...
        if (/Ignorez la prochaine case.*Passe ton tour/i.test(text)) {
            ...
            return this.core.appendLog(next, 'Protection Â« passe ton tour Â» activÃ©e.');
        }
        ...
        if (/Piochez a nouveau une carte au lieu de lancer le de/i.test(text)) {
            ...
            return this.core.appendLog(next, 'Au prochain tour, piochez une carte a la place du de.');
        }
        ...
        if (/case neutre la plus proche derriÃ¨re/i.test(text)) {
            ...
            if (prevPos != null) {
                next = this.core.appendLog(next, 'Retour a la case neutre la plus proche derriere.');
        ...
        if (/jusqu'a la case precedente Carte Noel/i.test(text)) {
            ...
            if (prevPos != null) {
                next = this.core.appendLog(next, "Recule jusqu'a la precedente Carte Noel.");
                next = this.setPos(next, playerId, prevPos);
                return this.applyLanding(next, playerId);
            }
        }
        if (/position avec le joueur juste derriÃ¨re/i.test(text)) {
            ...
            if (behind != null) {
                ...
                next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} Ã©change sa position avec ${(0, _playernamehelper.resolvePlayerNameFromState)(next, behind, MINUIT_PLAYER_NAME_OPTIONS)}.`);
                return next;
            }
        }
        if (/Relancez immÃ©diatement le dÃ©/i.test(text) || /Relancez le dÃ© maintenant/i.test(text)) {
            meta = {
                ...meta,
                statuses: {
                    ...meta.statuses,
    ...
    formatCardEffect(card) {
        ...
        const filtered = lines.filter((line)=>!/^si le joueur a la bonne rÃ©ponse/i.test(String(line ?? '').trim()));
    ...
    parseQuizCard(playerId, card) {
        ...
        const anyCorrect = lines.some((l)=>/Les trois rÃ©ponses sont just(e|es)/i.test(l));
    ...
    pawnPossessiveLabel(state, id) {
        ...
        const stripped = inner.replace(/^(le|la|les|un|une)\s+/i, '').replace(/^l['â€™]\s*/i, '').trim();
...
function extractMoveDelta(text) {
    ...
    const forward = text.match(/avancez?\s+(?:de|d['â€™])\s*([0-9]+|un|une|deux|trois|quatre|cinq|six)\s+cases?/i);
    if (forward) return parse(forward[1]);
    const backward = text.match(/reculez?\s+(?:de|d['â€™])\s*([0-9]+|un|une|deux|trois|quatre|cinq|six)\s+cases?/i);
...
function extractFailureDelta(text) {
    ...
    const backward = text.match(/sinon[^.]*reculez?\s+(?:de|d['â€™])\s*([0-9]+|un|une|deux|trois|quatre|cinq|six)\s+cases?/i);
    if (backward) return -parse(backward[1]);
    const forward = text.match(/sinon[^.]*avancez?\s+(?:de|d['â€™])\s*([0-9]+|un|une|deux|trois|quatre|cinq|six)\s+cases?/i);
    if (forward) return parse(forward[1]);
    return 0;
}
