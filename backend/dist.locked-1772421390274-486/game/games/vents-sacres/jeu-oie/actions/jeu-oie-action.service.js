...
Object.defineProperty(exports, "JeuOieActionService", {
    enumerable: true,
    get: function() {
        return JeuOieActionService;
    }
});
...
let JeuOieActionService = class JeuOieActionService {
    applyActions(state, actions) {
        const next = (0, _actionservicehelper.applyActionsSequentially)(this.ensurePawnSelectionPrompt(state), actions, (current, action)=>{
    ...
    handleChoosePawn(state, action) {
        ...
        const choicesForPending = allPawnsForPending.map((p)=>({
                id: String(p?.id ?? '').trim(),
                label: String(p?.label ?? '').trim(),
                feminine: Boolean(p?.feminine)
        ...
        const starterName = (0, _playernamehelper.resolvePlayerNameFromState)(started, resolvedStarterId ?? 0);
    ...
    handleRoll(state) {
        ...
        if (inWell) {
            if (roll !== 1) {
                const logged = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} reste bloqué dans le puits.`);
                return this.turns.advanceTurn(logged, {
                    playerNameResolver: (s, id)=>(0, _playernamehelper.resolvePlayerNameFromState)(s, id)
                });
            }
            const metaAfter = this.getMeta(next);
            const well = {
                ...metaAfter.statuses?.well ?? {}
            };
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, currentId)} sort du puits.`);
        }
    ...
    applyLanding(state, playerId, position, roll) {
        ...
        next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} place ${this.pawnPossessiveLabel(next, playerId)} en case ${position} (${compactLabel}).`);
        if (!tile) return next;
        if (tile.description && String(tile.description).trim()) {
            next = this.core.appendLog(next, String(tile.description).trim());
        }
        if (tile.type === 'finish') {
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} a gagné !`);
        ...
        if (tile.type === 'inn' || tile.type === 'prison') {
            ...
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} perd ${turns} tour(s).`);
            meta = this.getMeta(next);
        ...
        if (tile.type === 'magic_die') {
            ...
            next = this.core.appendLog(next, `Dé magique : ${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} lance "${magicRoll}".`);
            const delta = magicRoll <= 3 ? magicRoll : -magicRoll;
            const moved = this.move(position, delta);
            next = this.core.appendLog(next, magicRoll <= 3 ? `Dé magique : avance de ${magicRoll} case(s).` : `Dé magique : recule de ${magicRoll} case(s).`);
            return this.applyLanding(next, playerId, moved, magicRoll);
        }
        if (tile.type === 'well') {
            const metaNow = this.getMeta(next);
            const well = {
                ...metaNow.statuses?.well ?? {}
            };
            well[playerId] = true;
            next = this.core.appendLog(next, `${(0, _playernamehelper.resolvePlayerNameFromState)(next, playerId)} est bloqué dans le puits (il faut faire 1 pour sortir).`);
            return {
                ...next,
                metadata: {
                    ...next.metadata ?? {},
    ...
    pawnLabel(state, id) {
        ...
        const pawn = Array.isArray(meta?.pawns) ? meta.pawns.find((p)=>String(p?.id ?? '').trim() === pawnId) : null;
    ...
    ensurePawnSelectionPrompt(state) {
        ...
        const availablePawns = (Array.isArray(meta.pawns) ? meta.pawns : []).map((pawn)=>({
        ...
        const fallbackPawns = (Array.isArray(meta.pawns) ? meta.pawns : []).map((pawn)=>({
                id: String(pawn?.id ?? '').trim(),
                label: String(pawn?.label ?? '').trim(),
                feminine: Boolean(pawn?.feminine)
...
JeuOieActionService = _ts_decorate([
