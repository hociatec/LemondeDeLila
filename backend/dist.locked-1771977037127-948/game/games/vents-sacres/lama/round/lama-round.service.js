...
exports.LamaRoundService = void 0;
...
let LamaRoundService = class LamaRoundService {
    ...
    startNewRound(state, starterIndex) {
        ...
        const rngMeta = typeof meta.rng === 'object' && meta.rng ? { ...meta.rng } : {};
        ...
        const starterName = starterPlayerId != null
        ...
        log = this.logger.append(log, `Défausse: ${(0, lama_model_1.lamaCardLabel)(firstDiscard)}.`);
        const nextMeta = {
            ...meta,
        ...
        }, { step: 'turn_choice', playerId: starterPlayerId });
    }
    endRound(state, winnerPlayerId) {
        ...
        const alreadyLoggedEnd = log.some((l) => String(l?.message ?? '') ===
            `Fin de la manche ${roundNumber}.`);
        if (alreadyLoggedEnd) {
            const winnerName = winnerPlayerId != null
                ? this.shared.playerLabel(players, winnerPlayerId)
                : null;
            const winnerScore = winnerPlayerId != null
                ? Number(scoresByPlayerId[String(winnerPlayerId)] ?? 0)
                : 0;
            const eligible = this.shouldPromptReturn(roundNumber, winnerScore) &&
                winnerPlayerId != null
                ? [winnerPlayerId]
                : [];
            const nextMeta = {
        ...
        for (const p of players) {
            ...
            const gained = unique.reduce((sum, v) => sum + (0, lama_model_1.lamaCardScore)(v), 0);
    ...
    finishRoundAndMaybeStartNext(state) {
        ...
        const newlyEliminated = players.filter((p) => p?.id &&
        ...
        if (pauseMs > 0) {
            ...
            }, { step: 'round_pause', playerId: meta.ownerPlayerId ?? null });
        }
        return this.startNewRound({ ...state, metadata: updatedMeta, round: nextRound, log }, starter);
    }
    ...
    buildDeck() {
        const deck = [];
        for (const v of [1, 2, 3, 4, 5, 6, lama_model_1.LAMA_VALUE]) {
            for (let i = 0; i < 8; i += 1)
                deck.push(v);
        }
        return deck;
    ...
    shouldPromptReturn(roundNumber, winnerScore) {
        if (winnerScore < 1)
            return false;
...
exports.LamaRoundService = LamaRoundService;
exports.LamaRoundService = LamaRoundService = __decorate([
...
//# sourceMappingURL=lama-round.service.js.map