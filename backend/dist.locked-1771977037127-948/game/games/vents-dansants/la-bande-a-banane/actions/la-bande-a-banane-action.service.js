...
let BandeABananeActionService = class BandeABananeActionService {
    ...
    playTrap(state, playerId, card) {
        ...
        if (card.trap === 'tigre-rodeur') {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} chute sur un tigre et lâche une carte.`);
            return next;
        }
        return state;
    }
    playVol(state, playerId, targetId) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} vole ${this.getCardName(stolen)} à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)}.`);
        return next;
    }
    playCris(state, playerId, targetId, giveCardId) {
        ...
        if (originalTargetHand.length) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} échange ${this.getCardName(returned)} avec ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)}.`);
            return next;
        }
        nextMeta = this.addCardToHand(nextMeta, targetId, giveCardId);
        next = this.setMeta(next, nextMeta);
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} donne une carte à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)}.`);
        return next;
    }
    ...
    addCardToTroop(state, playerId, cardId, species, isJoker) {
        ...
        if (this.hasWinningTroupe(this.getMeta(next), playerId)) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} crie à BANAAAANE ! à et devient le chef de la Bande à Banane !`);
            next = {
                ...next,
                status: 'finished',
                metadata: {
    ...
    enforceHandLimit(state, playerId) {
        ...
        while (hand.length > BandeABananeActionService_1.HAND_LIMIT) {
            ...
            next = this.setMeta(next, nextMeta);
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} dépasse 7 cartes et défausse ${this.getCardName(cardId)}.`);
