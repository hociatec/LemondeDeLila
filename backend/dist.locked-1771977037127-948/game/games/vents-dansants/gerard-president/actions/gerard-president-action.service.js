...
exports.GerardPresidentActionService = void 0;
...
let GerardPresidentActionService = class GerardPresidentActionService {
    ...
    applyActions(state, actions) {
        return (0, action_service_helper_1.applyActionsSequentially)(state, actions, (current, action) => this.applyAction(current, action));
    }
    ...
    handleSetTheme(state) {
        ...
        if (metadata.masterId != null && metadata.masterId !== playerId)
            return state;
        if (metadata.roundPhase !== 'waiting_theme')
            return state;
        ...
        const nextId = metadata.roundPhase === 'collecting_names'
    ...
    handlePlayName(state, payload) {
        ...
        const players = this.getPlayers(state);
        const metadata = this.cloneMetadata(state);
        if (metadata.roundPhase !== 'collecting_names')
            return state;
        if (!metadata.pendingPlayers.includes(playerId))
            return state;
        ...
        if (!distinct.length || distinct.length > allowed)
            return state;
        if (metadata.lockedName && distinct.includes(metadata.lockedName))
            return state;
        if (!distinct.every((name) => hand.includes(name)))
            return state;
        const removed = [];
    ...
    handlePlaySpecial(state, payload) {
        ...
        const nextState = this.appendLog(state, effectMessage ||
    ...
    applySpecialEffect(metadata, playerId, cardId, payload, players) {
        const definition = gerard_president_cards_1.GERARD_PRESIDENT_SPECIAL_CARDS.find((card) => card.id === cardId);
        const actorLabel = this.formatPlayer(playerId);
        if (!definition)
            return `${actorLabel} a joué une carte spéciale.`;
        ...
        switch (definition.effect) {
            ...
            case 'main-fantome': {
                if (targetId == null)
                    return `${actorLabel} devait choisir un joueur.`;
            ...
            case 'chuchotement-confus': {
                ...
                if (neighbor == null)
                    return `${actorLabel} n'a pas trouvé de voisin.`;
            ...
            case 'ultra-sabotage': {
                const targets = [targetId, secondaryId].filter((id) => id != null);
                if (!targets.length)
                ...
                return `${actorLabel} sabote ${targets
                    .map((id) => this.formatPlayer(id))
                    .join(' et ')}.`;
            }
            case 'prenom-volant': {
                if (targetId == null)
                    return `${actorLabel} n'a pas ciblé de joueur.`;
                if (!metadata.hands[playerId])
...
exports.GerardPresidentActionService = GerardPresidentActionService;
exports.GerardPresidentActionService = GerardPresidentActionService = __decorate([
...
//# sourceMappingURL=gerard-president-action.service.js.map