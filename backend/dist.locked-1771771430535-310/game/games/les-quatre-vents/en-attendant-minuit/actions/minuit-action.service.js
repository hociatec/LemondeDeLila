...
exports.MinuitActionService = void 0;
...
let MinuitActionService = class MinuitActionService {
    ...
    handleRoll(state) {
        ...
        if (meta0.pendingQuiz || state.pending)
            return state;
        let meta = meta0;
        if (meta.statuses?.forceDrawNextTurn?.[currentId] === true) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} pioche une carte au lieu de lancer le dé.`);
            return {
                ...next,
                pending: {
                    type: 'draw',
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} lance le dé : "${roll}".`);
        next = this.move(next, currentId, roll);
        next = this.applyLanding(next, currentId);
        meta = this.getMeta(next);
        if (meta.winnerId != null)
            return { ...next, status: 'finished' };
        if (meta.pendingQuiz || next.pending)
            return next;
    ...
    handleDraw(state) {
        ...
        if (!pending || pending.type !== 'draw')
            return state;
        const currentId = typeof pending.playerId === 'number'
            ? pending.playerId
    ...
    applyDrawCard(state, playerId) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} pioche "${draw.card.title}".`);
        if (effectText) {
    ...
    handleAnswerQuiz(state, action) {
        ...
        if (!pending || pending.playerId !== currentId)
            return state;
        const answer = toText(asRecord(action.payload).answer).trim();
        const correct = pending.anyCorrect === true
            ? true
            : (pending.answer ?? '').trim().toLowerCase() === answer.toLowerCase();
        let next = state;
        const who = (0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS);
        if (correct) {
    ...
    handleChooseTarget(state, action) {
        ...
        if (ctx.kind === 'swap') {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} échange sa position avec ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetPlayerId, MINUIT_PLAYER_NAME_OPTIONS)}.`);
            return this.advanceTurnOrKeep(next, currentId);
        }
        if (ctx.kind === 'gift') {
            let next = {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId, MINUIT_PLAYER_NAME_OPTIONS)} offre un cadeau �  ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetPlayerId, MINUIT_PLAYER_NAME_OPTIONS)}.`);
            next = this.move(next, targetPlayerId, 1);
            next = this.move(next, currentId, 2);
            next = this.applyLanding(next, currentId);
            const nextMeta = this.getMeta(next);
            if (nextMeta.pendingQuiz || next.pending)
                return next;
            return this.advanceTurnOrKeep(next, currentId);
        }
        return { ...state, pending: null };
    ...
    ensurePawnSelection(state) {
        ...
        if (players.length < minuit_definition_1.MINUIT_GAME.minPlayers)
            return state;
        ...
        if (status === 'started') {
            ...
            const stillNeedsBotPawns = withBotsPlayers.some((p) => !!p &&
        ...
        if (status !== 'starting' && status !== 'setup')
            return state;
    ...
    queuePawnSelection(state) {
        ...
        const pendingInfo = this.setupFlow.createSequentialPawnPending({
            ...
            labelForPlayer: (playerLabel) => `C'est �  ${playerLabel} de choisir son pion.`,
    ...
    assignBotPawns(state) {
        ...
        const updatedPlayers = players.map((p) => {
            if (!p)
                return p;
            const pawn = typeof p.pawn === 'string' && String(p.pawn).trim().length > 0
                ? String(p.pawn).trim()
        ...
        if (!changed && !metaChanged)
            return state;
        const nextMeta = { ...meta, pawns: assigned };
        let next = {
            ...state,
        ...
        for (const bot of assignedBots) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, bot.id, MINUIT_PLAYER_NAME_OPTIONS)} choisit le pion: ${this.resolvePawnName(nextMeta, bot.pawn)}.`);
        }
        return next;
    }
    handlePickPawn(state, action) {
        ...
        if (!resolvedPawn)
            return state;
        const players = Array.isArray(state.players) ? state.players : [];
        const takenByOthers = new Set(players
            .filter((p) => Number(p?.id) !== playerId)
            .map((p) => (typeof p?.pawn === 'string' ? p.pawn.trim() : ''))
            .filter((pawn) => pawn.length > 0));
        if (takenByOthers.has(resolvedPawn))
            return state;
        const updatedPlayers = players.map((p) => Number(p?.id) === playerId ? { ...p, pawn: resolvedPawn } : p);
        const meta = this.getMeta(state);
        const nextPawns = {
            ...(meta.pawns ?? {}),
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} choisit le pion: ${resolvedPawnName}.`);
    ...
    applyLanding(state, playerId) {
        ...
        if (occupant != null) {
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} place ${this.pawnPossessiveLabel(next, playerId)} sur une case occupée : recul d'une case.`);
        ...
        const afterPos = meta.positions?.[playerId] ?? 0;
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} place ${this.pawnPossessiveLabel(next, playerId)} en case ${afterPos + 1} (${tile.title}).`);
        ...
        if (afterPos === 55) {
            ...
            next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} atteint Minuit !`);
    ...
    applyCard(state, playerId, card) {
        ...
        if (/Piochez � {2}nouveau une carte au lieu de lancer le dé/i.test(text)) {
            ...
            next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
            return this.core.appendLog(next, 'Au prochain tour, piochez une carte �  la place du dé.');
        }
        if (/case neutre la plus proche derrière/i.test(text)) {
            const pos = meta.positions[playerId] ?? 0;
            const prevPos = findPrev(meta.tiles, pos, (t) => t.type === 'neutral');
            if (prevPos != null) {
                next = this.core.appendLog(next, 'Retour �  la case neutre la plus proche derrière.');
        ...
        if (skip > 0) {
            ...
            next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} passe ${skip} tour(s).`);
        }
        ...
        if (/jusqu['’]� {2}la case précédente Carte Noël/i.test(text)) {
            ...
            if (prevPos != null) {
                next = this.core.appendLog(next, "Recule jusqu'�  la précédente Carte Noël.");
                next = this.setPos(next, playerId, prevPos);
                return this.applyLanding(next, playerId);
            }
        }
        if (/position avec le joueur juste derrière/i.test(text)) {
            ...
            if (behind != null) {
                ...
                next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} échange sa position avec ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, behind, MINUIT_PLAYER_NAME_OPTIONS)}.`);
                return next;
            }
        }
        if (/Relancez immédiatement le dé/i.test(text) ||
            ...
            next = { ...next, metadata: { ...(next.metadata ?? {}), ...meta } };
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId, MINUIT_PLAYER_NAME_OPTIONS)} rejoue.`);
        }
    ...
    formatCardEffect(card) {
        ...
        if (isQuiz) {
            const question = withoutChoices.find((l) => String(l).includes('?')) ??
                withoutChoices[0] ??
                '';
    ...
    otherPlayers(state, me) {
        ...
        return players
            ...
            .map((p) => ({
            id: p.id,
            username: (0, player_name_helper_1.resolvePlayerNameFromState)(state, p.id, MINUIT_PLAYER_NAME_OPTIONS),
        }));
    }
    ...
    resolvePawnName(meta, pawnIdOrLabel) {
        const rawLabel = pawnIdOrLabel === null || pawnIdOrLabel === undefined
            ? ''
            : pawnIdOrLabel;
        const preparedLabel = typeof rawLabel === 'string' ||
    ...
    getPromptPolicies() {
        return this.promptPolicies ?? new prompt_policies_service_1.PromptPoliciesService(this.core);
    }
    restoreStarterAfterPawnSelection(state) {
        ...
        const players = Array.isArray(state.players) ? state.players : [];
        const starterIdRaw = typeof meta.starterPlayerId === 'number'
...
exports.MinuitActionService = MinuitActionService;
exports.MinuitActionService = MinuitActionService = __decorate([
...
function extractSkipTurns(text) {
    ...
    if (/Passez deux tours/i.test(text))
        return 2;
    if (/Passez un tour/i.test(text))
        return 1;
    if (/Passez votre tour/i.test(text) || /Passe ton tour/i.test(text))
        return 1;
    if (/Vous passez trois tours/i.test(text))
        return 3;
    if (/Vous passez deux tours/i.test(text))
        return 2;
    if (/Vous passez un tour/i.test(text))
        return 1;
    return 0;
}
...
//# sourceMappingURL=minuit-action.service.js.map