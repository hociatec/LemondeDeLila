...
let AbsurdissimesActionService = class AbsurdissimesActionService {
    ...
    handlePlayCard(state, action) {
        ...
        if (!remainingPlayers.length) {
            ...
            next = this.core.appendLog(next, 'Les cartes sont pr�tes : le juge choisit la proposition gagnante.');
            return next;
        }
        next = {
            ...next,
    ...
    handleJudgePick(state, action) {
        ...
        meta = { ...meta, scores };
        let next = this.core.appendLog(state, `${(0, player_name_helper_1.resolvePlayerName)(state.players, winnerId)} remporte la manche avec la r�ponse ${meta.submissions[winnerId] ?? ''}.`);
