...
async function bootstrap() {
    ...
    const updatesDir = config.get('CLIENT_UPDATES_DIR') ||
        path.resolve(__dirname, '..', 'data', 'client-updates', 'client-win');
    if (!config.get('CLIENT_UPDATES_DIR') &&
        (process.env.NODE_ENV || '').toLowerCase() === 'production') {
        console.warn(`[updates] CLIENT_UPDATES_DIR is not set; using default inside repo: ${updatesDir}. ` +
