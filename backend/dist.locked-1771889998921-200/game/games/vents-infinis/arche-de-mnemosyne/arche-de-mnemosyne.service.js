...
exports.ArcheDeMnemosyneService = void 0;
...
let ArcheDeMnemosyneService = class ArcheDeMnemosyneService extends abstract_game_service_1.AbstractGameService {
    core;
    turns;
    store;
    random;
    ...
    description = 'Quiz à catégories (questions aléatoires).';
    minPlayers = 1;
    maxPlayers = 8;
    ...
    hydrateInitialState(baseState) {
        ...
        const ownerPlayerId = (() => {
            if (typeof baseMeta.roomOwnerId === 'number')
                return baseMeta.roomOwnerId;
    ...
    validateAction(state, action, actorId) {
        ...
        const actor = String(action?.meta?.actor ?? '')
            .trim()
            .toLowerCase();
        ...
        if (type === 'answer_quiz') {
            ...
            const deadline = typeof meta.quizDeadlineAtMs === 'number'
        ...
        if (type.startsWith('mnemo_')) {
            ...
            if (type === 'mnemo_set_config' || type === 'mnemo_prompt_cancel') {
                const ownerId = typeof meta.promptOwnerId === 'number'
                    ? meta.promptOwnerId
                    : null;
    ...
    applyActions(state, actions) {
        const next = (0, action_service_helper_1.applyActionsSequentially)(state, actions, (current, action) => this.applyOne(current, action));
    ...
    getShortcuts(ctx) {
        ...
        return [(0, shortcut_utils_1.interfaceShortcut)('S', 'score'), (0, shortcut_utils_1.actionShortcut)('SPACE', 'draw')];
    }
    ...
    applyOne(state, action) {
        ...
        if (type === 'mnemo_timeout') {
            ...
            const deadline = typeof meta.quizDeadlineAtMs === 'number'
                ...
                : null;
            if (!q || deadline == null) {
                const until = typeof meta.interQuestionUntilMs === 'number'
        ...
        if (type === 'mnemo_start') {
            ...
            if (actorId != null) {
                ...
                started = this.core.appendLog(started, `${(0, player_name_helper_1.resolvePlayerNameFromState)(started, actorId, ARCHE_PLAYER_NAME_OPTIONS)} choisit la catégorie : ${label}.`);
            }
        ...
        if (type === 'draw') {
            ...
            if (actorId == null)
                return state;
            const interUntilMs = typeof meta.interQuestionUntilMs === 'number'
                ? meta.interQuestionUntilMs
                : null;
        ...
        if (type === 'answer_quiz') {
            ...
            if (actorId == null || !question)
                return state;
            const answers = { ...(meta.quizAnswersByPlayerId ?? {}) };
            if (answers[actorId] != null)
                return state;
        ...
        if (type === 'mnemo_add_category') {
            try {
                ...
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, 'Catégorie ajoutée.');
            }
            catch (err) {
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, `Erreur: ${err.message}`);
            }
        }
        ...
        if (type === 'mnemo_rename_category') {
            ...
            try {
                ...
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, 'Catégorie renommée.');
            }
            catch (err) {
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, `Erreur: ${err.message}`);
            }
        ...
        if (type === 'mnemo_delete_category') {
            ...
            try {
                ...
                const nextView = { page: 'categories' };
        ...
        if (type === 'mnemo_add_question') {
            ...
            try {
                ...
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, 'Question ajoutée.');
            }
            catch (err) {
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, `Erreur: ${err.message}`);
            }
        ...
        if (type === 'mnemo_set_question_status') {
            ...
            try {
                ...
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, `Statut mis à jour (${this.statusLabel(status)}).`);
            }
            catch (err) {
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, `Erreur: ${err.message}`);
            }
        ...
        if (type === 'mnemo_edit_question') {
            ...
            try {
                ...
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, 'Question modifiée.');
            }
            catch (err) {
                return this.core.appendLog({ ...state, metadata: { ...meta, prompt: null } }, `Erreur: ${err.message}`);
            }
    ...
    resolveQuizIfReady(state, force = false, timedOutPlayerIds = []) {
        ...
        const currentRound = Number.isFinite(currentRoundRaw) && currentRoundRaw > 0
        ...
        if (correctIds.length === 0) {
            ...
            const msg = correctSoloPoints === 0
                ? `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, id, ARCHE_PLAYER_NAME_OPTIONS)} ne marque aucun point.`
                : correctSoloPoints > 0
                    ? `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, id, ARCHE_PLAYER_NAME_OPTIONS)} gagne +${correctSoloPoints} points.`
                    : `${(0, player_name_helper_1.resolvePlayerNameFromState)(state, id, ARCHE_PLAYER_NAME_OPTIONS)} perd ${Math.abs(correctSoloPoints)} points.`;
            ...
            const labels = correctIds
                .map((id) => (0, player_name_helper_1.resolvePlayerNameFromState)(state, id, ARCHE_PLAYER_NAME_OPTIONS))
                .join(', ');
            const msg = correctMultiPoints === 0
        ...
        if (force || endedBecauseAllAnswered) {
            for (const id of playerIds) {
                ...
                const who = (0, player_name_helper_1.resolvePlayerNameFromState)(state, id, ARCHE_PLAYER_NAME_OPTIONS);
                if (idx == null) {
                ...
                next = this.core.appendLog(next, correct
        ...
        if (reached.length) {
            ...
            const finished = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, winnerId, ARCHE_PLAYER_NAME_OPTIONS)} a gagné !`);
            return {
                ...finished,
                status: 'finished',
                round: currentRound,
    ...
    drawNextQuestionOrStay(state) {
        ...
        const selected = meta.selectedCategoryId &&
        ...
        const normalizeKey = (value) => (0, string_value_utils_1.stringOrEmpty)(value).trim().toLocaleLowerCase('fr');
        const rawChoices = [
            picked.correct,
            picked.wrong1,
            picked.wrong2,
            picked.wrong3,
        ].map((s) => (0, string_value_utils_1.stringOrEmpty)(s).trim());
        const unique = [];
        const seen = new Set();
    ...
    syncBotPending(state) {
        try {
            ...
            const phase = String(state.phase ?? '')
                .toLowerCase()
                .trim();
            const promptOwnerId = typeof meta.promptOwnerId === 'number'
                ? meta.promptOwnerId
                : null;
            if (phase === 'setup') {
    ...
    buildPendingForUser(state, userId) {
        ...
        const promptOwnerId = typeof meta.promptOwnerId === 'number'
            ? meta.promptOwnerId
            : null;
        const canSeePrompt = Boolean(meta.prompt) &&
        ...
        if (meta.currentQuestion &&
            meta.quizAnswersByPlayerId?.[userId] == null) {
            return {
                type: 'quiz',
                ...
                question: meta.currentQuestion.question,
                choices: meta.currentQuestion.choices,
                deadlineAtMs: typeof meta.quizDeadlineAtMs === 'number'
                    ? meta.quizDeadlineAtMs
                    : null,
            };
        }
    ...
    buildActionsForUser(state, userId) {
        ...
        if (canSeePrompt && meta.prompt) {
            ...
            const cancelType = String(meta.prompt?.cancelActionType ?? 'mnemo_prompt_cancel')
                .trim()
                .toLowerCase();
...
exports.ArcheDeMnemosyneService = ArcheDeMnemosyneService;
exports.ArcheDeMnemosyneService = ArcheDeMnemosyneService = __decorate([
...
//# sourceMappingURL=arche-de-mnemosyne.service.js.map