...
exports.OlympiaActionService = void 0;
...
let OlympiaActionService = class OlympiaActionService {
    ...
    handleDrawCard(state, action) {
        ...
        if (!entry.cardId) {
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} n'a plus de cartes dans le deck ${deck}.`);
        }
        const updatedMeta = this.addCardToHand({ ...entry.meta, rng: entry.meta.rng }, currentId, entry.cardId);
        next = this.setMeta(next, updatedMeta);
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} pioche ${this.getCardName(entry.cardId)} (${deck}).`);
    ...
    handlePlayCard(state, action) {
        ...
        if (!cardId)
            return state;
        const definition = olympia_cards_1.OLYMPIA_CARD_BY_ID[cardId];
        if (!definition)
            return state;
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, currentId)} joue ${definition.name}.`);
    ...
    applyEffect(state, actorId, effect, targetId) {
        ...
        if (effect.type === 'prestige') {
            ...
            if (targetId != null) {
                ...
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, targetId)} doit passer ${effect.turns} tour(s).`);
            }
        }
        return next;
    }
    ...
    addPrestige(state, playerId, amount) {
        ...
        next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} ${amount >= 0 ? 'gagne' : 'perd'} ${Math.abs(amount)} point(s) de prestige.`);
    ...
    discardRandom(state, playerId, amount) {
        ...
        if (removed.length) {
            return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} défausse ${removed.length} carte(s).`);
        }
        return next;
    }
    exchangeCard(state, actorId, targetId, categories) {
        ...
        const targetHand = Array.isArray(meta.hands?.[targetId])
            ? [...meta.hands[targetId]]
            : [];
        const actorCard = actorHand.find((cardId) => categories.includes(olympia_cards_1.OLYMPIA_CARD_BY_ID[cardId]?.category));
    ...
    applySteal(state, actorId, amount) {
        ...
        return this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, actorId)} vole ${amount} point(s) à ${(0, player_name_helper_1.resolvePlayerNameFromState)(next, target.id)}.`);
    }
    drawForPlayer(state, playerId, amount, decks) {
        ...
        for (let i = 0; i < amount; i += 1) {
            for (const deck of decks) {
                ...
                next = this.core.appendLog(next, `${(0, player_name_helper_1.resolvePlayerNameFromState)(next, playerId)} pioche ${this.getCardName(entry.cardId)} (${deck}).`);
                break;
            }
        }
        return next;
    ...
    getCardName(cardId) {
        return olympia_cards_1.OLYMPIA_CARD_BY_ID[cardId]?.name ?? cardId;
    }
};
exports.OlympiaActionService = OlympiaActionService;
exports.OlympiaActionService = OlympiaActionService = __decorate([
...
//# sourceMappingURL=olympia-action.service.js.map