...
|---------|------|------------------|
| `nginx` | Reverse-proxy HTTPS pour `hociatec.fr`, `api.hociatec.fr` et `ws.hociatec.fr`. Sert Symfony et relaie les WebSockets. | `sudo systemctl reload nginx` |
...
- `ws.hociatec.fr` est un reverse-proxy HTTPS → `http://127.0.0.1:8081`, avec les en-têtes nécessaires (`Upgrade`, `Connection`, `X-Forwarded-*`) pour les WebSockets.
...
curl -I https://ws.hociatec.fr/ws
