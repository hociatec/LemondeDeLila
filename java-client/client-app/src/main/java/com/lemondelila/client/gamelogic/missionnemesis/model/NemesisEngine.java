 ...
 */
public final class NemesisEngine implements GameEngine<NemesisState, NemesisEngine.Action, NemesisEngine.PlayerIdentity> {
    ...
    @Override
    public NemesisState defaultState(Iterable<PlayerIdentity> players) {
        List<NemesisState.Player> converted = new ArrayList<>();
        for (PlayerIdentity player : players) {
            converted.add(new NemesisState.Player(
                    player.id(),
                    player.username(),
                    new ArrayList<>(),
                    new ArrayList<>(),
    ...
    @Override
    public NemesisState apply(NemesisState state, Action action, PlayerIdentity actor) {
    ...
    @Override
    public int currentRound(NemesisState state) {
        return Math.max(1, state.round());
    }

    @Override
    public Score score(NemesisState state) {
    ...
    private MutableState handlePlacement(MutableState state, int playerIndex, PlaceShips action) {
        ...
        if (state.allPlayersReady()) {
            ...
            state.round = 1;
            state.log.add(new NemesisState.LogEntry("phase", "combat", null, null, null, null, null));
        }
    ...
    private MutableState handleFire(MutableState state, int playerIndex, FireShot action) {
        ...
        shooter.shots.add(new NemesisState.Shot(
                coordinate.x(),
                coordinate.y(),
                target.id,
                result
        ...
        state.log.add(new NemesisState.LogEntry(
                "shot",
                null,
                shooter.id,
                target.id,
    ...
    private static final class MutableState {
        ...
        private final List<NemesisState.LogEntry> log;
        private String status;
        private Integer winnerId;
        private int turnIndex;
        private int round;

        private MutableState(NemesisState source) {
            this.players = new ArrayList<>();
            for (NemesisState.Player player : source.players()) {
        ...
        private NemesisState toImmutable() {
            List<NemesisState.Player> immutablePlayers = new ArrayList<>();
        ...
        private boolean hasAlreadyShot(MutablePlayer shooter, Coordinate coord, int targetId) {
            for (NemesisState.Shot shot : shooter.shots) {
    ...
    private static final class MutablePlayer {
        ...
        private NemesisState.Player toImmutable() {
            List<NemesisState.Ship> immutableShips = new ArrayList<>();
            for (MutableShip ship : ships) {
                immutableShips.add(ship.toImmutable());
            }
            List<NemesisState.Shot> immutableShots = new ArrayList<>(shots);
            return new NemesisState.Player(id, username, immutableShips, immutableShots, status, bot);
        }
    }
    ...
    private static final class MutableShip {
        ...
        private MutableShip(NemesisState.Ship ship) {
            this.name = ship.name();
            this.coordinates = new ArrayList<>();
            for (NemesisState.Coordinate coordinate : ship.coordinates()) {
        ...
        private NemesisState.Ship toImmutable() {
            List<NemesisState.Coordinate> immutableCoords = new ArrayList<>();
            for (Coordinate coordinate : coordinates) {
                immutableCoords.add(new NemesisState.Coordinate(coordinate.x(), coordinate.y()));
            }
            return new NemesisState.Ship(name, immutableCoords, hits.clone());
        }
    }
}
